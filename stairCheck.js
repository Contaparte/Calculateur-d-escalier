 ...
 * =====================================================================
 * 
 * Conforme au Code de construction du Québec, Chapitre I â€“ Bâtiment
 * et Code national du bâtiment â€“ Canada 2020 (modifié)
 * 
 * Caractéristiques:
...
const CNB_LIMITS = {
    ...
    part3: {
        common: {
            ...
            minWidthUnder3: 900,    // CNB 3.4.3.2 Tableau - â‰¤3 étages
...
function generateStraightStairVisualization(stairData) {
    ...
    // ===== LEGENDE EN BAS (sur 2 lignes pour meilleure lisibilité) =====
    const legendY1 = H - 24;
    const legendY2 = H - 10;
    svg += '<text x="15" y="' + legendY1 + '" style="font:10px Arial;fill:#555;">';
    svg += 'Légende: CM = ' + riserText + '  |  G = ' + treadText + '  |  Largeur = ' + widthText;
    svg += '</text>';
...
function generateLShapedStairVisualization(stairData) {
    ...
    const legend1 = 'Légende: CM = ' + riserText + '  |  G = ' + treadText + '  |  Largeur = ' + widthText;
    ...
    if (isLanding) {
        ...
        legend2 += '  |  ' + numRadSteps + ' marches ray. (' + (numRadSteps === 2 ? '45°' : '30°') + ')';
    }
...
function generateUShapedStairVisualization(stairData) {
    ...
    // Pour un escalier en U avec palier simple, le palier a une profondeur = largeur de l'escalier
    const actualLandingDepth = landingLength || actualWidth;
    ...
    // L'escalier en U: 2 volées côte à côte + palier qui les relie
    // Largeur totale = 2 × largeur escalier
    // Hauteur totale = longueur volée + profondeur palier
    const totalPlanWidth = 2 * actualWidth;
    ...
    if (flight1Treads > 0) {
        ...
        // Numérotation volée 1 (à l'intérieur de la volée)
        const tread1W = flight1Run_plan / flight1Treads;
        svg += '<text x="' + r(flight1X + stairWidth_plan - 8) + '" y="' + r(flight1Y + flight1Run_plan - 8) + '" style="font:8px Arial;fill:#666;" text-anchor="end">1</text>';
        svg += '<text x="' + r(flight1X + stairWidth_plan - 8) + '" y="' + r(flight1Y + 12) + '" style="font:8px Arial;fill:#666;" text-anchor="end">' + flight1Treads + '</text>';
    }
    ...
    const flight2X = r(planStartX + stairWidth_plan);
    ...
    if (flight2Treads > 0) {
        ...
        // Numérotation volée 2 (à l'intérieur de la volée)
        svg += '<text x="' + r(flight2X + 8) + '" y="' + r(flight2Y + 12) + '" style="font:8px Arial;fill:#666;">' + (flight1Treads + 1) + '</text>';
        svg += '<text x="' + r(flight2X + 8) + '" y="' + r(flight2Y + flight2Run_plan - 8) + '" style="font:8px Arial;fill:#666;">' + numTreads + '</text>';
    }
    ...
    // E.H. en bas à gauche de la volée 1 (entrée)
    svg += '<text x="' + r(flight1X + stairWidth_plan/2 - 20) + '" y="' + r(arrowY1Start + 5) + '" style="font:bold 9px Arial;fill:#1b5e20;">E.H.</text>';
    
    // ===== COTATIONS =====
    // Cotation TOTALE côté gauche (volée 1 + palier) - comme pour escalier en L
    const dimX1 = r(flight1X - 30);
    const totalLeft = actualFlight1Run + actualLandingDepth;
    const totalLeftText = formatValuePrecise(totalLeft, isMetric);
    svg += '<line x1="' + dimX1 + '" y1="' + planStartY + '" x2="' + dimX1 + '" y2="' + r(flight1Y + flight1Run_plan) + '" stroke="#555" stroke-width="1" marker-start="url(#arrUS)" marker-end="url(#arrUE)"/>';
    svg += '<text x="' + (dimX1 - 5) + '" y="' + r((planStartY + flight1Y + flight1Run_plan)/2) + '" style="font:10px Arial;fill:#333;" text-anchor="end" transform="rotate(-90 ' + (dimX1 - 5) + ' ' + r((planStartY + flight1Y + flight1Run_plan)/2) + ')">' + totalLeftText + '</text>';
    
    // Cotation TOTALE côté droit (volée 2 + palier) - comme pour escalier en L
    const dimX2 = r(flight2X + stairWidth_plan + 30);
    const totalRight = actualFlight2Run + actualLandingDepth;
    const totalRightText = formatValuePrecise(totalRight, isMetric);
    svg += '<line x1="' + dimX2 + '" y1="' + planStartY + '" x2="' + dimX2 + '" y2="' + r(flight2Y + flight2Run_plan) + '" stroke="#555" stroke-width="1" marker-start="url(#arrUS)" marker-end="url(#arrUE)"/>';
    svg += '<text x="' + (dimX2 + 5) + '" y="' + r((planStartY + flight2Y + flight2Run_plan)/2) + '" style="font:10px Arial;fill:#333;" transform="rotate(-90 ' + (dimX2 + 5) + ' ' + r((planStartY + flight2Y + flight2Run_plan)/2) + ')">' + totalRightText + '</text>';
    
    // Cotation largeur totale (en bas)
    const dimY1 = r(flight1Y + Math.max(flight1Run_plan, flight2Run_plan) + 20);
    const totalWidthVal = 2 * actualWidth;
    const totalWidthText = formatValuePrecise(totalWidthVal, isMetric);
    ...
    // Annotations girons rectangulaires (dans les volées, plus discrètes)
    svg += '<text x="' + r(flight1X + stairWidth_plan/2) + '" y="' + r(flight1Y + flight1Run_plan/2) + '" style="font:9px Arial;fill:#666;" text-anchor="middle">' + flight1Treads + ' girons</text>';
    svg += '<text x="' + r(flight2X + stairWidth_plan/2) + '" y="' + r(flight2Y + flight2Run_plan/2) + '" style="font:9px Arial;fill:#666;" text-anchor="middle">' + flight2Treads + ' girons</text>';
    
    // ===== LÉGENDE EN BAS (sur 2 lignes) =====
    const legendY1 = H - 24;
    const legendY2 = H - 10;
    const landingText = formatValuePrecise(actualLandingDepth, isMetric);
...
function displayCalculatorResults(solutions, params) {
    ...
    if (!solutions || solutions.length === 0) {
        ...
        if (flightHeightExceeded && !hasIntermediateLanding) {
            ...
            if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
                ...
                if (treadTooSmall) {
                    if (isMetric) {
                        html += `<li>Longueur totale minimale requise â‰¥ ${minLengthWithLanding.toFixed(0)} mm (${minTreads} girons × ${limits.minTread} mm + 2× palier)</li>`;
                    } else {
                        html += `<li>Longueur totale minimale requise â‰¥ ${metricToImperial(minLengthWithLanding)} (${minTreads} girons × ${metricToImperial(limits.minTread)} + 2× palier)</li>`;
                    }
                ...
                if (treadTooSmall) {
                    if (isMetric) {
                        html += `<li>Longueur totale minimale requise â‰¥ ${minLength.toFixed(0)} mm (${minTreads} girons × ${limits.minTread} mm)</li>`;
                    } else {
                        html += `<li>Longueur totale minimale requise â‰¥ ${metricToImperial(minLength)} (${minTreads} girons × ${metricToImperial(limits.minTread)})</li>`;
                    }
                ...
                if (treadTooSmall) {
                    if (isMetric) {
                        html += `<li>Longueur minimale requise â‰¥ ${minLength.toFixed(0)} mm (${minTreads} girons × ${limits.minTread} mm)</li>`;
                    } else {
                        html += `<li>Longueur minimale requise â‰¥ ${metricToImperial(minLength)} (${minTreads} girons × ${metricToImperial(limits.minTread)})</li>`;
                    }
    ...
    if (!isRiserOk) {
        html += `<div class="warning"><p>âš  Hauteur de contremarche ${formatValuePrecise(best.riserHeight, isMetric)} hors limites CNB (${formatValue(limits.minRiser, isMetric, 0)} à ${formatValue(limits.maxRiser, isMetric, 0)})</p></div>`;
    }
    if (!isTreadOk) {
        if (hasMaxTreadLimit) {
            html += `<div class="warning"><p>âš  Profondeur de giron ${formatValuePrecise(best.treadDepth, isMetric)} hors limites CNB (${formatValue(limits.minTread, isMetric, 0)} à ${formatValue(limits.maxTread, isMetric, 0)})</p></div>`;
        } else {
    ...
    if (isMetric) {
        ...
        html += `${best.numRisers} × ${riserIn.toFixed(6)}" = ${totalRiseIn.toFixed(6)}" âœ“`;
    }
    ...
    html += `<li>${best.stepRule.rule2.isValid ? 'âœ“' : 'â—‹'} G × H = ${best.stepRule.rule2.value.toFixed(2)} (${best.stepRule.rule2.range})</li>`;
...
function displayVerificationResults(params) {
    ...
    html += `<li>${stepRule.rule2.isValid ? 'âœ“' : 'âœ—'} Règle 2 (G×H) : ${stepRule.rule2.value.toFixed(2)} po² (71-74)</li>`;
