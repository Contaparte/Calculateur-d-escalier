...
function calculateLShapedWithLanding(params) {
    ...
    for (let numRisers = minRisers; numRisers <= maxRisers + 3; numRisers++) {
        ...
        // Contremarches par volée: volée 1 a une CM de plus (monte sur le palier)
        const risersInFlight1 = Math.max(1, treadsInFlight1) + 1;
        const risersInFlight2 = numRisers - risersInFlight1;
        
        // Dimensions réelles de chaque côté long (girons + palier)
        const flight1RealLength = (Math.max(1, treadsInFlight1) * treadDepth) + landingDepth;
        const flight2RealLength = (Math.max(1, treadsInFlight2) * treadDepth) + landingDepth;
...
function calculateSpiralStair(params) {
    ...
    // Pour l'hélicoïdal, le giron est mesuré Ã  300mm de l'axe de la main courante
...
function displayCalculatorResults(solutions, params) {
    ...
    if (!solutions || solutions.length === 0) {
        ...
        let html = '<h3>âŒ Aucune solution conforme au CCQ</h3>';
        html += '<div class="warning">';
        html += '<p><strong>Raisons de non-conformité :</strong></p>';
        html += '<ul>';
        
        // Problème de largeur
        if (!isWidthOk) {
            if (isMetric) {
                html += `<li><strong>Largeur insuffisante :</strong> ${stairWidthValue.toFixed(0)} mm < minimum requis ${limits.minWidth} mm</li>`;
            } else {
                html += `<li><strong>Largeur insuffisante :</strong> ${metricToImperial(stairWidthValue)} < minimum requis ${metricToImperial(limits.minWidth)}</li>`;
            }
        }
        
        // Problème de giron
        if (isMetric) {
            html += `<li><strong>Giron calculé :</strong> ${calculatedTread.toFixed(0)} mm < minimum requis ${limits.minTread} mm (${codeRef})</li>`;
        } else {
            html += `<li><strong>Giron calculé :</strong> ${metricToImperial(calculatedTread)} < minimum requis ${metricToImperial(limits.minTread)} (${codeRef})</li>`;
        }
        ...
        if (!isWidthOk) {
            if (isMetric) {
                html += `<li>Augmentez la largeur de l'escalier Ã  au moins ${limits.minWidth} mm</li>`;
            } else {
                html += `<li>Augmentez la largeur de l'escalier Ã  au moins ${metricToImperial(limits.minWidth)}</li>`;
            }
        }
    ...
    // Filtrer pour ne garder que les solutions conformes Ã  la règle du pas
    const compliantSolutions = solutions.filter(sol => sol.stepRule.isValid);
    
    // Si la largeur n'est pas conforme OU aucune solution ne respecte la règle du pas
    if (!isWidthOk || compliantSolutions.length === 0) {
        let html = '<h3>âŒ Aucune solution conforme au CCQ</h3>';
        html += '<div class="warning">';
        html += '<p><strong>Raisons de non-conformité :</strong></p>';
        html += '<ul>';
        ...
        // Problème de règle du pas
        if (compliantSolutions.length === 0 && solutions.length > 0) {
            const best = solutions[0];
            html += '<li><strong>Règle du pas non respectée (0 sur 3) :</strong>';
            html += '<ul style="margin-top: 5px;">';
            
            const G = best.treadDepth;
            const H = best.riserHeight;
            const rule1 = G + H;
            const rule2 = G * H;
            const rule3 = G + 2 * H;
            
            if (isMetric) {
                html += `<li>G + H = ${rule1.toFixed(1)} mm (requis: 430-460 mm) ${rule1 >= 430 && rule1 <= 460 ? 'âœ”' : 'âœ—'}</li>`;
                html += `<li>G Ã— H = ${(rule2/1000).toFixed(1)} (requis: 45 800-48 000 mm²) ${rule2 >= 45800 && rule2 <= 48000 ? 'âœ”' : 'âœ—'}</li>`;
                html += `<li>G + 2H = ${rule3.toFixed(1)} mm (requis: 560-635 mm) ${rule3 >= 560 && rule3 <= 635 ? 'âœ”' : 'âœ—'}</li>`;
            } else {
                const Gin = G / 25.4;
                const Hin = H / 25.4;
                html += `<li>G + H = ${(Gin + Hin).toFixed(2)}" (requis: 17"-18") ${(Gin + Hin) >= 17 && (Gin + Hin) <= 18 ? 'âœ”' : 'âœ—'}</li>`;
                html += `<li>G Ã— H = ${(Gin * Hin).toFixed(2)} (requis: 71-74 po²) ${(Gin * Hin) >= 71 && (Gin * Hin) <= 74 ? 'âœ”' : 'âœ—'}</li>`;
                html += `<li>G + 2H = ${(Gin + 2*Hin).toFixed(2)}" (requis: 22"-25") ${(Gin + 2*Hin) >= 22 && (Gin + 2*Hin) <= 25 ? 'âœ”' : 'âœ—'}</li>`;
            }
            html += '</ul></li>';
            
            // Afficher les dimensions calculées pour information
            html += '</ul>';
            html += '<p><strong>Dimensions calculées (non conformes) :</strong></p>';
            html += '<ul>';
            html += `<li>Contremarches : ${best.numRisers} Ã— ${formatValuePrecise(best.riserHeight, isMetric)}</li>`;
            html += `<li>Girons : ${best.numTreads} Ã— ${formatValuePrecise(best.treadDepth, isMetric)}</li>`;
        }
        
        html += '</ul>';
        html += '<p><strong>Suggestions pour obtenir une solution conforme :</strong></p>';
        html += '<ul>';
        ...
        if (compliantSolutions.length === 0) {
            html += '<li>Augmentez la longueur horizontale disponible pour permettre des girons plus profonds</li>';
            html += '<li>Réduisez la hauteur totale Ã  gravir si possible</li>';
            if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
                html += '<li>Réduisez la largeur de l\'escalier (= profondeur du palier) pour libérer plus d\'espace pour les girons</li>';
            }
            html += '<li>Envisagez une configuration différente (escalier en U, avec palier intermédiaire)</li>';
        }
    ...
    // À partir d'ici, on a au moins une solution conforme
    // Utiliser les solutions conformes uniquement
    const best = compliantSolutions[0];
    
    let html = '';
    
    // Titre et statut - toujours conforme Ã  ce point
    html += `<h3>âœ” Solution optimale (${codeRef})</h3>`;
    ...
    if (best.useLandingConfiguration) {
        ...
        html += `<li><strong>Volée 1 :</strong> ${best.risersInFlight1} contremarches, ${best.treadsInFlight1} girons</li>`;
        html += `<li><strong>Volée 2 :</strong> ${best.risersInFlight2} contremarches, ${best.treadsInFlight2} girons</li>`;
    }
    
    html += `<li><strong>Largeur :</strong> ${formatValue(stairWidthValue, isMetric, 0)} âœ”</li>`;
    html += '</ul></div>';
    ...
    if (isMetric) {
        ...
        html += riseError < 0.01 ? 'âœ” Exact' : `âš Â  Â Écart ${riseError.toFixed(4)} mm`;
        ...
        if (best.useLandingConfiguration) {
            ...
            html += treadError < 0.01 ? 'âœ” Exact' : `âš Â  Â Écart ${treadError.toFixed(4)} mm`;
        }
    } else {
        ...
        if (best.useLandingConfiguration) {
            html += '<br><br><strong>Dimensions réelles par volée :</strong><br>';
            const treadIn = best.treadDepth / 25.4;
            const landingIn = best.landingDepth / 25.4;
            const flight1RealIn = best.flight1RealLength / 25.4;
            const flight2RealIn = best.flight2RealLength / 25.4;
            const flight1EnteredIn = best.firstFlightRun / 25.4;
            const flight2EnteredIn = best.secondFlightRun / 25.4;
            
            html += `Volée 1 : ${best.treadsInFlight1} Ã— ${treadIn.toFixed(6)}" + ${landingIn.toFixed(6)}" = ${flight1RealIn.toFixed(6)}" `;
            const diff1 = flight1RealIn - flight1EnteredIn;
            if (Math.abs(diff1) < 0.001) {
                html += 'âœ”';
            } else {
                html += diff1 > 0 ? `(+${diff1.toFixed(3)}" vs entrée)` : `(${diff1.toFixed(3)}" vs entrée)`;
            }
            
            html += '<br>';
            html += `Volée 2 : ${best.treadsInFlight2} Ã— ${treadIn.toFixed(6)}" + ${landingIn.toFixed(6)}" = ${flight2RealIn.toFixed(6)}" `;
            const diff2 = flight2RealIn - flight2EnteredIn;
            if (Math.abs(diff2) < 0.001) {
                html += 'âœ”';
            } else {
                html += diff2 > 0 ? `(+${diff2.toFixed(3)}" vs entrée)` : `(${diff2.toFixed(3)}" vs entrée)`;
            }
            
            // Distance totale dépliée (palier compté une seule fois)
            html += '<br><br><strong>Distance totale (tracé en coupe déplié) :</strong><br>';
            const totalUnfoldedIn = (best.numTreads * treadIn) + landingIn;
            const totalUnfoldedFeet = Math.floor(totalUnfoldedIn / 12);
            const totalUnfoldedRemIn = totalUnfoldedIn % 12;
            html += `${best.numTreads} girons + 1 palier = ${best.numTreads} Ã— ${treadIn.toFixed(6)}" + ${landingIn.toFixed(6)}" = ${totalUnfoldedIn.toFixed(6)}" `;
            html += `(â‰ˆ ${totalUnfoldedFeet}'-${totalUnfoldedRemIn.toFixed(2)}")`;
            html += '<br><em style="font-size: 0.9em; color: #666;">Note : Le palier est compté une seule fois dans la distance totale dépliée.</em>';
        }
    }
    ...
    // Règle du pas
    html += '<div class="result-section">';
    html += `<h4>${best.stepRule.isValid ? 'âœ”' : 'âš Â  '} Règle du pas (${best.stepRule.validCount} sur 3)</h4>`;
    html += '<ul>';
    ...
    if (best.useLandingConfiguration) {
        html += `<li>Volée 1 : ${best.treadsInFlight1} girons + palier (voir dimensions réelles ci-dessus)</li>`;
        html += `<li>Volée 2 : palier + ${best.treadsInFlight2} girons (voir dimensions réelles ci-dessus)</li>`;
    } else {
    ...
    if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
        ...
        html += `<li>Volée 1 : du plancher inférieur vers le palier (${best.risersInFlight1} CM, ${best.treadsInFlight1} girons)</li>`;
        html += `<li>Volée 2 : du palier vers le plancher supérieur (${best.risersInFlight2} CM, ${best.treadsInFlight2} girons)</li>`;
        html += '<li>Le giron uniforme respecte CCQ 9.8.4.4(3)</li>';
        html += '</ul></div>';
    }
    
    if (best.isSpiral) {
        html += '<div class="result-section">';
        html += '<h4>ðŸ“‹ Notes - Escalier hélicoïdal</h4>';
        html += '<ul>';
    ...
    // Tableau des alternatives (seulement les solutions conformes)
    if (compliantSolutions.length > 1) {
        html += '<div class="result-section">';
        html += '<h4>Alternatives conformes</h4>';
        html += '<table class="result-table">';
        html += '<thead><tr>';
        html += '<th>#</th><th>CM</th><th>Hauteur CM</th><th>Girons</th><th>Profondeur</th><th>Règle</th>';
        html += '</tr></thead><tbody>';
        
        compliantSolutions.forEach((sol, i) => {
...
function displayVerificationResults(params) {
    ...
    if (isCompliant) {
        ...
        html += `<h3>âš Â  Non conforme au ${codeReference}</h3>`;
        html += '<div class="result-section">';
    ...
    // Règle du pas
    html += '<div class="result-section">';
    html += `<h4>${stepRule.isValid ? 'âœ”' : 'âš Â  '} Règle du pas (${stepRule.validCount} sur 3)</h4>`;
    html += '<ul>';
    html += `<li>${stepRule.rule1.isValid ? 'âœ”' : 'âœ—'} Règle 1 (G+H) : ${stepRule.rule1.value.toFixed(2)}" (17"-18")</li>`;
    html += `<li>${stepRule.rule2.isValid ? 'âœ”' : 'âœ—'} Règle 2 (GÃ—H) : ${stepRule.rule2.value.toFixed(2)} po² (71-74)</li>`;
