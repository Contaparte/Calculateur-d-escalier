...
function imperialToMetric(imperialValue) {
    ...
    // Format: X' Y Z/W" (ex: 10' 2 1/2")
    let match = imperialValue.match(/^(\d+(?:\.\d+)?)'?\s*(\d+(?:\.\d+)?)\s+(\d+)\/(\d+)(?:"|in)?$/);
    if (match) {
    ...
    // Format: X' Z/W (ex: 3' 1/2)
    match = imperialValue.match(/^(\d+(?:\.\d+)?)'?\s*(\d+)\/(\d+)$/);
    if (match) {
        const feet = parseFloat(match[1]) || 0;
        const num = parseFloat(match[2]);
        const den = parseFloat(match[3]);
