...
function generateStraightStairVisualization(stairData) {
    ...
    // Conversion en mm pour calculs
    const rise = isMetric ? totalRise : totalRise * 25.4;
    const run = isMetric ? totalRun : totalRun * 25.4;
    const riser = isMetric ? riserHeight : riserHeight * 25.4;
    ...
    // Calcul de l'échelle pour que tout rentre
    const availableWidth = svgWidth - padding * 2;
    const availableHeight = svgHeight - padding * 2;
    const scaleX = availableWidth / run;
    const scaleY = availableHeight / rise;
    ...
    let svg = `<svg width="${svgWidth}" height="${svgHeight}" xmlns="http://www.w3.org/2000/svg" style="background: white; border: 1px solid #ccc; border-radius: 4px;">`;
    
    // Style
    svg += `<defs>
        <style>
            .step-line { stroke: #333; stroke-width: 2; fill: none; }
            .dimension-line { stroke: #666; stroke-width: 1; stroke-dasharray: 5,5; }
            .dimension-text { font-family: Arial, sans-serif; font-size: 13px; fill: #333; font-weight: bold; }
            .label-text { font-family: Arial, sans-serif; font-size: 12px; fill: #666; }
            .title-text { font-family: Arial, sans-serif; font-size: 15px; fill: #000; font-weight: bold; }
        </style>
    </defs>`;
    
    // Titre
    svg += `<text x="20" y="25" class="title-text">Vue de profil - Volée droite</text>`;
    ...
    for (let i = 0; i < numRisers; i++) {
        ...
        svg += ` L ${currentX} ${currentY}`;
        
        // Si ce n'est pas la dernière contremarche, avancer (giron)
        if (i < numTreads) {
    ...
    // Ligne de base (sol)
    svg += `<line x1="${startX - 20}" y1="${startY}" x2="${startX + run * scale + 20}" y2="${startY}" stroke="#999" stroke-width="1" stroke-dasharray="3,3"/>`;
    
    // Ligne de plafond/arrivée
    const endX = startX + run * scale;
    const endY = startY - rise * scale;
    svg += `<line x1="${endX - 20}" y1="${endY}" x2="${endX + 40}" y2="${endY}" stroke="#999" stroke-width="1" stroke-dasharray="3,3"/>`;
    
    // Dimension horizontale (longueur totale)
    const dimY = startY + 30;
    svg += `<line x1="${startX}" y1="${dimY}" x2="${endX}" y2="${dimY}" class="dimension-line"/>`;
    svg += `<line x1="${startX}" y1="${dimY - 5}" x2="${startX}" y2="${dimY + 5}" class="dimension-line"/>`;
    svg += `<line x1="${endX}" y1="${dimY - 5}" x2="${endX}" y2="${dimY + 5}" class="dimension-line"/>`;
    const runText = formatValuePrecise(totalRun, isMetric);
    svg += `<text x="${(startX + endX) / 2}" y="${dimY + 18}" class="dimension-text" text-anchor="middle">Longueur: ${runText}</text>`;
    
    // Dimension verticale (hauteur totale)
    const dimX = endX + 25;
    svg += `<line x1="${dimX}" y1="${startY}" x2="${dimX}" y2="${endY}" class="dimension-line"/>`;
    svg += `<line x1="${dimX - 5}" y1="${startY}" x2="${dimX + 5}" y2="${startY}" class="dimension-line"/>`;
    svg += `<line x1="${dimX - 5}" y1="${endY}" x2="${dimX + 5}" y2="${endY}" class="dimension-line"/>`;
    const riseText = formatValuePrecise(totalRise, isMetric);
    svg += `<text x="${dimX + 15}" y="${(startY + endY) / 2 + 5}" class="dimension-text" text-anchor="start">Hauteur: ${riseText}</text>`;
    
    // Annotations sur une marche (au milieu)
    const midStep = Math.floor(numTreads / 2);
    const annotX = startX + midStep * tread * scale;
    const annotY = startY - midStep * riser * scale;
    
    // Flèche pour le giron
    svg += `<line x1="${annotX}" y1="${annotY + 15}" x2="${annotX + tread * scale}" y2="${annotY + 15}" stroke="#0066cc" stroke-width="1.5"/>`;
    svg += `<polygon points="${annotX},${annotY + 15} ${annotX + 5},${annotY + 12} ${annotX + 5},${annotY + 18}" fill="#0066cc"/>`;
    svg += `<polygon points="${annotX + tread * scale},${annotY + 15} ${annotX + tread * scale - 5},${annotY + 12} ${annotX + tread * scale - 5},${annotY + 18}" fill="#0066cc"/>`;
    const treadText = formatValuePrecise(treadDepth, isMetric);
    svg += `<text x="${annotX + tread * scale / 2}" y="${annotY + 12}" class="label-text" text-anchor="middle">G: ${treadText}</text>`;
    
    // Flèche pour la contremarche
    svg += `<line x1="${annotX - 15}" y1="${annotY}" x2="${annotX - 15}" y2="${annotY - riser * scale}" stroke="#cc0000" stroke-width="1.5"/>`;
    svg += `<polygon points="${annotX - 15},${annotY} ${annotX - 12},${annotY - 5} ${annotX - 18},${annotY - 5}" fill="#cc0000"/>`;
    svg += `<polygon points="${annotX - 15},${annotY - riser * scale} ${annotX - 12},${annotY - riser * scale + 5} ${annotX - 18},${annotY - riser * scale + 5}" fill="#cc0000"/>`;
    const riserText = formatValuePrecise(riserHeight, isMetric);
    svg += `<text x="${annotX - 35}" y="${annotY - riser * scale / 2 + 4}" class="label-text" text-anchor="end">CM: ${riserText}</text>`;
    
    // Informations en bas à gauche
    svg += `<g transform="translate(20, ${svgHeight - 60})">
        <text x="0" y="0" class="label-text">Contremarches: ${numRisers} × ${riserText}</text>
        <text x="0" y="16" class="label-text">Girons: ${numTreads} × ${treadText}</text>
        <text x="0" y="32" class="label-text">CM = Contremarche, G = Giron</text>
    </g>`;
