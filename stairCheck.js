...
function checkStepRule(riserMm, treadMm) {
    ...
    // Règle 2: G Ã— H = 71 à 74 poÂ² (458-477 cmÂ²)
...
function displayCalculatorResults(solutions, params) {
    ...
    // Règle du pas
    html += '<div class="result-section">';
    html += `<h4>${best.stepRule.isValid ? 'âœ“' : 'âš  '} Règle du pas (${best.stepRule.validCount} sur 3)</h4>`;
    html += '<ul>';
...
function displayVerificationResults(params) {
    ...
    // Règle du pas
    html += '<div class="result-section">';
    html += `<h4>${stepRule.isValid ? 'âœ“' : 'âš  '} Règle du pas (${stepRule.validCount} sur 3)</h4>`;
    html += '<ul>';
