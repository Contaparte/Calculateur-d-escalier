 ...
 * CORRIGÉ: Les marches dansantes sont positionnées ENTRE les marches rectangulaires, pas superposées
 */
function generateDancingStepsVisualization(stairData) {
 ...
 // CORRECTION: Calculer les longueurs réelles basées sur les marches rectangulaires
 // Longueur zone rect 1 = nombre de marches rect × profondeur giron rect
 ...
 // CORRECTION: Calcul des dimensions totales basées sur les vraies longueurs
 let totalWidth, totalHeight;
 if (isUConfig) {
  // Pour U: largeur = longueur horizontale (flight2), hauteur = somme des verticales + zones virage
  totalWidth = rect2Length + actualWidth * 2;
  totalHeight = Math.max(rect1Length, rect3Length) + actualWidth * 2;
 } else {
  // Pour L: tenir compte de l'angle personnalisé
 ...
 // Dimensions à l'échelle - CORRECTION: utiliser les vraies longueurs de marches rect
 ...
 if (isUConfig) {
  ...
  // Structure: marches rect 1 → virage 1 → marches rect 2 → virage 2 → marches rect 3
  
  // Volée 1 (verticale, descend depuis le haut) - CORRIGÉ: longueur = rect1_plan seulement
  ...
  // Zone de virage 1 - CORRIGÉ: positionné juste après les marches rect 1
  const turn1CenterX = planStartX + stairWidth_plan;
  const turn1CenterY = planStartY + rect1_plan; // Juste après les marches rect
  ...
  // Dessiner secteur 1 (virage de 90° vers la droite)
  const angle1Start = 180; // Vers la gauche
  const angle1End = 270; // Vers le bas
  ...
  pathD1 += ' A ' + r(effectiveOuterR) + ' ' + r(effectiveOuterR) + ' 0 0 1 ' + r(turn1CenterX + effectiveOuterR * Math.cos(angle1End * Math.PI / 180)) + ' ' + r(turn1CenterY + effectiveOuterR * Math.sin(angle1End * Math.PI / 180));
  ...
  // Lignes radiales pour zone 1
  for (let i = 0; i <= dancingStepsPerZone; i++) {
   const angle = (angle1Start + i * (90 / dancingStepsPerZone)) * Math.PI / 180;
  ...
  svg += '<text x="' + r(turn1CenterX - stairWidth_plan * 0.35) + '" y="' + r(turn1CenterY + stairWidth_plan * 0.65) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">' + dancingStepsPerZone + '×' + Math.round(angleStep) + '°</text>';
  ...
  // Zone de virage 2 - CORRIGÉ: positionnée après les marches rect 2
  const turn2CenterX = planStartX + stairWidth_plan + rect2_plan;
  const turn2CenterY = planStartY + rect1_plan + stairWidth_plan;
  
  // Dessiner secteur 2 (virage de 90° vers le haut)
  const angle2Start = 270; // Vers le bas
  const angle2End = 360; // Vers la droite
  ...
  pathD2 += ' A ' + r(effectiveOuterR) + ' ' + r(effectiveOuterR) + ' 0 0 1 ' + r(turn2CenterX + effectiveOuterR * Math.cos(angle2End * Math.PI / 180)) + ' ' + r(turn2CenterY + effectiveOuterR * Math.sin(angle2End * Math.PI / 180));
  ...
  pathD2 += ' A ' + r(effectiveInnerR) + ' ' + r(effectiveInnerR) + ' 0 0 0 ' + r(turn2CenterX + effectiveInnerR * Math.cos(angle2Start * Math.PI / 180)) + ' ' + r(turn2CenterY + effectiveInnerR * Math.sin(angle2Start * Math.PI / 180));
  pathD2 += ' Z';
  svg += '<path d="' + pathD2 + '" fill="#ffe0b2" stroke="#ff9800" stroke-width="2"/>';
  
  // Lignes radiales pour zone 2
  for (let i = 0; i <= dancingStepsPerZone; i++) {
   const angle = (angle2Start + i * (90 / dancingStepsPerZone)) * Math.PI / 180;
  ...
  svg += '<text x="' + r(turn2CenterX + stairWidth_plan * 0.65) + '" y="' + r(turn2CenterY + stairWidth_plan * 0.35) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">' + dancingStepsPerZone + '×' + Math.round(angleStep) + '°</text>';
  ...
  // CORRIGÉ: Positionnement séquentiel correct
  // Structure: marches rect 1 → zone de virage → marches rect 2
  
  // Volée 1 (horizontale, de gauche à droite) - CORRIGÉ: longueur = rect1_plan
  const v1X = planStartX;
  const v1Y = planStartY + totalHeight * scale - stairWidth_plan; // En bas
  const v1Width = rect1_plan; // Vraie longueur des marches rect 1
  ...
  // Zone de virage - CORRIGÉ: centre positionné juste après les marches rect 1
  const turnCenterX = v1X + rect1_plan; // Juste après les marches rect
  const turnCenterY = v1Y; // Au niveau supérieur de la volée 1
  
  // Rayons effectifs pour le secteur d'anneau
  const effectiveInnerR = Math.min(stairWidth_plan * 0.15, 10);
  const effectiveOuterR = stairWidth_plan;
  
  // Angles du secteur: de 180° (vers la gauche) à (180° - dancingAngle) (vers le haut selon l'angle)
  const angleStart = 180; // Départ vers la gauche (fin de volée 1)
  const angleEnd = 180 - dancingAngle; // Arrivée selon l'angle de rotation
  const angleStep = dancingAngle / dancingStepsPerZone;
  
  // Path du secteur d'anneau
  ...
  const startInnerX = turnCenterX + effectiveInnerR * Math.cos(angleEnd * Math.PI / 180);
  const startInnerY = turnCenterY + effectiveInnerR * Math.sin(angleEnd * Math.PI / 180);
  const endInnerX = turnCenterX + effectiveInnerR * Math.cos(angleStart * Math.PI / 180);
  const endInnerY = turnCenterY + effectiveInnerR * Math.sin(angleStart * Math.PI / 180);
  
  const largeArcFlag = dancingAngle > 180 ? 1 : 0;
  
  // Path du secteur d'anneau
  svg += '<path d="M ' + r(startOuterX) + ' ' + r(startOuterY);
  svg += ' A ' + r(effectiveOuterR) + ' ' + r(effectiveOuterR) + ' 0 ' + largeArcFlag + ' 0 ' + r(endOuterX) + ' ' + r(endOuterY);
  svg += ' L ' + r(startInnerX) + ' ' + r(startInnerY);
  svg += ' A ' + r(effectiveInnerR) + ' ' + r(effectiveInnerR) + ' 0 ' + largeArcFlag + ' 1 ' + r(endInnerX) + ' ' + r(endInnerY);
  ...
  svg += '<path d="M ' + r(endInnerX) + ' ' + r(endInnerY) + ' A ' + r(effectiveInnerR) + ' ' + r(effectiveInnerR) + ' 0 ' + largeArcFlag + ' 0 ' + r(startInnerX) + ' ' + r(startInnerY) + '" fill="none" stroke="#e65100" stroke-width="2"/>';
  ...
  // Volée 2 - CORRIGÉ: part depuis la fin du secteur selon l'angle de rotation
  // Direction de la volée 2: perpendiculaire à la fin du secteur
  const flight2DirAngle = angleEnd - 90; // Direction de la volée 2 (perpendiculaire à la fin du secteur)
  ...
  // Point de fin de la volée 2 - CORRIGÉ: utiliser rect2_plan
  ...
  // Flèche: part du début de la volée 1, tourne au centre, va vers la fin de la volée 2
  ...
  // Cotations - Longueur partie 1
  const dimY1 = r(v1Y + stairWidth_plan + 25);
  ...
  svg += '<text x="' + r(v1X + rect1_plan/2) + '" y="' + r(dimY1 + 14) + '" style="font:10px Arial;fill:#333;" text-anchor="middle">' + formatValueForPlan(firstFlightRun, isMetric) + '</text>';
  
  // Cotation verticale (longueur volée 2)
  const dimX2 = r(Math.max(turnCenterX + stairWidth_plan, v2EndX + stairWidth_plan * Math.cos(perpAngle)) + 20);
  svg += '<line x1="' + dimX2 + '" y1="' + r(v2EndY) + '" x2="' + dimX2 + '" y2="' + r(v1Y) + '" stroke="#555" stroke-width="1" marker-start="url(#arrDS)" marker-end="url(#arrDE)"/>';
  svg += '<text x="' + r(dimX2 + 5) + '" y="' + r((v2EndY + v1Y)/2 + 4) + '" style="font:10px Arial;fill:#333;">' + formatValueForPlan(secondFlightRun, isMetric) + '</text>';
 }
