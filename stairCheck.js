...
const CNB_LIMITS = {
 ...
 // =====================================================================
 // ESCALIERS HÃ‰LICOàÂDAUX (CNB 2020, Article 9.8.4.7)
 // =====================================================================
 spiral: {
...
function checkStepRule(riserMm, treadMm) {
 ...
 // Règle 2: G Ã— H = 71 à  74 poÃ‚Â² (458-477 cmÃ‚Â²)
 ...
 return {
  ...
  rule2: { value: rule2Value, isValid: rule2Valid, range: '71-74 poÃ‚Â²' },
...
function calculateStraightStair(params) {
 ...
 for (let numRisers = minRisers; numRisers <= maxRisers; numRisers++) {
  ...
  // Ã‰valuer la qualité de la solution
  ...
  // Ã‰conomie d'espace: préférer plus de contremarches (escalier plus raide)
 ...
 * - Ã€ l'intersection des deux volées (cas standard)
 * - Ã€ l'extrémité de la volée (quand firstFlightRun ou secondFlightRun = stairWidth)
 */
function calculateLShapedWithRadiating(params) {
 ...
 if (radiatingAtEnd || radiatingAtStart) {
  // CAS SPÃ‰CIAL: Marches rayonnantes à  l'extrémité
...
function calculateUShapedStair(params) {
 ...
 switch (uShapedConfig) {
  ...
  // Configuration avec palier uniquement
  // 1ère partie = girons + profondeur palier
  // 2ème partie = largeur totale du palier
  // 3ème partie = girons + profondeur palier
  numLandings = 1;
  availableForTreads = (flight1Run - landingLength) + ((flight3Run || flight1Run) - landingLength);
  break;
  case 'rect_landing_radiating_rect':
  // Configuration palier + marches rayonnantes
  // 1ère partie = 1er segment vertical du U (girons + landingDepth)
  // 2ème partie = segment horizontal du U
  // 3ème partie = dernier segment vertical du U (girons + landingDepth)
  numLandings = 1;
  availableForTreads = (flight1Run - landingLength) + ((flight3Run || flight1Run) - landingLength);
  break;
  ...
  // Configuration marches rayonnantes + palier + marches rayonnantes
  // 1ère partie = 1er segment vertical du U (girons + landingDepth)
  // 2ème partie = segment horizontal du U
  // 3ème partie = dernier segment vertical du U (girons + landingDepth)
  numLandings = 1;
 ...
 let minRequiredWidth = 2 * stairWidth; // Minimum: 2 Ã— largeur escalier (volées côte à côte)
 let flight2RunError = null;
 
 if (uShapedConfig === 'rect_landing_rect' || uShapedConfig === 'two_landings') {
  // Palier seul: largeur min = 2 Ã— stairWidth
  minRequiredWidth = 2 * stairWidth;
 } else if (uShapedConfig === 'rect_landing_radiating_rect' || uShapedConfig === 'rect_radiating_landing_rect') {
  // 1 coin rayonnant + palier: largeur min = 2 Ã— stairWidth (coin = stairWidth, palier se partage l'espace)
  minRequiredWidth = 2 * stairWidth;
 } else if (uShapedConfig === 'rect_radiating_landing_radiating_rect') {
  // 2 coins rayonnants + palier: largeur min = 2 Ã— stairWidth + landingLength (profondeur palier minimum)
 ...
 // Configuration avec DEUX séries de marches rayonnantes (2 Ã— 90°)
 else if (uShapedConfig === 'rect_radiating_landing_radiating_rect') {
...
function generateLShapedStairVisualization(stairData) {
 ...
 // Ã‰chelle pour occuper l'espace disponible
 ...
 if (isLanding) {
  ...
  // Numéro du palier dans la séquence continue - décalé vers le coin sup. gauche pour éviter conflit avec la flèche
  svg += '<text x="' + r(palierX + landingW_plan * 0.20) + '" y="' + r(palierY + landingW_plan * 0.25 + 4) + '" style="font:bold 9px Arial;fill:#f57f17;" text-anchor="middle">' + (flight1Treads + 1) + '</text>';
  // Mot "Palier" décalé vers le bas pour éviter conflit avec la flèche
  svg += '<text x="' + r(palierX + landingW_plan/2) + '" y="' + r(palierY + landingW_plan * 0.85 + 4) + '" style="font:italic 10px Arial;fill:#f57f17;" text-anchor="middle">Palier</text>';
  ...
  // NUMÃ‰ROTATION CONTINUE: Volée 1 (1 à  flight1Treads) + Palier (flight1Treads+1) + Volée 2 (flight1Treads+2 à  numTreads+1)
...
function generateUShapedStairVisualization(stairData) {
 ...
 const scalePlan = Math.min(planW / totalPlanWidth, planH / maxFlightRun) * 0.70;
 ...
 // IMPORTANT: Les dimensions des volées sont calculées à  partir du nombre de girons Ã— profondeur uniforme
 ...
 // Numéro du palier - décalé vers le coin sup. gauche pour éviter conflit avec la flèche horizontale
 svg += '<text x="' + r(landingX + landingW * 0.08) + '" y="' + r(landingY + landingH * 0.35 + 4) + '" style="font:bold 9px Arial;fill:#f57f17;" text-anchor="middle">' + (flight1Treads + 1) + '</text>';
 // Mot "Palier" décalé vers le haut pour éviter conflit avec la flèche qui traverse au centre
 svg += '<text x="' + r(landingX + landingW/2) + '" y="' + r(landingY + landingH * 0.25 + 4) + '" style="font:italic 11px Arial;fill:#f57f17;" text-anchor="middle">Palier</text>';
 
 // ===== VOLÃ‰E 2 (droite, descend de haut vers bas = sens opposé à  volée 1) =====
...
function generateUShapedRadiatingVisualization(stairData) {
 ...
 // Longueurs des parties basées sur les valeurs RÃ‰ELLES entrées par l'utilisateur
 ...
 // Donc TOUS les girons rectangulaires ont la MÃŠME profondeur (treadDepth calculé globalement)
 ...
 * Conforme au CNB 9.8.4.6: chaque série de marches rayonnantes â‰¤ 90°
 */
function generateUShapedMixedVisualization(stairData) {
 ...
 // Espace entre volées = largeur totale - (2 Ã— largeur escalier)
 ...
 const maxFlightRun = Math.max(actualFlight1Run, actualFlight3Run);
 const scalePlan = Math.min(planW / totalPlanWidth, planH / maxFlightRun) * 0.60;
...
function generateUShapedDoubleRadiatingVisualization(stairData) {
    ...
    const totalPlanWidth = actualFlight2Run;
    const maxFlightRun = Math.max(actualFlight1Run, actualFlight3Run);
    const scalePlan = Math.min(planW / totalPlanWidth, planH / maxFlightRun) * 0.55;
    ...
    // ===== PALIER CENTRAL =====
    // Le palier occupe l'espace entre les deux coins: flight2Run - (2 Ã— stairWidth)
    ...
    // ===== VOLÃ‰E 2 (droite, descend de haut vers bas) =====
    const flight2X = corner2X;
    ...
    // ===== FLÃˆCHE DE DIRECTION (virage à  90°) =====
    const arrowX1 = r(flight1X + stairWidth_plan / 2);
    const arrowX2 = r(flight2X + stairWidth_plan / 2);
...
function displayCalculatorResults(solutions, params) {
 ...
 if (solutions && solutions.error && solutions.flight2RunError) {
  ...
  let html = '<h3>âœ— Paramètre incohérent</h3>';
 ...
 if (!solutions || solutions.length === 0) {
  ...
  if (stairConfigValue === 'l_shaped' && (lShapedConfigValue === 'two_45deg' || lShapedConfigValue === 'three_30deg')) {
   html += '<li>Note: Un escalier en L avec marches rayonnantes est considéré comme UNE SEULE VOLÃ‰E selon le CNB</li>';
  }
 ...
 if (best.totalRadiatingSteps > 0) {
  ...
  if (best.numRadiatingSeries === 1) {
   // 1 série de 90°: "2Ã—45°" ou "3Ã—30°"
   radiatingDesc = best.radiatingAngle === 45 ? '2Ã—45°' : '3Ã—30°';
  } else {
   // 2 séries de 90°: "2Ã—2Ã—45°" ou "2Ã—3Ã—30°"
   radiatingDesc = best.radiatingAngle === 45 ? '2Ã—2Ã—45°' : '2Ã—3Ã—30°';
  }
 ...
 html += `<li>Nombre de girons tracés : ${best.numTreads} (le dernier giron n'est pas comptabilisé car il corespond au plancher supérieur)</li>`;
...
function displayVerificationResults(params) {
 ...
 html += `<li>${stepRule.rule2.isValid ? 'âœ“' : 'âœ—'} Règle 2 (GÃ—H) : ${stepRule.rule2.value.toFixed(2)} poÃ‚Â² (71-74)</li>`;
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 function performVerification() {
  ...
  // Ã‰chappée (sauf hélicoïdal qui a ses propres règles)
  if (stairConfigValue !== 'spiral' && headroomValue < limits.minHeadroom) {
  issues.push(`Ã‰chappée ${headroomValue.toFixed(0)} mm < minimum ${limits.minHeadroom} mm`);
  isCompliant = false;
  }
  ...
  // Ã‰chappée spécifique pour hélicoïdal: 1980 mm (CNB 9.8.4.7.(1)e)
  if (headroomValue > 0 && headroomValue < CNB_LIMITS.spiral.minHeadroom) {
   issues.push(`Ã‰chappée ${headroomValue.toFixed(0)} mm < minimum ${CNB_LIMITS.spiral.minHeadroom} mm pour hélicoïdal (CNB 9.8.4.7.(1)e)`);
   isCompliant = false;
  }
  ...
  * Règles:
  * - Largeur palier â‰¥ 2 Ã— largeur escalier (géométrie)
  ...
  * - Giron à  300 mm â‰¥ 190 mm (CNB 9.8.4.7)
  */
 ...
 // =====================================================================
 // VALIDATION CNB EN TEMPS RÃ‰EL - PANNEAU D'AVERTISSEMENTS
 // =====================================================================
 
 /**
  * Valide les paramètres en temps réel et affiche les avertissements CNB
  */
 function validateCNBParameters() {
  ...
  // 1. Largeur minimale de l'escalier (CNB 9.8.2.1 / 3.4.3.2)
  if (stairWidth > 0 && stairWidth < limits.minWidth) {
  const article = buildingType === 'part3' ? '3.4.3.2' : '9.8.2.1';
  if (isMetric) {
   warnings.push({
   type: 'error',
   text: `Largeur ${stairWidth.toFixed(0)} mm < minimum ${limits.minWidth} mm (CNB ${article})`
   });
  } else {
   warnings.push({
   type: 'error',
   text: `Largeur ${metricToImperial(stairWidth)} < minimum ${metricToImperial(limits.minWidth)} (CNB ${article})`
   });
  }
  }
  
  // 2. Hauteur maximale par volée (CNB 9.8.3.3 / 3.4.6.3)
  const maxFlightHeight = limits.maxRise || 3700;
  const needsLanding = stairConfig === 'straight' && totalRise > maxFlightHeight;
  
  if (totalRise > 0 && needsLanding) {
  const article = buildingType === 'part3' ? '3.4.6.3.(1)' : '9.8.3.3.(1)';
  if (isMetric) {
   warnings.push({
   type: 'error',
   text: `Hauteur ${totalRise.toFixed(0)} mm > max ${maxFlightHeight} mm par volée â†' palier requis (CNB ${article})`
   });
  } else {
   warnings.push({
   type: 'error',
   text: `Hauteur ${metricToImperial(totalRise)} > max ${metricToImperial(maxFlightHeight)} par volée â†' palier requis (CNB ${article})`
   });
  }
  }
  
  // 3. Vérifications spécifiques aux escaliers en U
  if (stairConfig === 'u_shaped') {
  // Profondeur palier â‰¥ largeur escalier (CNB 9.8.6.3.(1))
  if (landingDepth > 0 && stairWidth > 0 && landingDepth < stairWidth) {
   if (isMetric) {
   warnings.push({
    type: 'error',
    text: `Profondeur palier ${landingDepth.toFixed(0)} mm < largeur escalier ${stairWidth.toFixed(0)} mm (CNB 9.8.6.3.(1))`
   });
   } else {
   warnings.push({
    type: 'error',
    text: `Profondeur palier ${metricToImperial(landingDepth)} < largeur escalier ${metricToImperial(stairWidth)} (CNB 9.8.6.3.(1))`
   });
   }
  }
  
  // Largeur palier â‰¥ 2 Ã— largeur escalier (géométrie U)
  if (landingWidth > 0 && stairWidth > 0 && landingWidth < (2 * stairWidth)) {
   const minRequired = 2 * stairWidth;
   if (isMetric) {
   warnings.push({
    type: 'error',
    text: `Largeur palier ${landingWidth.toFixed(0)} mm < 2 Ã— largeur escalier (${minRequired.toFixed(0)} mm) pour config. en U`
   });
   } else {
   warnings.push({
    type: 'error',
    text: `Largeur palier ${metricToImperial(landingWidth)} < 2 Ã— largeur escalier (${metricToImperial(minRequired)}) pour config. en U`
   });
   }
  }
  }
  
  // 4. Marches rayonnantes - restrictions
  if (stairConfig === 'l_shaped' && (lShapedConfig === 'two_45deg' || lShapedConfig === 'three_30deg')) {
  if (buildingType === 'part3') {
   warnings.push({
   type: 'error',
   text: 'Marches rayonnantes non permises pour Partie 3 (CNB A-9.8.3.1 Tableau)'
   });
  }
  if (stairType === 'common') {
   warnings.push({
   type: 'error',
   text: 'Marches rayonnantes non permises pour escaliers communs (CNB A-9.8.3.1 Tableau)'
   });
  }
  }
  
  // 5. Marches rayonnantes dans escalier en U - restrictions
  ...
  // 6. Information sur les tolérances (CNB 9.8.4.4)
  if (stairWidth > 0 || totalRise > 0) {
  // Information sur les tolérances de construction
  // (affichée seulement si d'autres warnings sont présents)
  }
  
  // ===== AFFICHAGE DES AVERTISSEMENTS =====
  if (warnings.length > 0) {
  warningsList.innerHTML = warnings.map(w => 
   `<li class="warning-${w.type}">${w.text}</li>`
  ).join('');
 ...
 // Attacher la validation CNB à tous les champs pertinents
 const cnbFieldsToValidate = [
  'stairDesiredWidth', 'stairDesiredWidthImperial',
  'totalRise', 'totalRiseImperial',
  'uLandingWidth', 'uLandingWidthImperial',
  'uLandingDepth', 'uLandingDepthImperial',
 ...
 cnbFieldsToValidate.forEach(fieldId => {
  const field = document.getElementById(fieldId);
  if (field) {
 ...
 // Valider aussi quand les sélecteurs changent
 const cnbConfigSelects = [
  'calcStairConfig', 'calcLShapedConfig', 'calcUShapedConfig',
  'calcBuildingType', 'calcStairType', 'calcMeasurementSystem'
 ];
 
 cnbConfigSelects.forEach(selectId => {
  const select = document.getElementById(selectId);
  if (select) {
