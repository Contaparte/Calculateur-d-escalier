...
function generateStraightStairVisualization(stairData) {
    ...
    // Numerotation des marches SUR les marches (au centre de chaque marche)
    const markerY = r(planStartY + stairH_plan/2 + 3);  // Centre vertical de la marche
...
function generateLShapedStairVisualization(stairData) {
    ...
    if (isLanding) {
        ...
        // Numerotation volee 1 - SUR les marches (au centre)
        const v1MarkerY = r(planStartY + stairW2_plan + stairWidth_plan/2 + 3);
        ...
        const v2MarkerX = r(palierX + stairWidth_plan/2);
        ...
        if (isRadiatingAtExtremity && radiatingAtEnd) {
            ...
            // Numerotation volee 1 - SUR les marches (au centre)
            const markerY_v1m = r(planStartY + stairW2_plan + stairWidth_plan/2 + 3);
            ...
            const markerX_v2m = r(cornerX + stairWidth_plan/2);
    ...
    if (isLanding) {
        ...
        svg += '<text x="' + (legendX + cellW * 4.5) + '" y="' + (legendY + 12) + '" style="font:bold 9px Arial;fill:#333;" text-anchor="middle">Ray.</text>';
    }
...
function generateUShapedStairVisualization(stairData) {
    ...
    if (flight1Treads > 0) {
        ...
        // Numérotation volée 1 (au centre de chaque marche)
        // Marche 1: en bas (Y = flight1Y + flight1Run - treadDepth/2)
        const tread1CenterY = r(flight1Y + flight1Run_plan - treadDepth_plan/2);
        svg += '<text x="' + r(flight1X + stairWidth_plan/2) + '" y="' + r(tread1CenterY + 3) + '" style="font:9px Arial;fill:#333;font-weight:bold;" text-anchor="middle">1</text>';
        // Dernière marche de V1: en haut (Y = flight1Y + treadDepth/2)
        const treadLastV1CenterY = r(flight1Y + treadDepth_plan/2);
        svg += '<text x="' + r(flight1X + stairWidth_plan/2) + '" y="' + r(treadLastV1CenterY + 3) + '" style="font:9px Arial;fill:#333;font-weight:bold;" text-anchor="middle">' + flight1Treads + '</text>';
    }
    ...
    if (flight2Treads > 0) {
        ...
        // Numérotation volée 2 (au centre de chaque marche)
        // Première marche de V2: en haut près du palier (Y = flight2Y + treadDepth/2)
        const treadFirstV2CenterY = r(flight2Y + treadDepth_plan/2);
        svg += '<text x="' + r(flight2X + stairWidth_plan/2) + '" y="' + r(treadFirstV2CenterY + 3) + '" style="font:9px Arial;fill:#333;font-weight:bold;" text-anchor="middle">' + (flight1Treads + 1) + '</text>';
        // Dernière marche de V2: en bas (Y = flight2Y + flight2Run - treadDepth/2)
        const treadLastV2CenterY = r(flight2Y + flight2Run_plan - treadDepth_plan/2);
        svg += '<text x="' + r(flight2X + stairWidth_plan/2) + '" y="' + r(treadLastV2CenterY + 3) + '" style="font:9px Arial;fill:#333;font-weight:bold;" text-anchor="middle">' + numTreads + '</text>';
    }
    ...
    const totalRightText = formatValueForPlan(totalRight, isMetric);
