...
function displayCalculatorResults(solutions, params) {
    ...
    if (!solutions || solutions.length === 0) {
        ...
        let html = '<h3>❌ Aucune solution conforme au CCQ</h3>';
        ...
        if (!isWidthOk) {
            if (isMetric) {
                html += `<li>Augmentez la largeur de l'escalier à au moins ${limits.minWidth} mm</li>`;
            } else {
                html += `<li>Augmentez la largeur de l'escalier à au moins ${metricToImperial(limits.minWidth)}</li>`;
            }
        }
    ...
    // Filtrer pour ne garder que les solutions conformes à la règle du pas
    ...
    if (!isWidthOk || compliantSolutions.length === 0) {
        ...
        if (compliantSolutions.length === 0 && solutions.length > 0) {
            ...
            if (isMetric) {
                html += `<li>G + H = ${rule1.toFixed(1)} mm (requis: 430-460 mm) ${rule1 >= 430 && rule1 <= 460 ? '✔' : '✗'}</li>`;
                html += `<li>G × H = ${(rule2/1000).toFixed(1)} (requis: 45 800-48 000 mm²) ${rule2 >= 45800 && rule2 <= 48000 ? '✔' : '✗'}</li>`;
                html += `<li>G + 2H = ${rule3.toFixed(1)} mm (requis: 560-635 mm) ${rule3 >= 560 && rule3 <= 635 ? '✔' : '✗'}</li>`;
            } else {
                const Gin = G / 25.4;
                const Hin = H / 25.4;
                html += `<li>G + H = ${(Gin + Hin).toFixed(2)}" (requis: 17"-18") ${(Gin + Hin) >= 17 && (Gin + Hin) <= 18 ? '✔' : '✗'}</li>`;
                html += `<li>G × H = ${(Gin * Hin).toFixed(2)} (requis: 71-74 po²) ${(Gin * Hin) >= 71 && (Gin * Hin) <= 74 ? '✔' : '✗'}</li>`;
                html += `<li>G + 2H = ${(Gin + 2*Hin).toFixed(2)}" (requis: 22"-25") ${(Gin + 2*Hin) >= 22 && (Gin + 2*Hin) <= 25 ? '✔' : '✗'}</li>`;
            }
            html += '</ul></li>';
            ...
            html += '<ul>';
            html += `<li>Contremarches : ${best.numRisers} × ${formatValuePrecise(best.riserHeight, isMetric)}</li>`;
            html += `<li>Girons : ${best.numTreads} × ${formatValuePrecise(best.treadDepth, isMetric)}</li>`;
        }
        ...
        if (compliantSolutions.length === 0) {
            ...
            html += '<li>Réduisez la hauteur totale à gravir si possible</li>';
            if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
    ...
    // Titre et statut - toujours conforme à ce point
    html += `<h3>✔ Solution optimale (${codeRef})</h3>`;
    ...
    html += `<li><strong>Largeur :</strong> ${formatValue(stairWidthValue, isMetric, 0)} ✔</li>`;
    html += '</ul></div>';
    ...
    if (isMetric) {
        ...
        if (best.useLandingConfiguration) {
            ...
            html += `Volée 1 : ${best.treadsInFlight1} × ${treadIn.toFixed(6)}" + ${landingIn.toFixed(6)}" = ${flight1RealIn.toFixed(6)}" `;
            ...
            html += '<br>';
            html += `Volée 2 : ${best.treadsInFlight2} × ${treadIn.toFixed(6)}" + ${landingIn.toFixed(6)}" = ${flight2RealIn.toFixed(6)}" `;
            ...
            html += `${best.numTreads} girons + 1 palier = ${best.numTreads} × ${treadIn.toFixed(6)}" + ${landingIn.toFixed(6)}" = ${totalUnfoldedIn.toFixed(6)}" `;
            html += `(≈ ${totalUnfoldedFeet}'-${totalUnfoldedRemIn.toFixed(2)}")`;
