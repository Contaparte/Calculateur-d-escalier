...
function generateUShapedRadiatingVisualization(stairData) {
    ...
    // Répartition égale entre les 2 volées rectangulaires
    const flight1RectTreads = Math.ceil(numRectTreads / 2);
    const flight2RectTreads = numRectTreads - flight1RectTreads;
    ...
    if (flight1RectTreads > 0) {
        ...
        // Lignes des girons volée 1 - épaisseur uniforme 1.5
        for (let i = 1; i <= flight1RectTreads; i++) {
            const lineY = r(flight1Y + flight1Run_plan - (i * treadDepth_plan));
            svg += '<line x1="' + flight1X + '" y1="' + lineY + '" x2="' + r(flight1X + stairWidth_plan) + '" y2="' + lineY + '" stroke="#1b5e20" stroke-width="1.5"/>';
        }
        
        // Numérotation volée 1 - DÉCALÉE vers l'extérieur (25% de la largeur, côté gauche)
        ...
        // Annotation nombre de girons rectangulaires volée 1 (côté gauche extérieur, rotation -90)
    ...
    // Virage de 90° - entrée par le bas (volée 1), sortie vers la droite
    ...
    // Lignes rayonnantes coin 1 - pivot en haut-gauche (coin intérieur)
    const pivot1X = corner1X;
    const pivot1Y = corner1Y;
    
    // Tracer les lignes de séparation des marches rayonnantes
    for (let i = 1; i < stepsPerCorner; i++) {
        const angle = (Math.PI / 2) * (i / stepsPerCorner); // De 0 à 90°
        const x2 = r(pivot1X + Math.cos(angle) * stairWidth_plan);
        const y2 = r(pivot1Y + Math.sin(angle) * stairWidth_plan);
        svg += '<line x1="' + pivot1X + '" y1="' + pivot1Y + '" x2="' + x2 + '" y2="' + y2 + '" stroke="#e65100" stroke-width="1.5"/>';
    }
    
    // Annotation coin 1
    const radStart1 = flight1RectTreads + 1;
    const radEnd1 = flight1RectTreads + stepsPerCorner;
    svg += '<text x="' + r(corner1X + stairWidth_plan * 0.65) + '" y="' + r(corner1Y + stairWidth_plan * 0.35) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">' + stepsPerCorner + String.fromCharCode(215) + radiatingAngle + String.fromCharCode(176) + '</text>';
    svg += '<text x="' + r(corner1X + stairWidth_plan * 0.65) + '" y="' + r(corner1Y + stairWidth_plan * 0.55) + '" style="font:8px Arial;fill:#e65100;" text-anchor="middle">' + radStart1 + '-' + radEnd1 + '</text>';
    
    // ===== COIN 2 - MARCHES RAYONNANTES (haut-droite) =====
    // Virage de 90° - entrée par la gauche, sortie vers le bas (volée 2)
    ...
    // Lignes rayonnantes coin 2 - pivot en haut-droite (coin intérieur)
    const pivot2X = r(corner2X + stairWidth_plan);
    const pivot2Y = corner2Y;
    
    // Tracer les lignes de séparation des marches rayonnantes
    for (let i = 1; i < stepsPerCorner; i++) {
        const angle = (Math.PI / 2) + (Math.PI / 2) * (i / stepsPerCorner); // De 90° à 180°
        const x2 = r(pivot2X + Math.cos(angle) * stairWidth_plan);
        const y2 = r(pivot2Y + Math.sin(angle) * stairWidth_plan);
        svg += '<line x1="' + pivot2X + '" y1="' + pivot2Y + '" x2="' + x2 + '" y2="' + y2 + '" stroke="#e65100" stroke-width="1.5"/>';
    }
    
    // Annotation coin 2
    const radStart2 = flight1RectTreads + stepsPerCorner + 1;
    const radEnd2 = flight1RectTreads + totalRadiatingSteps;
    svg += '<text x="' + r(corner2X + stairWidth_plan * 0.35) + '" y="' + r(corner2Y + stairWidth_plan * 0.35) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">' + stepsPerCorner + String.fromCharCode(215) + radiatingAngle + String.fromCharCode(176) + '</text>';
    svg += '<text x="' + r(corner2X + stairWidth_plan * 0.35) + '" y="' + r(corner2Y + stairWidth_plan * 0.55) + '" style="font:8px Arial;fill:#e65100;" text-anchor="middle">' + radStart2 + '-' + radEnd2 + '</text>';
    ...
    if (flight2RectTreads > 0) {
        ...
        // Lignes des girons volée 2 - épaisseur uniforme 1.5
        for (let i = 1; i <= flight2RectTreads; i++) {
            const lineY = r(flight2Y + ((i - 1) * treadDepth_plan));
            svg += '<line x1="' + flight2X + '" y1="' + lineY + '" x2="' + r(flight2X + stairWidth_plan) + '" y2="' + lineY + '" stroke="#1b5e20" stroke-width="1.5"/>';
        }
        
        // Numérotation volée 2 - DÉCALÉE vers l'extérieur (75% de la largeur, côté droit)
        // Seulement première et dernière marche
        ...
        // Annotation nombre de girons rectangulaires volée 2 (côté droit extérieur, rotation -90)
    ...
    // Segment 4: Connecteur coin 2 -> volée 2
    ...
    if (!solutions || solutions.length === 0) {
        ...
        if (flightHeightExceeded && !hasIntermediateLanding) {
            ...
            if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
                ...
                if (treadTooSmall) {
                    if (isMetric) {
                        html += `<li>Longueur totale minimale requise ââ‰¥ ${minLengthWithLanding.toFixed(0)} mm (${minTreads} girons × ${limits.minTread} mm + 2× palier)</li>`;
                    } else {
                        html += `<li>Longueur totale minimale requise ââ‰¥ ${metricToImperial(minLengthWithLanding)} (${minTreads} girons × ${metricToImperial(limits.minTread)} + 2× palier)</li>`;
                    }
                ...
                if (treadTooSmall) {
                    if (isMetric) {
                        html += `<li>Longueur totale minimale requise ââ‰¥ ${minLength.toFixed(0)} mm (${minTreads} girons × ${limits.minTread} mm)</li>`;
                    } else {
                        html += `<li>Longueur totale minimale requise ââ‰¥ ${metricToImperial(minLength)} (${minTreads} girons × ${metricToImperial(limits.minTread)})</li>`;
                    }
                ...
                if (treadTooSmall) {
                    if (isMetric) {
                        html += `<li>Longueur minimale requise ââ‰¥ ${minLength.toFixed(0)} mm (${minTreads} girons × ${limits.minTread} mm)</li>`;
                    } else {
                        html += `<li>Longueur minimale requise ââ‰¥ ${metricToImperial(minLength)} (${minTreads} girons × ${metricToImperial(limits.minTread)})</li>`;
                    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performCalculation() {
        ...
        if (stairConfigValue === 'l_shaped') {
            ...
            // Validation: Largeur palier ââ‰¥ 2 × largeur escalier
     ...
     * Règles:
     * - Largeur palier ââ‰¥ 2 × largeur escalier (géométrie)
    ...
    function validateUShapedInputs() {
        ...
        // Valider: Largeur palier ââ‰¥ 2 × largeur escalier
     ...
     * - Giron à 300 mm ââ‰¥ 190 mm (CNB 9.8.4.7)
     */
