...
function generateDancingStepsVisualization(stairData) {
 ...
 if (isUConfig) {
  ...
  // Zone de virage 1 (coin inférieur gauche) - dessiner comme secteur d'anneau
  ...
  // Pour les marches dansantes, utiliser stairWidth comme rayon extérieur pour cohérence visuelle
  const effectiveInnerR1 = Math.min(innerR_plan, stairWidth_plan * 0.15); // Max 15% de la largeur
  const effectiveOuterR1 = stairWidth_plan;
  
  // Angles pour la zone 1 (virage 90° dans le sens horaire)
  const angleStart1 = 180; // Début de l'arc (vers la gauche)
  const angleEnd1 = 180 + dancingAngle; // Fin selon l'angle des marches dansantes
  
  // Lignes radiales pour les contremarches des marches dansantes
  const angleStep = dancingAngle / dancingStepsPerZone;
  
  // Points pour le secteur d'anneau - Zone 1
  const start1OuterX = turn1CenterX + effectiveOuterR1 * Math.cos(angleStart1 * Math.PI / 180);
  const start1OuterY = turn1CenterY + effectiveOuterR1 * Math.sin(angleStart1 * Math.PI / 180);
  const end1OuterX = turn1CenterX + effectiveOuterR1 * Math.cos(angleEnd1 * Math.PI / 180);
  const end1OuterY = turn1CenterY + effectiveOuterR1 * Math.sin(angleEnd1 * Math.PI / 180);
  const start1InnerX = turn1CenterX + effectiveInnerR1 * Math.cos(angleEnd1 * Math.PI / 180);
  const start1InnerY = turn1CenterY + effectiveInnerR1 * Math.sin(angleEnd1 * Math.PI / 180);
  const end1InnerX = turn1CenterX + effectiveInnerR1 * Math.cos(angleStart1 * Math.PI / 180);
  const end1InnerY = turn1CenterY + effectiveInnerR1 * Math.sin(angleStart1 * Math.PI / 180);
  
  const largeArcFlag1 = dancingAngle > 180 ? 1 : 0;
  
  // Path du secteur d'anneau - Zone 1
  svg += '<path d="M ' + r(start1OuterX) + ' ' + r(start1OuterY);
  svg += ' A ' + r(effectiveOuterR1) + ' ' + r(effectiveOuterR1) + ' 0 ' + largeArcFlag1 + ' 1 ' + r(end1OuterX) + ' ' + r(end1OuterY);
  svg += ' L ' + r(start1InnerX) + ' ' + r(start1InnerY);
  svg += ' A ' + r(effectiveInnerR1) + ' ' + r(effectiveInnerR1) + ' 0 ' + largeArcFlag1 + ' 0 ' + r(end1InnerX) + ' ' + r(end1InnerY);
  ...
  for (let i = 0; i <= dancingStepsPerZone; i++) {
   ...
   const x1 = turn1CenterX + effectiveInnerR1 * Math.cos(angle);
   const y1 = turn1CenterY + effectiveInnerR1 * Math.sin(angle);
   const x2 = turn1CenterX + effectiveOuterR1 * Math.cos(angle);
   const y2 = turn1CenterY + effectiveOuterR1 * Math.sin(angle);
   svg += '<line x1="' + r(x1) + '" y1="' + r(y1) + '" x2="' + r(x2) + '" y2="' + r(y2) + '" stroke="#e65100" stroke-width="1.5"/>';
  }
  
  // Arc intérieur visible - Zone 1
  svg += '<path d="M ' + r(end1InnerX) + ' ' + r(end1InnerY) + ' A ' + r(effectiveInnerR1) + ' ' + r(effectiveInnerR1) + ' 0 ' + largeArcFlag1 + ' 1 ' + r(start1InnerX) + ' ' + r(start1InnerY) + '" fill="none" stroke="#e65100" stroke-width="2"/>';
  ...
  // Zone de virage 2 (coin inférieur droit) - dessiner comme secteur d'anneau
  ...
  // Réutiliser les rayons effectifs de la zone 1
  const effectiveInnerR2 = effectiveInnerR1;
  const effectiveOuterR2 = effectiveOuterR1;
  
  // Angles pour la zone 2 (virage après la zone 1)
  const angleStart2 = 270;
  const angleEnd2 = 270 + dancingAngle;
  
  // Points pour le secteur d'anneau - Zone 2
  const start2OuterX = turn2CenterX + effectiveOuterR2 * Math.cos(angleStart2 * Math.PI / 180);
  const start2OuterY = turn2CenterY + effectiveOuterR2 * Math.sin(angleStart2 * Math.PI / 180);
  const end2OuterX = turn2CenterX + effectiveOuterR2 * Math.cos(angleEnd2 * Math.PI / 180);
  const end2OuterY = turn2CenterY + effectiveOuterR2 * Math.sin(angleEnd2 * Math.PI / 180);
  const start2InnerX = turn2CenterX + effectiveInnerR2 * Math.cos(angleEnd2 * Math.PI / 180);
  const start2InnerY = turn2CenterY + effectiveInnerR2 * Math.sin(angleEnd2 * Math.PI / 180);
  const end2InnerX = turn2CenterX + effectiveInnerR2 * Math.cos(angleStart2 * Math.PI / 180);
  const end2InnerY = turn2CenterY + effectiveInnerR2 * Math.sin(angleStart2 * Math.PI / 180);
  
  const largeArcFlag2 = dancingAngle > 180 ? 1 : 0;
  
  // Path du secteur d'anneau - Zone 2
  svg += '<path d="M ' + r(start2OuterX) + ' ' + r(start2OuterY);
  svg += ' A ' + r(effectiveOuterR2) + ' ' + r(effectiveOuterR2) + ' 0 ' + largeArcFlag2 + ' 1 ' + r(end2OuterX) + ' ' + r(end2OuterY);
  svg += ' L ' + r(start2InnerX) + ' ' + r(start2InnerY);
  svg += ' A ' + r(effectiveInnerR2) + ' ' + r(effectiveInnerR2) + ' 0 ' + largeArcFlag2 + ' 0 ' + r(end2InnerX) + ' ' + r(end2InnerY);
  svg += ' Z" fill="#ffe0b2" stroke="#ff9800" stroke-width="2"/>';
  
  // Lignes radiales pour zone 2
  for (let i = 0; i <= dancingStepsPerZone; i++) {
   const angle = (angleStart2 + i * angleStep) * Math.PI / 180;
   const x1 = turn2CenterX + effectiveInnerR2 * Math.cos(angle);
   const y1 = turn2CenterY + effectiveInnerR2 * Math.sin(angle);
   const x2 = turn2CenterX + effectiveOuterR2 * Math.cos(angle);
   const y2 = turn2CenterY + effectiveOuterR2 * Math.sin(angle);
   svg += '<line x1="' + r(x1) + '" y1="' + r(y1) + '" x2="' + r(x2) + '" y2="' + r(y2) + '" stroke="#e65100" stroke-width="1.5"/>';
  }
  
  // Arc intérieur zone 2
  svg += '<path d="M ' + r(end2InnerX) + ' ' + r(end2InnerY) + ' A ' + r(effectiveInnerR2) + ' ' + r(effectiveInnerR2) + ' 0 ' + largeArcFlag2 + ' 1 ' + r(start2InnerX) + ' ' + r(start2InnerY) + '" fill="none" stroke="#e65100" stroke-width="2"/>';
  ...
  // Zone de virage (coin) - dessiner comme secteur d'anneau
  ...
  // Pour les marches dansantes, utiliser stairWidth comme rayon extérieur pour cohérence visuelle
  const effectiveInnerR = Math.min(innerR_plan, stairWidth_plan * 0.15); // Max 15% de la largeur
  const effectiveOuterR = stairWidth_plan;
  
  // Lignes radiales pour les contremarches des marches dansantes
  const angleStep = dancingAngle / dancingStepsPerZone;
  const angleStart = 180; // Début de l'arc (vers la gauche)
  const angleEnd = 180 - dancingAngle;
  
  // Dessiner le secteur d'anneau (zone de virage) avec un path
