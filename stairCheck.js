...
function generateDancingStepsVisualization(stairData) {
 ...
 // Calcul des dimensions totales selon la configuration et l'angle
 let totalWidth, totalHeight;
 if (isUConfig) {
  totalWidth = actualFlight2Run;
  totalHeight = Math.max(actualFlight1Run, actualFlight3Run) + actualWidth * 2;
 } else {
  // Pour config L, tenir compte de l'angle personnalisé
  const flight2ProjectionX = actualFlight2Run * Math.sin(angleRad);
  const flight2ProjectionY = actualFlight2Run * Math.cos(angleRad);
  totalWidth = actualFlight1Run + actualWidth + flight2ProjectionX;
 ...
 if (isUConfig) {
  ...
  // Virage 1: 90° sens horaire, Virage 2: 90° sens horaire
  
  // Volée 1 (verticale, descend depuis le haut)
  ...
  // Zone de virage 1 (coin inférieur gauche) - secteur d'anneau
  const turn1CenterX = planStartX + stairWidth_plan;
  const turn1CenterY = planStartY + flight1_plan - stairWidth_plan;
  ...
  // Zone de virage 2 (coin inférieur droit) - secteur d'anneau
  const turn2CenterX = planStartX + flight2_plan - stairWidth_plan;
  const turn2CenterY = planStartY + flight1_plan;
  ...
  // Volée 3 (verticale, monte vers le haut)
  ...
  // La volée 1 est horizontale (de gauche à droite)
  // Les marches dansantes tournent vers le haut (angle personnalisable)
  // La volée 2 part vers le haut selon l'angle
  ...
  // Zone de virage - centre au coin intérieur (coin supérieur droit de la volée 1)
  const turnCenterX = v1X + flight1_plan;
  const turnCenterY = v1Y;
  ...
  const v2EndX = v2StartX + flight2_plan * Math.cos(flight2DirRad);
  const v2EndY = v2StartY + flight2_plan * Math.sin(flight2DirRad);
  ...
  if (flight2RectTreads > 0) {
   ...
   const treadLen = flight2_plan / flight2RectTreads;
   for (let i = 1; i < flight2RectTreads; i++) {
  ...
  const arrowMidX = turnCenterX + stairWidth_plan/2 * Math.cos(perpAngle);
  const arrowMidY = v1Y + stairWidth_plan/2 * Math.sin(perpAngle);
  ...
  // Cotations - Longueur totale horizontale
  const dimY1 = r(v1Y + stairWidth_plan + 25);
  const totalHorizontal = flight1_plan + stairWidth_plan + flight2_plan * Math.sin(angleRad);
  svg += '<line x1="' + v1X + '" y1="' + dimY1 + '" x2="' + r(v1X + totalHorizontal) + '" y2="' + dimY1 + '" stroke="#555" stroke-width="1" marker-start="url(#arrDS)" marker-end="url(#arrDE)"/>';
  svg += '<text x="' + r(v1X + totalHorizontal/2) + '" y="' + r(dimY1 + 14) + '" style="font:10px Arial;fill:#333;" text-anchor="middle">' + formatValueForPlan(actualFlight1Run + actualWidth, isMetric) + '</text>';
  ...
  svg += '<text x="' + r(dimX2 + 5) + '" y="' + r((v2EndY + v1Y)/2 + 4) + '" style="font:10px Arial;fill:#333;">' + formatValueForPlan(actualFlight2Run, isMetric) + '</text>';
 }
