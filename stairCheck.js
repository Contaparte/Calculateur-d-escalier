...
function generateDancingStepsVisualization(stairData) {
 ...
 if (isUConfig) {
  ...
  // CORRIGÉ: Parcours de gauche à droite puis vers le bas
  // Structure: marches rect 1 (horizontal) → zone de virage → marches rect 2 (vertical vers le bas)
  
  // Calcul des dimensions totales pour un L classique
  const totalWidth = rect1Length + actualWidth;
  const totalHeight = actualWidth + rect2Length;
  ...
  // Centre du virage: coin INFÉRIEUR-DROIT de la volée 1
  // C'est le coin intérieur de l'escalier pour un virage à droite
  ...
  // Angles: de 270° (vers le haut, connexion avec volée 1) vers 270° + angle (vers la droite/bas)
  // Dans SVG: 0° = droite, 90° = bas, 180° = gauche, 270° = haut
  const angleStart = 270; // Vers le haut (fin de volée 1)
  const angleEnd = 270 + dancingAngle; // Tourner dans le sens horaire
  ...
  // Lignes radiales pour les contremarches
  for (let i = 0; i <= dancingStepsPerZone; i++) {
  ...
  // Volée 2: verticale, descendant vers le bas, ALIGNÉE avec le bord droit de la zone de virage
  // Pour un angle de 90°, la volée 2 est parfaitement verticale sous le virage
  // Pour d'autres angles, elle est inclinée
  
  // Point de départ de la volée 2 = fin du secteur sur le bord extérieur
  const v2StartX = endOuterX;
  const v2StartY = endOuterY;
  
  // Direction de la volée 2: tangente au cercle au point final
  const flight2DirAngle = angleEnd; // Direction radiale au point final
  ...
  // Direction perpendiculaire pour la largeur (vers l'intérieur du virage)
  ...
  // Flèche de montée - suit le parcours en L
  ...
  // Segment dans le virage puis vers la volée 2
  const arrowMidX = turnCenterX + stairWidth_plan/2 * Math.cos((angleStart + dancingAngle/2) * Math.PI / 180);
  const arrowMidY = turnCenterY + stairWidth_plan/2 * Math.sin((angleStart + dancingAngle/2) * Math.PI / 180);
  svg += '<line x1="' + r(turnCenterX) + '" y1="' + arrowY + '" x2="' + r(arrowMidX) + '" y2="' + r(arrowMidY) + '" stroke="#1b5e20" stroke-width="1.5"/>';
  
  // Segment vers la fin de volée 2
  const arrowEndX = v2EndX + stairWidth_plan/2 * Math.cos(perpAngle);
  const arrowEndY = v2EndY + stairWidth_plan/2 * Math.sin(perpAngle);
  svg += '<line x1="' + r(arrowMidX) + '" y1="' + r(arrowMidY) + '" x2="' + r(arrowEndX) + '" y2="' + r(arrowEndY) + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrDMontee)"/>';
  ...
  const dimX2 = r(Math.max(turnCenterX + stairWidth_plan, v2EndX + stairWidth_plan) + 20);
  svg += '<line x1="' + dimX2 + '" y1="' + r(v1Y + stairWidth_plan) + '" x2="' + dimX2 + '" y2="' + r(v2EndY) + '" stroke="#555" stroke-width="1" marker-start="url(#arrDS)" marker-end="url(#arrDE)"/>';
  svg += '<text x="' + r(dimX2 + 5) + '" y="' + r((v1Y + stairWidth_plan + v2EndY)/2 + 4) + '" style="font:10px Arial;fill:#333;">' + formatValueForPlan(rect2Length, isMetric) + '</text>';
 }
