...
const CNB_LIMITS = {
 ...
 // =====================================================================
 // ESCALIERS HÉLICOÃ  ÂDAUX (CNB 2020, Article 9.8.4.7)
 // =====================================================================
 spiral: {
...
function metricToImperial(mmValue) {
 ...
 // Gérer le cas oÃ  Â¹ wholeInches >= 12 (par exemple, 3' 12— devient 4'-0—)
 if (wholeInches >= 12) {
 ...
 * doivent Ãªtre testés APRÃ Ë— S les formats avec apostrophe obligatoire
 */
/**
 * Convertit une valeur impériale en millimètres
 ...
 * - Ã —šÂ¬ l'intersection des deux volées (cas standard)
 * - Ã —šÂ¬ l'extrémité de la volée (quand firstFlightRun ou secondFlightRun = stairWidth)
 */
function calculateLShapedWithRadiating(params) {
 ...
 // Détecter si les marches rayonnantes sont Ã  Â  l'extrémité
 // (quand l'une des dimensions est égale Ã  Â  la largeur de l'escalier)
 ...
 if (radiatingAtEnd || radiatingAtStart) {
  // CAS SPÉCIAL: Marches rayonnantes Ã  Â  l'extrémité
...
function calculateUShapedStair(params) {
 ...
 switch (uShapedConfig) {
  ...
  numLandings = 1;
  availableForTreads = flight1Run + flight2Run - landingLength;
  break;
  case 'rect_landing_radiating_rect':
  // Configuration palier + marches rayonnantes (3 volées)
  numLandings = 1;
  availableForTreads = flight1Run + flight2Run + (flight3Run || 0) - landingLength;
  break;
 ...
 if (hasRadiating) {
  // Pour 180° de rotation totale avec marches rayonnantes:
  // 45°: 2 marches par coin Ã— 2 coins = 4 marches
  // 30°: 3 marches par coin Ã— 2 coins = 6 marches
...
function calculateSpiralStair(params) {
 ...
 // Pour l'hélicoïdal, le giron est mesuré Ã  Â  300mm de l'axe de la main courante
...
function generateLShapedStairVisualization(stairData) {
 ...
 if (isLanding) {
  ...
  // Annotation nombre de girons rectangulaires volée 2 (Ã  Â  gauche de la volée)
...
function generateUShapedStairVisualization(stairData) {
 ...
 // IMPORTANT: Les dimensions des volées sont calculées Ã  Â  partir du nombre de girons Ã— profondeur uniforme
 ...
 if (flight1Treads > 0) {
  for (let i = 1; i <= flight1Treads; i++) {
  // Position Y: du bas vers le haut, chaque giron a la MÃ Ã…Â ME profondeur que dans volée 2
 ...
 // ===== VOLÉE 2 (droite, descend de haut vers bas = sens opposé Ã  Â  volée 1) =====
 ...
 if (flight2Treads > 0) {
  for (let i = 1; i <= flight2Treads; i++) {
  // Position Y: depuis le haut, mÃªme profondeur que volée 1
  ...
  // Annotation nombre de girons rectangulaires volée 2 - MÃ Ã…Â ME DIRECTION que volée 1 (rotation -90)
 ...
 * Conforme au CNB 9.8.4.6: marches rayonnantes Ã  Â  30° ou 45°, max 90° par série
 */
function generateUShapedRadiatingVisualization(stairData) {
 ...
 // Un espace correspondrait Ã Â  un palier, ce qui est une autre configuration
 ...
 // CNB 9.8.4.4.(3) et 9.8.4.5.(1): Dans une mÃªme volée, le giron doit Ãªtre UNIFORME
 // Pour cette configuration sans palier = UNE SEULE VOLÉE
 // Donc TOUS les girons rectangulaires ont la MÃ Ã…Â ME profondeur (treadDepth calculé globalement)
 ...
 // Note: totalRectRun et numRectTreads déjÃ Â  calculés plus haut
 ...
 if (flight1RectTreadsCalc > 0) {
  // Hauteur du rectangle = nombre de girons Ã— giron uniforme (pour cohérence visuelle)
 ...
 // Les deux coins étant adjacents, le pivot est Ã Â  leur intersection
 const pivotX = corner2X; // = corner1X + stairWidth_plan
 const pivotY = r(corner1Y + stairWidth_plan);
 
 // COIN 1: Lignes de 180° (gauche) Ã Â  90° (haut)
 ...
 // COIN 2: Lignes de 90° (haut) Ã Â  0° (droite) - la ligne Ã Â  90° est déjÃ Â  tracée
 ...
 const arrowX1 = r(flight1X + stairWidth_plan / 2);
 const arrowX2 = r(flight2X + stairWidth_plan / 2);
 // Les flèches doivent démarrer exactement Ã  la première contremarche et finir Ã  la dernière
 ...
 const dimX1 = r(flight1X - 30);
 // Afficher la dimension réelle (giron uniforme Ã— nombre de girons + largeur)
 ...
 * Conforme au CNB 9.8.4.6: chaque série de marches rayonnantes â—°Â¤ 90°
 */
function generateUShapedMixedVisualization(stairData) {
 ...
 if (configType === 'rect_landing_radiating_rect') {
  ...
  svg += '<text x=—' + r(landingX + stairWidth_plan/2) + '— y=—' + r(landingY + landingDepth_plan * 0.35 + 4) + '— style=—font:italic 10px Arial;fill:#f57f17;— text-anchor=—middle">Palier</text>';
  
  // Zone de marches rayonnantes (en haut Ã  droite, coin 90°)
  ...
  // Flèche de direction (virage Ã  90°)
  const arrowX1 = r(flight1X + stairWidth_plan/2);
  const arrowX2 = r(flight2X + stairWidth_plan/2);
  ...
  // Segment 2: Traverse le palier horizontalement (virage Ã  90°)
  ...
  // Zone de marches rayonnantes (en haut Ã  gauche, coin 90°)
  const cornerX = planStartX;
  const cornerY = planStartY;
  svg += '<rect x=—' + cornerX + '— y=—' + cornerY + '— width=—' + stairWidth_plan + '— height=—' + stairWidth_plan + '— fill=—#ffecb3— stroke=—#f9a825— stroke-width=—2—/>';
  ...
  const arrowX1 = r(flight1X + stairWidth_plan/2);
  const arrowX2 = r(flight2X + stairWidth_plan/2);
  const arrowCornerY = r(cornerY + stairWidth_plan * 0.65);
  
  // Segment 1: Monte dans la volée 1 jusqu'au coin
  svg += '<line x1=—' + arrowX1 + '— y1=—' + r(flight1Y + flight1Run_plan) + '— x2=—' + arrowX1 + '— y2=—' + arrowCornerY + '— stroke=—#1b5e20— stroke-width=—1.5—/>';
  // Segment 2: Traverse horizontalement le coin et le palier (virage Ã  90°)
  svg += '<line x1=—' + arrowX1 + '— y1=—' + arrowCornerY + '— x2=—' + arrowX2 + '— y2=—' + arrowCornerY + '— stroke=—#1b5e20— stroke-width=—1.5—/>';
  // Segment 3: Descend vers la volée 2
  svg += '<line x1=—' + arrowX2 + '— y1=—' + arrowCornerY + '— x2=—' + arrowX2 + '— y2=—' + r(flight2Y) + '— stroke=—#1b5e20— stroke-width=—1.5—/>';
  // Segment 4: Descend dans la volée 2 avec flèche
  svg += '<line x1=—' + arrowX2 + '— y1=—' + r(flight2Y) + '— x2=—' + arrowX2 + '— y2=—' + r(flight2Y + flight3Run_plan) + '— stroke=—#1b5e20— stroke-width=—1.5— marker-end=—url(#arrUMMontee)—/>';
  svg += '<text x=—' + arrowX1 + '— y=—' + r(flight1Y + flight1Run_plan + 14) + '— style=—font:bold 9px Arial;fill:#1b5e20;— text-anchor=—middle">En haut</text>';
 }
...
function generateUShapedDoubleRadiatingVisualization(stairData) {
    ...
    // Répartition équitable des girons rectangulaires
    let flight1RectTreads, flight2RectTreads;
    if (totalRectRun > 0 && numRectTreads > 0) {
        const ratio1 = actualFlight1Run / totalRectRun;
    ...
    // Calculer l'échelle
    // Structure: deux colonnes (volées) avec palier central en haut
    const totalPlanWidth = actualLandingWidth + actualLandingDepth; // Largeur palier + une zone rayonnante
    const maxFlightRun = Math.max(actualFlight1Run, actualFlight2Run);
    const totalPlanHeight = maxFlightRun + actualWidth; // Volées + coins rayonnants
    
    const scalePlan = Math.min(planW / totalPlanWidth, planH / totalPlanHeight) * 0.55;
    ...
    // Position de départ centrée
    // Structure: [volée1][corner1][palier][corner2][volée2]
    const totalW_plan = (2 * stairWidth_plan) + landingDepth_plan;
    const totalH_plan = Math.max(flight1Run_visual, flight2Run_visual) + stairWidth_plan;
    const planStartX = r(margin.left + (planW - totalW_plan) / 2);
    const planStartY = r(margin.top + (planH - totalH_plan) / 2);
    ...
    svg += '<rect x=—' + landingX + '— y=—' + landingY + '— width=—' + landingDepth_plan + '— height=—' + stairWidth_plan + '— fill=—#fff9c4— stroke=—#f9a825— stroke-width=—2—/>';
    // Texte —Palier— positionné en haut pour éviter conflit avec la flèche
    svg += '<text x=—' + r(landingX + landingDepth_plan/2) + '— y=—' + r(landingY + stairWidth_plan * 0.35) + '— style=—font:italic 10px Arial;fill:#f57f17;— text-anchor=—middle">Palier</text>';
    
    // ===== COIN 2 - MARCHES RAYONNANTES (haut-droite) =====
    const corner2X = r(landingX + landingDepth_plan);
    const corner2Y = planStartY;
    ...
    // Cotation largeur totale (en bas)
    const dimY1 = r(Math.max(flight1Y + flight1Run_visual, flight2Y + flight2Run_visual) + 20);
    const totalWidthReal = (2 * actualWidth) + actualLandingDepth;
    const totalWidthText = formatValueForPlan(totalWidthReal, isMetric);
    svg += '<line x1=—' + planStartX + '— y1=—' + dimY1 + '— x2=—' + r(corner2X + stairWidth_plan) + '— y2=—' + dimY1 + '— stroke=—#555— stroke-width=—1— marker-start=—url(#arrUDRS)— marker-end=—url(#arrUDRE)—/>';
    svg += '<text x=—' + r((planStartX + corner2X + stairWidth_plan) / 2) + '— y=—' + (dimY1 + 14) + '— style=—font:10px Arial;fill:#333;— text-anchor=—middle">' + totalWidthText + '</text>';
...
function displayCalculatorResults(solutions, params) {
 ...
 if (!solutions || solutions.length === 0) {
  ...
  let html = '<h3>âœ— Aucune solution conforme trouvée</h3>';
  ...
  if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
   ...
   if (treadTooSmall) {
    if (isMetric) {
    html += `<li>Longueur totale minimale requise >= ${minLengthWithLanding.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm + 2Ã— palier)</li>`;
    } else {
    html += `<li>Longueur totale minimale requise >= ${metricToImperial(minLengthWithLanding)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)} + 2Ã— palier)</li>`;
    }
   ...
   if (treadTooSmall) {
    if (isMetric) {
    html += `<li>Longueur totale minimale requise >= ${minLength.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm)</li>`;
    } else {
    html += `<li>Longueur totale minimale requise >= ${metricToImperial(minLength)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)})</li>`;
    }
   ...
   html += '<li>Les marches rayonnantes se situent Ã  Â  l\'intersection des deux volées</li>';
  } else {
   if (treadTooSmall) {
    if (isMetric) {
    html += `<li>Longueur minimale requise >= ${minLength.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm)</li>`;
    } else {
    html += `<li>Longueur minimale requise >= ${metricToImperial(minLength)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)})</li>`;
    }
 ...
 if (isCompliant) {
  ...
  html += '<h3>âœ— Aucune solution conforme trouvée</h3>';
 }
 ...
 if (!isTreadOk) {
  ...
  html += `<div class="warning"><p>âš  Profondeur de giron ${formatValuePrecise(best.treadDepth, isMetric)} inférieure au minimum CNB (${formatValue(limits.minTread, isMetric, 0)})</p></div>`;
  }
 }
 ...
 if (best.totalRadiatingSteps > 0) {
  ...
  html += `<li><strong>Marches rayonnantes :</strong> ${best.totalRadiatingSteps} (${best.totalRadiatingSteps === 4 ? '2Ã—45°' : '2Ã—3Ã—30°'})</li>`;
 ...
 if (best.useLandingConfiguration) {
  ...
  html += `<li><strong>Hauteur volée 1 :</strong> ${formatValuePrecise(best.flight1Height, isMetric)} (${best.risersInFlight1} CM) ${flight1Ok ? 'âœ“' : 'âœ— > 3,7 m'}</li>`;
  html += `<li><strong>Hauteur volée 2 :</strong> ${formatValuePrecise(best.flight2Height, isMetric)} (${best.risersInFlight2} CM) ${flight2Ok ? 'âœ“' : 'âœ— > 3,7 m'}</li>`;
  
  // Avertissement si une volée dépasse la limite
  if (!flight1Ok || !flight2Ok) {
   html += '</ul></div>';
   html += '<div class="warning"><p><strong>âš  Non-conformité CNB 9.8.3.3 :</strong> ';
   if (!flight1Ok && !flight2Ok) {
 ...
 if (isMetric) {
  ...
  html += riseError < 0.01 ? 'âœ“ Exact' : `âš  Écart ${riseError.toFixed(4)} mm`;
  ...
  html += treadError < 0.01 ? 'âœ“ Exact' : `âš  Écart ${treadError.toFixed(4)} mm`;
  }
 } else {
 ...
 if (stairConfigValue === 'l_shaped' && (lShapedConfigValue === 'two_45deg' || lShapedConfigValue === 'three_30deg')) {
  ...
  html += '<ul>';
  html += '<li>Les marches rayonnantes se situent Ã  Â  l\'intersection des deux volées perpendiculaires</li>';
  ...
  html += '<li> â†³  1ère direction : ' + best.firstFlightRectTreads + ' girons Ã— ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
  ...
  html += '<li> â†³  2ème direction : ' + best.secondFlightRectTreads + ' girons Ã— ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
  ...
  html += '<li>Giron mesuré Ã  Â  500 mm de la rive étroite (CNB)</li>';
  html += '</ul></div>';
 }
 ...
 if (isCompliant && stairConfigValue === 'l_shaped' && (best.useLandingConfiguration || best.numRadiatingSteps > 0)) {
  ...
  // Pour marches rayonnantes Ã  Â  l'extrémité
 ...
 // (chaque série de marches rayonnantes ne peut tourner qu'Ã  max 90°, donc un virage de 180° nécessite un palier)
 ...
 if (best.isSpiral) {
  ...
  html += '<ul>';
  html += '<li>Giron mesuré Ã  Â  300 mm de l\'axe de la main courante</li>';
...
function displayVerificationResults(params) {
 ...
 // Règle du pas
 html += '<div class="result-section">';
 html += `<h4>${stepRule.isValid ? 'âœ“' : 'âœ—'} Règle du pas (${stepRule.validCount} sur 3)</h4>`;
 html += '<ul>';
 html += `<li>${stepRule.rule1.isValid ? 'âœ“' : 'âœ—'} Règle 1 (G+H) : ${stepRule.rule1.value.toFixed(2)}— (17—-18—)</li>`;
 html += `<li>${stepRule.rule2.isValid ? 'âœ“' : 'âœ—'} Règle 2 (GÃ—H) : ${stepRule.rule2.value.toFixed(2)} poÂ² (71-74)</li>`;
 html += `<li>${stepRule.rule3.isValid ? 'âœ“' : 'âœ—'} Règle 3 (G+2H) : ${stepRule.rule3.value.toFixed(2)}— (22—-25")</li>`;
 html += '</ul></div>';
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 // ===== Mise Ã  Â  jour des placeholders et visibilité selon le système de mesure =====
 ...
 function updateCalcLShapedSubOptions() {
  ...
  // (les marches rayonnantes se situent Ã  Â  l'intersection des deux volées)
  calcStandardRunContainer.style.display = 'none';
 ...
 function updateCalcUShapedSubOptions() {
  ...
  // Mettre Ã  Â  jour les labels des volées selon la configuration
 ...
 function performCalculation() {
  ...
  // Validation: Largeur palier >= 2 Ã— largeur escalier
  if (uLandingWidthValue > 0 && stairWidthValue > 0 && uLandingWidthValue < (2 * stairWidthValue)) {
   ...
   if (errEl) {
    if (isMetric) {
    errEl.textContent = `La largeur du palier (${uLandingWidthValue.toFixed(0)} mm) ne peut Ãªtre inférieure au double de la largeur de l'escalier (${(2 * stairWidthValue).toFixed(0)} mm) pour cette configuration en U.`;
    } else {
    errEl.textContent = `La largeur du palier ne peut Ãªtre inférieure au double de la largeur de l'escalier (min. ${metricToImperial(2 * stairWidthValue)}) pour cette configuration en U.`;
    }
   }
   isValid = false;
  }
  ...
  if (uLandingDepthValue > 0 && stairWidthValue > 0 && uLandingDepthValue < stairWidthValue) {
   ...
   if (errEl) {
    if (isMetric) {
    errEl.textContent = `La profondeur du palier (${uLandingDepthValue.toFixed(0)} mm) ne peut Ãªtre inférieure Ã  Â  la largeur de l'escalier (${stairWidthValue.toFixed(0)} mm) selon CNB 9.8.6.3.`;
    } else {
    errEl.textContent = `La profondeur du palier ne peut Ãªtre inférieure Ã  Â  la largeur de l'escalier (min. ${metricToImperial(stairWidthValue)}) selon CNB 9.8.6.3.`;
    }
   }
   isValid = false;
  }
  ...
  contentDiv.innerHTML = '<h3>âœ— Configuration non permise</h3>' +
   '<div class="warning"><p>Les escaliers hélicoÃ Â¯daux ne sont pas permis pour les bâtiments de Partie 3 (CNB 9.8.4.7.(3)).</p>' +
   '<p><strong>Solutions :</strong></p><ul>' +
   '<li>Utilisez un escalier droit ou avec palier</li>' +
   '<li>Les hélicoÃ Â¯daux sont réservés aux logements de Partie 9</li></ul></div>';
  resultDiv.className = 'result non-compliant';
  resultDiv.style.display = 'block';
  return;
  }
  ...
  // Calculer l'espace entre les volées (si largeur palier > 2Ã— largeur escalier)
 ...
 function performVerification() {
  ...
  // Vérifications spécifiques Ã  Â  la configuration
  if (stairConfigValue === 'dancing_steps' && narrowSideValue > 0) {
  if (narrowSideValue < limits.minNarrowSide) {
   issues.push(`Giron côté étroit ${narrowSideValue.toFixed(0)} mm < minimum ${limits.minNarrowSide} mm Ã  Â  300 mm de l'axe`);
   isCompliant = false;
  }
  ...
  // Vérifications spécifiques pour escaliers hélicoÃ Â¯daux (CNB 9.8.4.7)
  ...
  * Règles:
  * - Largeur palier >= 2 Ã— largeur escalier (géométrie)
 ...
 function validateUShapedInputs() {
  ...
  // Valider: Largeur palier >= 2 Ã— largeur escalier
  ...
  if (stairWidth > 0 && landingWidth > 0 && landingWidth < (2 * stairWidth)) {
   ...
   if (isMetric) {
    landingWidthError.textContent = `La largeur du palier (${landingWidth.toFixed(0)} mm) ne peut Ãªtre inférieure au double de la largeur de l'escalier (${minRequired.toFixed(0)} mm) pour cette configuration en U.`;
   } else {
    landingWidthError.textContent = `La largeur du palier ne peut Ãªtre inférieure au double de la largeur de l'escalier (min. ${metricToImperial(minRequired)}) pour cette configuration en U.`;
   }
  } else {
  ...
  if (stairWidth > 0 && landingDepth > 0 && landingDepth < stairWidth) {
   if (isMetric) {
    landingDepthError.textContent = `La profondeur du palier (${landingDepth.toFixed(0)} mm) ne peut Ãªtre inférieure Ã  Â  la largeur de l'escalier (${stairWidth.toFixed(0)} mm) selon CNB 9.8.6.3.`;
   } else {
    landingDepthError.textContent = `La profondeur du palier ne peut Ãªtre inférieure Ã  Â  la largeur de l'escalier (min. ${metricToImperial(stairWidth)}) selon CNB 9.8.6.3.`;
   }
  } else {
  ...
  // Aussi valider la largeur de l'escalier si les dimensions du palier sont déjÃ  Â  saisies
  const stairWidthError = document.getElementById('stairDesiredWidthError');
  if (stairWidthError && stairWidth > 0) {
  // Vérifier si largeur escalier est compatible avec largeur palier déjÃ  Â  saisie
 ...
 function validateLShapedInputs() {
  ...
  // Pour l'escalier en L, la profondeur du palier est égale Ã  Â  la largeur de l'escalier
  ...
  * - Giron Ã  Â  300 mm >= 190 mm (CNB 9.8.4.7)
  */
 function validateSpiralInputs() {
  ...
  // Note: Pour l'instant, les champs hélicoÃ Â¯daux ne sont pas encore présents dans le calculateur
  // Cette fonction est prÃªte pour une future implémentation
 }
