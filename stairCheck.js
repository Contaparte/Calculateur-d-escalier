...
function generateLShapedStairVisualization(stairData) {
    ...
    if (isLanding) {
        ...
        if (isRadiatingAtExtremity && radiatingAtEnd) {
            // CAS SPÉCIAL: Marches rayonnantes à la FIN de la volée
            // Configuration: volée horizontale + carré rayonnant à droite
            // L'escalier monte de gauche à droite, puis tourne vers le haut
            
            const treadW1_exact = stairW1_plan / flight1Treads;
            
            // Position du carré rayonnant (à droite de la volée)
            const cornerX = r(planStartX + stairW1_plan);
            const cornerY = planStartY; // Aligné en haut
            ...
            for (let i = 1; i < flight1Treads; i++) {
                ...
                svg += '<line x1="' + lineX + '" y1="' + planStartY + '" x2="' + lineX + '" y2="' + r(planStartY + stairWidth_plan) + '" stroke="#1b5e20" stroke-width="1"/>';
            }
            
            // Zone marches rayonnantes (carré à droite)
            svg += '<rect x="' + cornerX + '" y="' + cornerY + '" width="' + L + '" height="' + L + '" fill="#ffe0b2" stroke="#ff9800" stroke-width="2"/>';
            
            // Lignes rayonnantes - pivot au coin HAUT-GAUCHE (côté intérieur)
            // Les lignes vont vers le BAS-DROIT (extérieur)
            const pivotX = cornerX;
            const pivotY = cornerY;
            
            if (numRadSteps === 2) {
                // Ligne diagonale à 45° du haut-gauche vers bas-droit
            ...
            // Flèche de montée - horizontale puis virage vers le haut
            ...
            const dimY1 = r(planStartY + stairWidth_plan + 18);
            ...
            // Annotation
            svg += '<text x="' + r(planStartX + stairW1_plan/2) + '" y="' + r(planStartY - 5) + '" style="font:8px Arial;fill:#666;" text-anchor="middle">' + flight1Treads + ' rect.</text>';
            
        } else if (isRadiatingAtExtremity && radiatingAtStart) {
            // CAS SPÉCIAL: Marches rayonnantes au DÉBUT de la volée
            // Configuration: carré rayonnant en bas + volée verticale au-dessus
            // L'escalier tourne d'abord puis monte vers le haut
            
            const treadW2_exact = stairW2_plan / flight2Treads;
            
            // Position du carré rayonnant (en bas, aligné avec la volée au-dessus)
            const cornerX = planStartX;
            const cornerY = r(planStartY + stairW2_plan);
            
            // Zone marches rayonnantes (carré en bas)
            svg += '<rect x="' + cornerX + '" y="' + cornerY + '" width="' + L + '" height="' + L + '" fill="#ffe0b2" stroke="#ff9800" stroke-width="2"/>';
            
            // Lignes rayonnantes - pivot au coin HAUT-GAUCHE (côté intérieur de l'escalier)
            // Les lignes vont vers le BAS-DROIT (extérieur)
            const pivotX = cornerX;
            const pivotY = cornerY;
            ...
            // Volée 2 (verticale, au-dessus du carré) - girons rectangulaires
            ...
            // Flèche de montée - part du bas du carré, tourne et monte
            const arrowStartY = r(cornerY + L/2);
            const arrowX = r(cornerX + stairWidth_plan/2);
            svg += '<line x1="' + r(cornerX - 15) + '" y1="' + arrowStartY + '" x2="' + arrowX + '" y2="' + arrowStartY + '" stroke="#1b5e20" stroke-width="2"/>';
            svg += '<line x1="' + arrowX + '" y1="' + arrowStartY + '" x2="' + arrowX + '" y2="' + r(planStartY + 15) + '" stroke="#1b5e20" stroke-width="2" marker-end="url(#arrLEG)"/>';
            
            svg += '<text x="' + r(cornerX - 25) + '" y="' + (arrowStartY - 5) + '" style="font:italic 9px Arial;fill:#1b5e20;">Montee</text>';
            
            // Cotations horizontales (largeur du carré)
            const dimY1 = r(cornerY + L + 18);
            ...
            svg += '<text x="' + r(cornerX + L/2) + '" y="' + (dimY1 + 12) + '" style="font:9px Arial;fill:#333;" text-anchor="middle">' + widthText + '</text>';
            
            // Cotation verticale (longueur totale)
            ...
            svg += '<text x="' + (dimX2 + 5) + '" y="' + r((planStartY + cornerY + L)/2) + '" style="font:9px Arial;fill:#333;">' + run2Text + '</text>';
