...
function generateStraightStairVisualization(stairData) {
    ...
    // Fleche de montee - fine, de la 1ere a la derniere contremarche
    const arrowY = r(planStartY + stairH_plan / 2);
    const arrowStartX = r(planStartX + treadW_plan);  // Commence a la 1ere contremarche
    ...
    // Indicateur E.H. (En Haut) au point de depart de la fleche
    svg += '<text x="' + r(arrowStartX - 5) + '" y="' + (arrowY + 4) + '" style="font:bold 10px Arial;fill:#1b5e20;" text-anchor="end">E.H.</text>';
    ...
    // ===== LEGENDE COMPLETE EN BAS =====
    const legendY = H - 12;
    svg += '<text x="15" y="' + legendY + '" style="font:10px Arial;fill:#555;">';
    svg += 'Legende: CM (contremarche) = ' + riserText + '  |  G (giron) = ' + treadText + '  |  Largeur = ' + widthText + '  |  Hauteur totale = ' + riseText + ' (' + numRisers + ' CM)';
    svg += '</text>';
...
function generateLShapedStairVisualization(stairData) {
    ...
    if (isLanding) {
        ...
        if (isRadiatingAtExtremity && radiatingAtEnd) {
            ...
            if (numRadSteps === 2) {
                ...
                svg += '<text x="' + r(cornerX + L/2) + '" y="' + r(cornerY + L/2 + 4) + '" style="font:bold 10px Arial;fill:#e65100;" text-anchor="middle">2x45</text>';
            } else {
                ...
                svg += '<text x="' + r(cornerX + L/2) + '" y="' + r(cornerY + L/2 + 4) + '" style="font:bold 10px Arial;fill:#e65100;" text-anchor="middle">3x30</text>';
            }
            
            // Fleche de montee continue - une seule fleche qui tourne a 90 degres
            // Part de la 1ere CM horizontale, tourne dans le carre, sort par le haut
            const arrow1Y = r(planStartY + stairWidth_plan/2);
            const arrowStartX = r(planStartX + treadW1_exact);  // Depart a la 1ere contremarche
            ...
            // E.H. au depart
            svg += '<text x="' + r(arrowStartX - 5) + '" y="' + (arrow1Y + 4) + '" style="font:bold 9px Arial;fill:#1b5e20;" text-anchor="end">E.H.</text>';
            ...
            // Fleche de montee continue - une seule fleche qui tourne a 90 degres
            // Part du bas du carre rayonnant, tourne, et monte jusqu'a la derniere CM
            const arrowX = r(cornerX + stairWidth_plan/2);
            const arrowStartY = r(cornerY + L);  // Depart au bas du carre (entree de l'escalier)
            const arrowCornerY = r(cornerY + L/2);  // Point de virage au centre du carre
            const arrowEndY = r(planStartY);  // Fin a la derniere contremarche
            
            // Segment dans le carre (du bas vers le centre)
            svg += '<line x1="' + arrowX + '" y1="' + arrowStartY + '" x2="' + arrowX + '" y2="' + arrowCornerY + '" stroke="#1b5e20" stroke-width="1.5"/>';
            // Segment vertical avec fleche (du centre jusqu'a la derniere CM)
            svg += '<line x1="' + arrowX + '" y1="' + arrowCornerY + '" x2="' + arrowX + '" y2="' + arrowEndY + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrLMontee)"/>';
            
            // E.H. au depart (en bas du carre rayonnant)
            svg += '<text x="' + (arrowX + 5) + '" y="' + r(arrowStartY - 3) + '" style="font:bold 9px Arial;fill:#1b5e20;">E.H.</text>';
    ...
    // ===== LEGENDE EN BAS =====
    let legendText = 'Legende: CM = ' + riserText + '  |  G = ' + treadText + '  |  Largeur = ' + widthText + '  |  Hauteur = ' + riseText + ' (' + numRisers + ' CM)';
    if (isLanding) {
