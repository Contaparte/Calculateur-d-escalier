...
function generateStairVisualization(stairConfig, params) {
    ...
    const svgPadding = 40;
    const scale = Math.min(600 / (totalRun + 200), 400 / (totalRise + 200));
    ...
    // Ajouter une grille de fond légère
    svg += `<defs>
    ...
    // Fonction pour dessiner une marche
    const drawStep = (x, y, width, depth, height, fill = "#d9d9d9", stroke = "#666") => {
        return `
            <g>
                <rect x="${x}" y="${y}" width="${width}" height="${depth}" fill="${fill}" stroke="${stroke}" stroke-width="1"/>
                <rect x="${x}" y="${y + depth}" width="${width}" height="${height}" fill="#bbb" stroke="${stroke}" stroke-width="1"/>
                <line x1="${x}" y1="${y + depth}" x2="${x + width}" y2="${y + depth}" stroke="#999" stroke-width="1"/>
            </g>
        `;
    };
    
    // Dessiner les escaliers selon la configuration
    let stairsContent = '';
    ...
    switch(stairConfig) {
        case 'straight':
            ...
            const arrowY = svgHeight - svgPadding - (numRisers * scaledRiserHeight) / 2;
            stairsContent += `
        ...
        case 'turning_90':
            ...
            if (params.lShapedConfig === 'standard_landing') {
                ...
                stairsContent += `<rect x="${landingX}" y="${landingY - scaledStairWidth}" width="${scaledStairWidth}" height="${scaledStairWidth}" fill="#d9d9d9" stroke="#666" stroke-width="1"/>`;
            } else if (params.lShapedConfig === 'two_45deg' || params.lShapedConfig === 'three_30deg') {
                // Marches rayonnantes (simplifié)
                ...
                for (let i = 0; i < numRadialSteps; i++) {
                    ...
                    stairsContent += `
                        <path d="M ${svgPadding} ${cornerY - i * scaledRiserHeight} 
                              ...
                              fill="#d9d9d9" stroke="#666" stroke-width="1"/>
                    `;
                }
            }
            ...
            for (let i = halfTreads; i < numTreads; i++) {
                ...
                stairsContent += drawStep(x, y, scaledTreadDepth, scaledStairWidth, scaledRiserHeight, "#d9d9d9", "#666");
            }
            
            // Ajouter des flèches indiquant la direction
            stairsContent += `
                <path d="M ${svgPadding + scaledStairWidth / 2} ${svgHeight - svgPadding} 
        ...
        case 'turning_180':
            ...
            stairsContent += `<rect x="${landingX}" y="${landingY - scaledStairWidth}" width="${scaledStairWidth * 2 + scaledTreadDepth}" height="${scaledStairWidth}" fill="#d9d9d9" stroke="#666" stroke-width="1"/>`;
            
            // Dessiner la deuxième volée (descendante)
            for (let i = 0; i < thirdTreads; i++) {
                const x = svgPadding + scaledStairWidth + scaledTreadDepth;
                const y = landingY - scaledStairWidth - (i + 1) * scaledRiserHeight;
                stairsContent += drawStep(x, y, scaledStairWidth, scaledTreadDepth, scaledRiserHeight);
            }
            ...
            stairsContent += `
                <path d="M ${svgPadding + scaledStairWidth / 2} ${svgHeight - svgPadding} 
                      ...
                      fill="none" stroke="#333" stroke-width="2" marker-end="url(#arrowhead)" />
                <path d="M ${svgPadding + scaledStairWidth + scaledTreadDepth + scaledStairWidth / 2} ${landingY - scaledStairWidth - 20} 
                          L ${svgPadding + scaledStairWidth + scaledTreadDepth + scaledStairWidth / 2} 
                            ${landingY - scaledStairWidth - thirdTreads * scaledRiserHeight - 20}" 
                      fill="none" stroke="#333" stroke-width="2" marker-end="url(#arrowhead)" />
                <defs>
                    <marker id="arrowhead" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
                        <polygon points="0 0, 10 3.5, 0 7" fill="#333" />
        ...
        case 'turning_60':
            ...
            // Dessiner les marches tournantes
            ...
            for (let i = 0; i < turnSteps; i++) {
                ...
                stairsContent += `
                    <path d="M ${innerX1} ${innerY1} 
                             L ${outerX1} ${outerY1} 
                             A ${radius} ${radius} 0 0 1 ${outerX2} ${outerY2} 
                             L ${innerX1} ${innerY1} Z" 
                          fill="#d9d9d9" stroke="#666" stroke-width="1"/>
                    <line x1="${innerX1}" y1="${innerY1 - scaledRiserHeight}" x2="${outerX2}" y2="${outerY2 - scaledRiserHeight}" 
            ...
            for (let i = 0; i < afterTurn; i++) {
                ...
                // Rotation du rectangle pour aligner avec la nouvelle direction
                const rotationAngle = angle;
                
                stairsContent += `
                    <g transform="translate(${stepX}, ${stepY}) rotate(${rotationAngle}) translate(${-scaledTreadDepth}, 0)">
                        ${drawStep(0, 0, scaledStairWidth, scaledTreadDepth, scaledRiserHeight)}
                    </g>
                `;
            }
        ...
        case 'dancing_steps':
            ...
            for (let i = 0; i < danceSteps; i++) {
                ...
                stairsContent += `
                    <path d="M ${innerX1} ${innerY1} 
                             ...
                             L ${innerX1} ${innerY1} Z" 
                          fill="#d9d9d9" stroke="#666" stroke-width="1"/>
                    <line x1="${innerX1}" y1="${innerY1}" x2="${outerX2}" y2="${outerY2}" 
                          stroke="#999" stroke-width="1" stroke-dasharray="5,5"/>
                    
                    <!-- Indication de la largeur du côté étroit -->
                    <line x1="${innerX1}" y1="${innerY1}" x2="${innerX1 + narrowSideLength * Math.sin(endRadAngle)}" 
                          y2="${innerY1 - narrowSideLength * (1 - Math.cos(endRadAngle))}" 
            ...
            for (let i = 0; i < afterDance; i++) {
                ...
                stairsContent += `
                    <g transform="translate(${stepX}, ${stepY}) rotate(${danceAngle}) translate(${-scaledTreadDepth}, 0)">
                        ${drawStep(0, 0, scaledStairWidth, scaledTreadDepth, scaledRiserHeight)}
                    </g>
                `;
            }
            ...
            stairsContent += `
                <path d="M ${svgPadding + scaledStairWidth / 2} ${svgHeight - svgPadding + 20} 
                          L ${svgPadding + scaledStairWidth / 2} ${svgHeight - svgPadding - beforeDance * scaledRiserHeight - 20}" 
                      fill="none" stroke="#333" stroke-width="2" marker-end="url(#arrowhead)" />
                <defs>
                    <marker id="arrowhead" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
                        <polygon points="0 0, 10 3.5, 0 7" fill="#333" />
        ...
        case 'spiral':
            ...
            stairsContent += `<circle cx="${centerX}" cy="${centerY}" r="10" fill="#8B4513" stroke="#666" stroke-width="1"/>`;
            ...
            for (let i = 0; i < numRisers; i++) {
                ...
                // Calculer la hauteur visuelle (ajustée pour la perspective)
                const visualHeight = 3 + (numRisers - i) * 0.5;
                
                // Dessiner la marche hélicoïdale avec une légère perspective
                stairsContent += `
                    <path d="M ${innerX} ${innerY} 
                             L ${outerX1} ${outerY1} 
                             A ${spiralRadius} ${spiralRadius} 0 0 1 ${outerX2} ${outerY2} 
                             L ${innerX} ${innerY} Z" 
                          fill="#d9d9d9" stroke="#666" stroke-width="1" transform="translate(0, ${-stepHeight/numRisers*i})"/>
                `;
            }
            
            // Ajouter flèche de direction circulaire
            stairsContent += `
                <path d="M ${centerX} ${centerY + spiralRadius + 20} A ${spiralRadius + 20} ${spiralRadius + 20} 0 1 1 ${centerX} ${centerY - spiralRadius - 20}" 
                      fill="none" stroke="#333" stroke-width="2" stroke-dasharray="5,5" marker-end="url(#arrowhead)" />
                <defs>
                    <marker id="arrowhead" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
                        <polygon points="0 0, 10 3.5, 0 7" fill="#333" />
                    </marker>
            ...
            if (narrowSide > 0) {
                const scaledNarrowSide = narrowSide * scale;
                stairsContent += `
                    <line x1="${centerX}" y1="${centerY}" x2="${centerX + scaledNarrowSide}" y2="${centerY}" 
                          stroke="#f00" stroke-width="2"/>
                    <text x="${centerX + scaledNarrowSide/2}" y="${centerY - 5}" text-anchor="middle" fill="#f00" font-size="10">
                        ${narrowSide} mm
                    </text>
                `;
            }
    ...
    svg += `
        <g class="dimensions">
            <!-- Cotation hauteur -->
            <line x1="${svgPadding - 20}" y1="${svgHeight - svgPadding}" x2="${svgPadding - 20}" y2="${svgHeight - svgPadding - scaledTotalRise}" 
                  stroke="#333" stroke-width="1" marker-start="url(#dimensionMarker)" marker-end="url(#dimensionMarker)"/>
            <text x="${svgPadding - 25}" y="${svgHeight - svgPadding - scaledTotalRise/2}" transform="rotate(-90, ${svgPadding - 25}, ${svgHeight - svgPadding - scaledTotalRise/2})" 
                  text-anchor="middle" fill="#333" font-size="12">
                ${formatNumber(totalRise)} mm
            </text>
            
            <!-- Cotation longueur -->
            <line x1="${svgPadding}" y1="${svgHeight - svgPadding + 20}" x2="${svgPadding + scaledTotalRun}" y2="${svgHeight - svgPadding + 20}" 
                  stroke="#333" stroke-width="1" marker-start="url(#dimensionMarker)" marker-end="url(#dimensionMarker)"/>
    ...
    // Ajouter les marches au SVG
    svg += stairsContent;
    
    // Légende
    svg += `
        <g class="legend" transform="translate(${svgWidth - 150}, 20)">
            <rect x="0" y="0" width="130" height="80" fill="white" stroke="#ccc" stroke-width="1" opacity="0.8"/>
            <text x="5" y="20" font-weight="bold" font-size="12">Légende:</text>
            <rect x="5" y="30" width="15" height="15" fill="#d9d9d9" stroke="#666" stroke-width="1"/>
            <text x="25" y="42" font-size="10">Marche</text>
            <line x1="5" y1="60" x2="20" y2="60" stroke="#f00" stroke-width="2"/>
            <text x="25" y="62" font-size="10">Dimension critique</text>
        </g>
    `;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    checkButton.addEventListener('click', function() {
        ...
        if (config === 'spiral') {
            ...
            totalRun = stairWidthValue * 3; // Estimation de la largeur totale
            totalRise = riserHeightValue * (numTreads + 1); // Hauteur totale estimée
        } else {
            ...
            totalRun = treadDepthValue * numTreads; // Longueur totale estimée
            totalRise = riserHeightValue * (numTreads + 1); // Hauteur totale estimée
        }
