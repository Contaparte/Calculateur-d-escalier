...
function generateLShapedStairVisualization(stairData) {
    ...
    if (isLanding) {
        ...
        // Règle: les lignes partent du coin INTÉRIEUR (haut-gauche du carré) vers le côté EXTÉRIEUR (bas-droit)
        // Le coin intérieur est où les côtés intérieurs de l'escalier se rejoignent
        const pivotX = cornerX;
        const pivotY = cornerY;
        
        if (numRadSteps === 2) {
            // 2 marches à 45 degrés - une ligne à 45 degrés
            // La ligne va du pivot (haut-gauche/intérieur) vers le bas-droit (extérieur)
            ...
            // Angles mesurés depuis l'horizontale (vers le bas-droit)
            // Ligne à 30° depuis l'horizontale (proche de la volée 1)
            const endX30 = r(pivotX + stairWidth_plan * Math.cos(Math.PI/6));
            const endY30 = r(pivotY + stairWidth_plan * Math.sin(Math.PI/6));
            // Ligne à 60° depuis l'horizontale (proche de la volée 2)
