...
function generateLShapedStairVisualization(stairData) {
    ...
    if (isRadiatingAtExtremity && radiatingAtEnd) {
        ...
        // Cas standard (palier ou intersection)
        totalPlanW = flight1Run + actualWidth;
        totalPlanH = flight2Run + actualWidth;
    }
    ...
    if (isLanding) {
        ...
        // Fleche de montee continue - part de l'EXTERIEUR, traverse le palier, finit en haut
        const arrow1Y = r(planStartY + stairW2_plan + stairWidth_plan/2);
        const arrow1StartX = r(planStartX);  // Depart a l'EXTERIEUR (avant la 1ere CM)
        const arrowCornerX = r(palierX + stairWidth_plan/2);  // Centre du palier
        const arrowEndY = r(planStartY);  // Fin a la derniere CM
        
        // Segment horizontal (volee 1) - part de l'exterieur
        ...
        // E.H. a l'EXTERIEUR de la premiere marche
        svg += '<text x="' + r(arrow1StartX - 8) + '" y="' + (arrow1Y + 4) + '" style="font:bold 9px Arial;fill:#1b5e20;" text-anchor="end">E.H.</text>';
        ...
        if (isRadiatingAtExtremity && radiatingAtEnd) {
            ...
            // Fleche de montee continue - part de l'EXTERIEUR, tourne a 90 degres
            const arrow1Y = r(planStartY + stairWidth_plan/2);
            const arrowStartX = r(planStartX);  // Depart a l'EXTERIEUR (avant la 1ere CM)
            const arrowCornerX = r(cornerX + L/2);
            const arrowEndY = r(cornerY);  // Fin au bord superieur du carre (derniere CM)
            
            // Segment horizontal (de l'EXTERIEUR jusqu'au centre du carre)
            ...
            // E.H. a l'EXTERIEUR de la premiere marche
            svg += '<text x="' + r(arrowStartX - 8) + '" y="' + (arrow1Y + 4) + '" style="font:bold 9px Arial;fill:#1b5e20;" text-anchor="end">E.H.</text>';
            ...
            // Fleche de montee continue - part de l'EXTERIEUR (sous le carre), monte jusqu'en haut
            const arrowX = r(cornerX + stairWidth_plan/2);
            const arrowStartY = r(cornerY + L + 8);  // Depart a l'EXTERIEUR (sous le carre)
            const arrowEndY = r(planStartY);  // Fin a la derniere contremarche
            
            // Segment vertical complet (de l'EXTERIEUR jusqu'a la derniere CM)
            svg += '<line x1="' + arrowX + '" y1="' + arrowStartY + '" x2="' + arrowX + '" y2="' + arrowEndY + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrLMontee)"/>';
            
            // E.H. a l'EXTERIEUR (sous le carre rayonnant)
            svg += '<text x="' + arrowX + '" y="' + r(arrowStartY + 12) + '" style="font:bold 9px Arial;fill:#1b5e20;" text-anchor="middle">E.H.</text>';
