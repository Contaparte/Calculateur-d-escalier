...
function metricToImperial(mmValue) {
 ...
 // Gérer le cas oà Â¹ wholeInches >= 12 (par exemple, 3' 12" devient 4'-0")
 if (wholeInches >= 12) {
...
function generateUShapedStairVisualization(stairData) {
 ...
 // Numéro du palier dans la séquence continue (flight1Treads + 1)
...
function displayCalculatorResults(solutions, params) {
 ...
 html += '<div class="result-section">';
 html += `<h4>${best.stepRule.isValid ? '✓' : 'â—‹'} Règle du pas (${best.stepRule.validCount} sur 3)</h4>`;
 html += '<ul>';
 html += `<li>${best.stepRule.rule1.isValid ? '✓' : 'â—‹'} G + H = ${best.stepRule.rule1.value.toFixed(2)}" (${best.stepRule.rule1.range})</li>`;
 html += `<li>${best.stepRule.rule2.isValid ? '✓' : 'â—‹'} G × H = ${best.stepRule.rule2.value.toFixed(2)} (${best.stepRule.rule2.range})</li>`;
 html += `<li>${best.stepRule.rule3.isValid ? '✓' : 'â—‹'} G + 2H = ${best.stepRule.rule3.value.toFixed(2)}" (${best.stepRule.rule3.range})</li>`;
 html += '</ul></div>';
