...
function validateImperialInput(inputValue) {
    ...
    inputValue = inputValue.replace(/[""Ã¢â‚¬Â³]/g, '"');
 ...
 * doivent Ãªtre testÃ©s APRÃƒË†S les formats avec apostrophe obligatoire
 */
/**
 * Convertit une valeur impÃ©riale en millimÃ¨tres
...
function imperialToMetric(imperialValue) {
    ...
    input = input.replace(/[''']/g, "'").replace(/[""Ã¢â‚¬Â³]/g, '"');
 ...
 * Partie 9: Pas de limite spÃ©cifique mais recommandÃ© < 3,7 m
 */
function checkFlightHeightLimits(riserHeight, numRisers, buildingType, stairType) {
    ...
    if (buildingType === 'part3') {
        // Groupe B, division 2 (soins, traitement, dÃ©tention) - max 2,4 m
        ...
        if (flightHeight > maxHeight) {
            return {
                ...
                maxHeight: maxHeight,
                message: `La hauteur de volÃ©e (${(flightHeight/1000).toFixed(2)} m) dÃ©passe la limite de ${(maxHeight/1000).toFixed(1)} m (CNB 3.4.6.3)`
            };
        }
    }
...
function calculateStraightStair(params) {
    ...
    for (let numRisers = minRisers; numRisers <= maxRisers + 3; numRisers++) {
        ...
        // VÃ©rifier la hauteur maximale de volÃ©e (CNB 3.4.6.3)
...
function calculateLShapedWithRadiating(params) {
    ...
    // La longueur totale est la somme des deux volÃ©es perpendiculaires
    ...
    for (let numRisers = minRisers; numRisers <= maxRisers + 3; numRisers++) {
        ...
        // IMPORTANT: Escalier en L avec marches rayonnantes = UNE SEULE VOLÃ‰E
        // Tous les girons rectangulaires doivent avoir la MÃŠME profondeur (uniformitÃ© dans une volÃ©e)
        // Les marches rayonnantes occupent un espace diagonal Ã  l'intersection
        
        // Facteur d'espace : une marche rayonnante occupe environ 0.7 Ã— giron dans chaque direction
        const radiatingSpace = numRadiatingSteps * radiatingFactor;
        
        // RÃ©soudre le systÃ¨me pour trouver la rÃ©partition optimale des girons
        // N1 Ã— G + radiatingSpace Ã— G = firstFlightRun
        // N2 Ã— G + radiatingSpace Ã— G = secondFlightRun
        // N1 + N2 = numRectTreads
        //
        // Pour que G soit identique dans les deux Ã©quations :
        // N1 = (firstFlightRun Ã— numRectTreads + radiatingSpace Ã— (firstFlightRun - secondFlightRun)) / totalRun
        ...
        // Calculer le giron uniforme Ã  partir de chaque direction
        ...
        // VÃ©rifier que la diffÃ©rence entre les deux est acceptable
        // (sinon les dimensions entrÃ©es ne permettent pas un giron parfaitement uniforme)
        const treadVariation = Math.abs(treadDepth1 - treadDepth2);
        if (treadVariation > 25) continue; // Max 25mm de diffÃ©rence
...
function generateStraightStairVisualization(stairData) {
    const {
        numRisers,
        numTreads,
        riserHeight,   // en mm (toujours)
        treadDepth,    // en mm (toujours)
        totalRise,     // en mm (toujours)
        totalRun,      // en mm (toujours)
        isMetric
    } = stairData;
    
    // Toutes les valeurs sont déjà en mm - pas de conversion nécessaire
    const rise = totalRise;
    const run = totalRun;
    const riser = riserHeight;
    const tread = treadDepth;
    ...
    // Calcul de l'échelle pour que l'escalier remplisse bien la zone
    const scaleX = drawWidth / run;
    const scaleY = drawHeight / rise;
    const scale = Math.min(scaleX, scaleY) * 0.95;
    
    // Dimensions réelles de l'escalier à l'échelle
    const stairWidth = run * scale;
    const stairHeight = rise * scale;
    
    // Centrage horizontal
    const offsetX = paddingLeft + (drawWidth - stairWidth) / 2;
    
    // Point de départ (bas gauche de l'escalier)
    const startX = offsetX;
    const startY = svgHeight - paddingBottom;
    
    // Formatage des dimensions selon le système choisi
    ...
    let svg = `<svg width="${svgWidth}" height="${svgHeight}" xmlns="http://www.w3.org/2000/svg" style="background: #fafafa; border: 1px solid #ddd; border-radius: 6px;">`;
    
    // Définitions et styles
    svg += `<defs>
        <style>
            .stair-step { stroke: #2d5016; stroke-width: 2.5; fill: none; }
            .stair-fill { fill: #e8f5e9; stroke: none; }
            .ground-line { stroke: #8b4513; stroke-width: 2; }
            .floor-line { stroke: #8b4513; stroke-width: 2; }
            .dim-line { stroke: #666; stroke-width: 1; }
            .dim-text { font-family: 'Segoe UI', Arial, sans-serif; font-size: 12px; fill: #333; font-weight: 600; }
            .title-text { font-family: 'Segoe UI', Arial, sans-serif; font-size: 14px; fill: #1a1a1a; font-weight: bold; }
            .label-text { font-family: 'Segoe UI', Arial, sans-serif; font-size: 11px; fill: #555; }
            .annotation-line { stroke-width: 1.5; }
            .annotation-text { font-family: 'Segoe UI', Arial, sans-serif; font-size: 11px; font-weight: 600; }
        </style>
        <marker id="arrowStart" markerWidth="8" markerHeight="8" refX="0" refY="4" orient="auto">
            <path d="M8,0 L0,4 L8,8" fill="none" stroke="#666" stroke-width="1"/>
        </marker>
        <marker id="arrowEnd" markerWidth="8" markerHeight="8" refX="8" refY="4" orient="auto">
            <path d="M0,0 L8,4 L0,8" fill="none" stroke="#666" stroke-width="1"/>
        </marker>
    </defs>`;
    
    // Titre
    svg += `<text x="20" y="28" class="title-text">Vue de profil - Volée droite</text>`;
    
    // Construire le chemin de l'escalier (forme remplie)
    let fillPath = `M ${startX} ${startY}`;
    let currentX = startX;
    let currentY = startY;
    ...
    for (let i = 0; i < numRisers; i++) {
        ...
        // Avancer (giron) sauf pour la dernière marche
        if (i < numTreads) {
    ...
    const endX = currentX;
    const endY = currentY;
    fillPath += ` L ${endX + 30} ${endY} L ${endX + 30} ${startY} Z`;
    
    // Dessiner le remplissage
    svg += `<path d="${fillPath}" class="stair-fill"/>`;
    
    // Dessiner le contour de l'escalier (marches)
    currentX = startX;
    currentY = startY;
    let stepPath = `M ${startX} ${startY}`;
    
    for (let i = 0; i < numRisers; i++) {
        currentY -= riser * scale;
        stepPath += ` L ${currentX} ${currentY}`;
    ...
    svg += `<path d="${stepPath}" class="stair-step"/>`;
    
    // Ligne du sol (niveau de départ)
    svg += `<line x1="${startX - 40}" y1="${startY}" x2="${startX + stairWidth + 10}" y2="${startY}" class="ground-line"/>`;
    svg += `<text x="${startX - 45}" y="${startY + 4}" class="label-text" text-anchor="end">Sol</text>`;
    
    // Ligne du plancher supérieur (niveau d'arrivée)
    svg += `<line x1="${endX - 10}" y1="${endY}" x2="${endX + 60}" y2="${endY}" class="floor-line"/>`;
    svg += `<text x="${endX + 65}" y="${endY + 4}" class="label-text" text-anchor="start">Arrivée</text>`;
    
    // === COTATION HORIZONTALE (longueur totale) ===
    const dimYBottom = startY + 35;
    svg += `<line x1="${startX}" y1="${dimYBottom}" x2="${startX + stairWidth}" y2="${dimYBottom}" class="dim-line" marker-start="url(#arrowStart)" marker-end="url(#arrowEnd)"/>`;
    svg += `<line x1="${startX}" y1="${startY + 5}" x2="${startX}" y2="${dimYBottom + 5}" class="dim-line"/>`;
    svg += `<line x1="${startX + stairWidth}" y1="${startY + 5}" x2="${startX + stairWidth}" y2="${dimYBottom + 5}" class="dim-line"/>`;
    svg += `<text x="${startX + stairWidth / 2}" y="${dimYBottom + 18}" class="dim-text" text-anchor="middle">${runText}</text>`;
    svg += `<text x="${startX + stairWidth / 2}" y="${dimYBottom + 32}" class="label-text" text-anchor="middle">(${numTreads} girons × ${treadText})</text>`;
    
    // === COTATION VERTICALE (hauteur totale) ===
    const dimXRight = endX + 45;
    svg += `<line x1="${dimXRight}" y1="${startY}" x2="${dimXRight}" y2="${endY}" class="dim-line" marker-start="url(#arrowStart)" marker-end="url(#arrowEnd)"/>`;
    svg += `<line x1="${endX + 5}" y1="${startY}" x2="${dimXRight + 5}" y2="${startY}" class="dim-line"/>`;
    svg += `<line x1="${endX + 5}" y1="${endY}" x2="${dimXRight + 5}" y2="${endY}" class="dim-line"/>`;
    
    // Texte vertical pour la hauteur
    const midY = (startY + endY) / 2;
    svg += `<text x="${dimXRight + 12}" y="${midY - 8}" class="dim-text" text-anchor="start">${riseText}</text>`;
    svg += `<text x="${dimXRight + 12}" y="${midY + 8}" class="label-text" text-anchor="start">(${numRisers} CM)</text>`;
    
    // === ANNOTATION D'UNE MARCHE TYPE (au milieu) ===
    const midStep = Math.floor(numTreads / 2);
    const annotBaseX = startX + midStep * tread * scale;
    const annotBaseY = startY - midStep * riser * scale;
    
    // Annotation du giron (horizontal - bleu)
    const gironY = annotBaseY - riser * scale + 8;
    svg += `<line x1="${annotBaseX + 2}" y1="${gironY}" x2="${annotBaseX + tread * scale - 2}" y2="${gironY}" stroke="#0066cc" class="annotation-line" marker-start="url(#arrowStart)" marker-end="url(#arrowEnd)"/>`;
    svg += `<text x="${annotBaseX + tread * scale / 2}" y="${gironY - 6}" class="annotation-text" fill="#0066cc" text-anchor="middle">G</text>`;
    
    // Annotation de la contremarche (vertical - rouge)
    const cmX = annotBaseX - 12;
    svg += `<line x1="${cmX}" y1="${annotBaseY - 2}" x2="${cmX}" y2="${annotBaseY - riser * scale + 2}" stroke="#cc0000" class="annotation-line" marker-start="url(#arrowStart)" marker-end="url(#arrowEnd)"/>`;
    svg += `<text x="${cmX - 6}" y="${annotBaseY - riser * scale / 2 + 4}" class="annotation-text" fill="#cc0000" text-anchor="end">CM</text>`;
    
    // === LÉGENDE ===
    const legendX = 20;
    const legendY = svgHeight - 35;
    svg += `<text x="${legendX}" y="${legendY}" class="label-text">CM (contremarche) = ${riserText}</text>`;
    svg += `<text x="${legendX + 200}" y="${legendY}" class="label-text">G (giron) = ${treadText}</text>`;
...
function displayCalculatorResults(solutions, params) {
    ...
    if (!solutions || solutions.length === 0) {
        ...
        let html = '<h3>Aucune solution conforme trouvÃ©e</h3>';
        ...
        if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
            ...
            if (isMetric) {
                html += `<li>Longueur totale minimale requise Ã¢â€°Ë† ${minLengthWithLanding.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm + 2Ã— palier)</li>`;
            } else {
                html += `<li>Longueur totale minimale requise Ã¢â€°Ë† ${metricToImperial(minLengthWithLanding)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)} + 2Ã— palier)</li>`;
            }
            ...
            if (isMetric) {
                html += `<li>Longueur totale minimale requise â‰ˆ ${minLength.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm)</li>`;
            } else {
                html += `<li>Longueur totale minimale requise â‰ˆ ${metricToImperial(minLength)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)})</li>`;
            }
            ...
            if (isMetric) {
                html += `<li>Longueur minimale requise Ã¢â€°Ë† ${minLength.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm)</li>`;
            } else {
                html += `<li>Longueur minimale requise Ã¢â€°Ë† ${metricToImperial(minLength)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)})</li>`;
            }
    ...
    html += `<h3>${isCompliant ? 'âœ“' : 'âš '} Solution optimale (${codeRef})</h3>`;
    ...
    html += `<li><strong>Largeur :</strong> ${formatValue(stairWidthValue, isMetric, 0)} ${isWidthOk ? 'âœ“' : 'âš  '}</li>`;
    html += '</ul></div>';
    ...
    // RÃ¨gle du pas
    html += '<div class="result-section">';
    html += `<h4>${best.stepRule.isValid ? 'âœ“' : 'âš  '} RÃ¨gle du pas (${best.stepRule.validCount} sur 3)</h4>`;
    html += '<ul>';
    ...
    // Visualisation graphique pour volÃ©e droite uniquement
...
function displayVerificationResults(params) {
    ...
    // RÃ¨gle du pas
    html += '<div class="result-section">';
    html += `<h4>${stepRule.isValid ? 'âœ“' : 'âš  '} RÃ¨gle du pas (${stepRule.validCount} sur 3)</h4>`;
    html += '<ul>';
