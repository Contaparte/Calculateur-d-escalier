...
const CNB_LIMITS = {
 ...
 // =====================================================================
 // ESCALIERS HÉLICOàÃ‚ÂDAUX (CNB 2020, Article 9.8.4.7)
 // =====================================================================
 spiral: {
...
function checkStepRule(riserMm, treadMm) {
 ...
 // Règle 2: G × H = 71 à 74 poÃƒâ€šÃ‚Â² (458-477 cmÃƒâ€šÃ‚Â²)
 ...
 return {
  ...
  rule2: { value: rule2Value, isValid: rule2Valid, range: '71-74 poÃƒâ€šÃ‚Â²' },
...
function generateUShapedStairVisualization(stairData) {
 ...
 if (flight1Treads > 0) {
  for (let i = 1; i <= flight1Treads; i++) {
  // Position Y: du bas vers le haut, chaque giron a la MÃƒÅ ME profondeur que dans volée 2
 ...
 if (flight2Treads > 0) {
  ...
  // Annotation nombre de girons rectangulaires volée 2 - MÃƒÅ ME DIRECTION que volée 1 (rotation -90)
...
function generateUShapedRadiatingVisualization(stairData) {
 ...
 // Donc TOUS les girons rectangulaires ont la MÃƒÅ ME profondeur (treadDepth calculé globalement)
...
function generateUShapedDoubleRadiatingVisualization(stairData) {
    ...
    // ===== FLÃƒË†CHE DE DIRECTION (virage à 90°) =====
    const arrowX1 = r(flight1X + stairWidth_plan / 2);
    const arrowX2 = r(flight2X + stairWidth_plan / 2);
...
function displayCalculatorResults(solutions, params) {
 ...
 if (solutions && solutions.error && solutions.flight2RunError) {
  ...
  let html = '<h3>âÅ“â€” Paramètre incohérent</h3>';
 ...
 if (!solutions || solutions.length === 0) {
  ...
  let html = '<h3>âÅ“â€” Aucune solution conforme trouvée</h3>';
  html += '<div class="warning">';
 ...
 if (isCompliant) {
  html += '<h3>âÅ“â€œ Solution optimale</h3>';
 } else {
  html += '<h3>âÅ“â€” Aucune solution conforme trouvée</h3>';
 }
 
 // Avertissements pour non-conformité
 if (!isWidthOk) {
  html += `<div class="warning"><p>âÅ¡Â  Largeur ${formatValue(stairWidthValue, isMetric, 0)} inférieure au minimum requis (${formatValue(limits.minWidth, isMetric, 0)})</p></div>`;
 }
 if (!isRiserOk) {
  html += `<div class="warning"><p>âÅ¡Â  Hauteur de contremarche ${formatValuePrecise(best.riserHeight, isMetric)} hors limites CNB (${formatValue(limits.minRiser, isMetric, 0)} à ${formatValue(limits.maxRiser, isMetric, 0)})</p></div>`;
 }
 if (!isTreadOk) {
  if (hasMaxTreadLimit) {
  html += `<div class="warning"><p>âÅ¡Â  Profondeur de giron ${formatValuePrecise(best.treadDepth, isMetric)} hors limites CNB (${formatValue(limits.minTread, isMetric, 0)} à ${formatValue(limits.maxTread, isMetric, 0)})</p></div>`;
  } else {
  html += `<div class="warning"><p>âÅ¡Â  Profondeur de giron ${formatValuePrecise(best.treadDepth, isMetric)} inférieure au minimum CNB (${formatValue(limits.minTread, isMetric, 0)})</p></div>`;
  }
 }
 ...
 if (best.useLandingConfiguration) {
  ...
  html += `<li><strong>Hauteur volée 1 :</strong> ${formatValuePrecise(best.flight1Height, isMetric)} (${best.risersInFlight1} CM) ${flight1Ok ? 'âÅ“â€œ' : 'âÅ“â€” > 3,7 m'}</li>`;
  html += `<li><strong>Hauteur volée 2 :</strong> ${formatValuePrecise(best.flight2Height, isMetric)} (${best.risersInFlight2} CM) ${flight2Ok ? 'âÅ“â€œ' : 'âÅ“â€” > 3,7 m'}</li>`;
 ...
 html += `<li><strong>Largeur :</strong> ${formatValue(stairWidthValue, isMetric, 0)} ${isWidthOk ? 'âÅ“â€œ' : 'âÅ“â€”'}</li>`;
 html += '</ul></div>';
 ...
 if (isMetric) {
  ...
  html += riseError < 0.01 ? 'âÅ“â€œ Exact' : `âÅ¡Â  Écart ${riseError.toFixed(4)} mm`;
  ...
  html += treadError < 0.01 ? 'âÅ“â€œ Exact' : `âÅ¡Â  Écart ${treadError.toFixed(4)} mm`;
  }
 } else {
  const riserIn = best.riserHeight / 25.4;
  const totalRiseIn = (best.riserHeight * best.numRisers) / 25.4;
  html += `${best.numRisers} × ${riserIn.toFixed(6)}" = ${totalRiseIn.toFixed(6)}" âÅ“â€œ`;
 }
 ...
 html += '<div class="result-section">';
 html += `<h4>${best.stepRule.isValid ? 'âÅ“â€œ' : 'ââ€”â€¹'} Règle du pas (${best.stepRule.validCount} sur 3)</h4>`;
 html += '<ul>';
 html += `<li>${best.stepRule.rule1.isValid ? 'âÅ“â€œ' : 'ââ€”â€¹'} G + H = ${best.stepRule.rule1.value.toFixed(2)}" (${best.stepRule.rule1.range})</li>`;
 html += `<li>${best.stepRule.rule2.isValid ? 'âÅ“â€œ' : 'ââ€”â€¹'} G × H = ${best.stepRule.rule2.value.toFixed(2)} (${best.stepRule.rule2.range})</li>`;
 html += `<li>${best.stepRule.rule3.isValid ? 'âÅ“â€œ' : 'ââ€”â€¹'} G + 2H = ${best.stepRule.rule3.value.toFixed(2)}" (${best.stepRule.rule3.range})</li>`;
 html += '</ul></div>';
 ...
 html += `<li>Nombre de girons tracés : ${best.numTreads} (le dernier giron n'est pas comptabiliseÌ, car il correspond au niveau du plancher supeÌrieur)</li>`;
 ...
 if (stairConfigValue === 'l_shaped' && (lShapedConfigValue === 'two_45deg' || lShapedConfigValue === 'three_30deg')) {
  ...
  html += '<li> ââ€ Â³  1ère direction : ' + best.firstFlightRectTreads + ' girons × ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
  ...
  html += '<li> ââ€ Â³  2ème direction : ' + best.secondFlightRectTreads + ' girons × ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
...
function displayVerificationResults(params) {
 ...
 if (isCompliant) {
  html += `<h3>âÅ“â€œ Conforme au ${codeReference}</h3>`;
 } else {
  html += `<h3>âÅ“â€” Non conforme au ${codeReference}</h3>`;
  html += '<div class="result-section">';
 ...
 // Règle du pas
 html += '<div class="result-section">';
 html += `<h4>${stepRule.isValid ? 'âÅ“â€œ' : 'âÅ“â€”'} Règle du pas (${stepRule.validCount} sur 3)</h4>`;
 html += '<ul>';
 html += `<li>${stepRule.rule1.isValid ? 'âÅ“â€œ' : 'âÅ“â€”'} Règle 1 (G+H) : ${stepRule.rule1.value.toFixed(2)}" (17"-18")</li>`;
 html += `<li>${stepRule.rule2.isValid ? 'âÅ“â€œ' : 'âÅ“â€”'} Règle 2 (G×H) : ${stepRule.rule2.value.toFixed(2)} poÃƒâ€šÃ‚Â² (71-74)</li>`;
 html += `<li>${stepRule.rule3.isValid ? 'âÅ“â€œ' : 'âÅ“â€”'} Règle 3 (G+2H) : ${stepRule.rule3.value.toFixed(2)}" (22"-25")</li>`;
 html += '</ul></div>';
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 function performCalculation() {
  ...
  // Vérifier que la 2ème partie (segment horizontal) peut contenir le palier minimum
  ...
  if (uConfigHasLanding && stairWidthValue > 0 && uSecondFlightRunValue > 0) {
   // Le segment horizontal doit contenir: 2 × largeur escalier (coins) + profondeur palier minimum
   const minLandingDepth = stairWidthValue; // CNB 9.8.6.3: palier >= largeur escalier
   const minRequiredWidth = (2 * stairWidthValue) + minLandingDepth;
   
   if (uSecondFlightRunValue < minRequiredWidth) {
    const errEl = document.getElementById('uSecondFlightRunError');
    if (errEl) {
    if (isMetric) {
     errEl.textContent = `Dimension insuffisante pour palier CNB 9.8.6.3: min. ${minRequiredWidth.toFixed(0)} mm (2×${stairWidthValue.toFixed(0)} mm + palier ${minLandingDepth.toFixed(0)} mm)`;
    } else {
     errEl.textContent = `Dimension insuffisante pour palier CNB 9.8.6.3: min. ${metricToImperial(minRequiredWidth)} (2× largeur + palier min.)`;
    }
    }
    isValid = false;
   }
  }
  // Validation CNB 9.8.6.3 pour 1ère et 3ème parties: doivent contenir zone virage + girons
  if (uConfigHasLanding && stairWidthValue > 0) {
   const limits = getCNBLimits(buildingTypeValue, stairTypeValue);
   const minTread = limits.minTread || 255;
   const minDimensionRequired = stairWidthValue + minTread; // Zone virage + au moins un giron
   
   if (uFirstFlightRunValue > 0 && uFirstFlightRunValue < minDimensionRequired) {
    const errEl = document.getElementById('uFirstFlightRunError');
    if (errEl) {
    if (isMetric) {
     errEl.textContent = `Dimension insuffisante CNB 9.8.6.3: min. ${minDimensionRequired.toFixed(0)} mm (zone ${stairWidthValue.toFixed(0)} mm + giron min.)`;
    } else {
     errEl.textContent = `Dimension insuffisante CNB 9.8.6.3: min. ${metricToImperial(minDimensionRequired)}`;
    }
    }
    isValid = false;
   }
   if (uThirdFlightRunValue > 0 && uThirdFlightRunValue < minDimensionRequired) {
    const errEl = document.getElementById('uThirdFlightRunError');
    if (errEl) {
    if (isMetric) {
     errEl.textContent = `Dimension insuffisante CNB 9.8.6.3: min. ${minDimensionRequired.toFixed(0)} mm (zone ${stairWidthValue.toFixed(0)} mm + giron min.)`;
    } else {
  ...
  if (buildingTypeValue === 'part3') {
   contentDiv.innerHTML = '<h3>âÅ“â€” Configuration non permise</h3>' +
