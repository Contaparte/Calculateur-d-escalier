...
const CNB_LIMITS = {
 ...
 // =====================================================================
 // ESCALIERS HÉLICOà à‚ÂDAUX (CNB 2020, Article 9.8.4.7)
 // =====================================================================
 spiral: {
...
function checkStepRule(riserMm, treadMm) {
 ...
 // Règle 2: G à— H = 71 à  74 poÂ² (458-477 cmÂ²)
...
function calculateUShapedStair(params) {
 ...
 let minRequiredWidth = 2 * stairWidth; // Minimum: 2 à— largeur escalier (volées côte à  côte)
 let flight2RunError = null;
 
 if (uShapedConfig === 'rect_landing_rect' || uShapedConfig === 'two_landings') {
  // Palier seul: largeur min = 2 à— stairWidth
  minRequiredWidth = 2 * stairWidth;
 } else if (uShapedConfig === 'rect_landing_radiating_rect' || uShapedConfig === 'rect_radiating_landing_rect') {
  // 1 coin rayonnant + palier: largeur min = 2 à— stairWidth (coin = stairWidth, palier se partage l'espace)
  minRequiredWidth = 2 * stairWidth;
 } else if (uShapedConfig === 'rect_radiating_landing_radiating_rect') {
  // 2 coins rayonnants + palier: largeur min = 2 à— stairWidth + landingLength (profondeur palier minimum)
 ...
 // Configuration avec DEUX séries de marches rayonnantes (2 à— 90°)
 else if (uShapedConfig === 'rect_radiating_landing_radiating_rect') {
...
function calculateDancingSteps(params) {
 ...
 // Le giron est mesuré à 300mm de l'axe de la main courante côté intérieur
 ...
 // Arc à 300mm de l'axe = giron effectif pour chaque marche dansante
 const treadAt300 = measurementRadius * anglePerStepRad;
 
 // Giron à l'extrémité étroite (sur le rayon intérieur)
 const treadAtNarrow = innerRadius * anglePerStepRad;
 
 // Vérification CNB 9.8.4.3: giron min 150mm à l'extrémité étroite
 if (treadAtNarrow < 150) {
  return {
   error: true,
   errorMessage: `Giron à l'extrémité étroite (${treadAtNarrow.toFixed(0)} mm) inférieur au minimum CNB de 150 mm. Augmentez le rayon intérieur ou réduisez le nombre de marches dansantes.`,
   solutions: []
  };
 }
 
 // Vérification CNB 9.8.4.3: giron à 300mm conforme au tableau 9.8.4.2
 if (treadAt300 < limits.minTread) {
  return {
   error: true,
   errorMessage: `Giron à 300 mm de l'axe (${treadAt300.toFixed(0)} mm) inférieur au minimum CNB de ${limits.minTread} mm. Augmentez le rayon intérieur ou réduisez le nombre de marches dansantes.`,
   solutions: []
  };
 }
 
 // Calcul de l'espace disponible pour les marches rectangulaires
 // Pour type L: firstFlightRun + secondFlightRun (moins les zones de virage)
 // Pour type U: firstFlightRun + secondFlightRun + thirdFlightRun (moins les zones de virage)
 
 // Espace occupé par chaque zone de virage (approximation basée sur la largeur de l'escalier)
 ...
 if (totalRectRun <= 0) {
  return {
   error: true,
   errorMessage: `Dimensions insuffisantes pour les marches rectangulaires. Augmentez les longueurs des parties ou réduisez la largeur de l'escalier.`,
   solutions: []
  };
 }
 ...
 for (let numRisers = minRisers; numRisers <= maxRisers; numRisers++) {
  ...
  // Nombre total de girons = contremarches - 1 (le dernier giron = plancher supérieur)
  const numTreads = numRisers - 1;
  
  // Nombre de girons rectangulaires
  const numRectTreads = numTreads - totalDancingSteps;
  
  if (numRectTreads < 2) continue;
  
  // Giron moyen pour les marches rectangulaires
  const rectTreadDepth = totalRectRun / numRectTreads;
  
  if (rectTreadDepth < limits.minTread || rectTreadDepth > limits.maxTread) continue;
  
  // CNB 9.8.4.5: Uniformité des girons dans volées mixtes
  // Le giron à 300mm des marches dansantes doit être proche du giron rectangulaire
  const treadVariation = Math.abs(treadAt300 - rectTreadDepth);
  if (treadVariation > 25) continue; // Tolérance CNB
  ...
  if (isUConfig) {
   // Configuration U: 3 volées
   const run1Adj = firstFlightRun - turnZoneSpace;
   const run2Adj = secondFlightRun;
   const run3Adj = thirdFlightRun - turnZoneSpace;
   ...
   // Configuration L: 2 volées
   const run1Adj = firstFlightRun;
   const run2Adj = secondFlightRun - turnZoneSpace;
 ...
 if (solutions.length === 0) {
  return {
   error: true,
   errorMessage: `Aucune solution conforme CNB trouvée. Essayez d'ajuster les dimensions ou le nombre de marches dansantes.`,
   solutions: []
  };
 }
...
function generateStraightStairVisualization(stairData) {
 ...
 // Valeurs
 svg += '<text x="' + (legendX + cellW * 0.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + numRisers + ' à— ' + riserText + '</text>';
 svg += '<text x="' + (legendX + cellW * 1.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + numTreads + ' à— ' + treadText + '</text>';
...
function generateLShapedStairVisualization(stairData) {
 ...
 if (isLanding) {
  ...
  if (numRadSteps === 2) {
   ...
   svg += '<text x="' + r(cornerX + L/2 - 5) + '" y="' + r(cornerY + L/2 + 14) + '" style="font:bold 10px Arial;fill:#e65100;" text-anchor="middle">3à—30°</text>';
  }
 ...
 if (isLanding) {
  ...
  svg += '<text x="' + (legendX + cellW * 4.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + numRadSteps + 'à—' + (numRadSteps === 2 ? '45°' : '30°') + '</text>';
 }
...
function generateUShapedStairVisualization(stairData) {
 ...
 // IMPORTANT: Les dimensions des volées sont calculées à  partir du nombre de girons à— profondeur uniforme
 ...
 if (flight1Treads > 0) {
  for (let i = 1; i <= flight1Treads; i++) {
  // Position Y: du bas vers le haut, chaque giron a la Mà…Â ME profondeur que dans volée 2
 ...
 if (flight2Treads > 0) {
  ...
  // Annotation nombre de girons rectangulaires volée 2 - Mà…Â ME DIRECTION que volée 1 (rotation -90)
...
function generateUShapedRadiatingVisualization(stairData) {
 ...
 // Donc TOUS les girons rectangulaires ont la Mà…Â ME profondeur (treadDepth calculé globalement)
 ...
 if (flight1RectTreadsCalc > 0) {
  // Hauteur du rectangle = nombre de girons à— giron uniforme (pour cohérence visuelle)
 ...
 if (flight2RectTreadsCalc > 0) {
  // Hauteur du rectangle = nombre de girons à— giron uniforme
 ...
 const dimX1 = r(flight1X - 30);
 // Afficher la dimension réelle (giron uniforme à— nombre de girons + largeur)
...
function generateUShapedMixedVisualization(stairData) {
 ...
 // Espace entre volées = largeur totale - (2 à— largeur escalier)
...
function generateUShapedDoubleRadiatingVisualization(stairData) {
    ...
    // ===== PALIER CENTRAL =====
    // Le palier occupe l'espace entre les deux coins: flight2Run - (2 à— stairWidth)
...
function displayCalculatorResults(solutions, params) {
 ...
 if (!solutions || solutions.length === 0) {
  ...
  if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
   ...
   if (treadTooSmall) {
    if (isMetric) {
    html += `<li>Longueur totale minimale requise ≥ ${minLengthWithLanding.toFixed(0)} mm (${minTreads} girons à— ${limits.minTread} mm + 2× palier)</li>`;
    } else {
    html += `<li>Longueur totale minimale requise ≥ ${metricToImperial(minLengthWithLanding)} (${minTreads} girons à— ${metricToImperial(limits.minTread)} + 2× palier)</li>`;
    }
   ...
   if (treadTooSmall) {
    if (isMetric) {
    html += `<li>Longueur totale minimale requise ≥ ${minLength.toFixed(0)} mm (${minTreads} girons à— ${limits.minTread} mm)</li>`;
    } else {
    html += `<li>Longueur totale minimale requise ≥ ${metricToImperial(minLength)} (${minTreads} girons à— ${metricToImperial(limits.minTread)})</li>`;
    }
   ...
   if (treadTooSmall) {
    if (isMetric) {
    html += `<li>Longueur minimale requise ≥ ${minLength.toFixed(0)} mm (${minTreads} girons à— ${limits.minTread} mm)</li>`;
    } else {
    html += `<li>Longueur minimale requise ≥ ${metricToImperial(minLength)} (${minTreads} girons à— ${metricToImperial(limits.minTread)})</li>`;
    }
 ...
 html += '<ul>';
 html += `<li><strong>Contremarches :</strong> ${best.numRisers} à— ${formatValuePrecise(best.riserHeight, isMetric)}</li>`;
 
 // Pour les configurations avec marches rayonnantes, afficher distinctement
 if (best.totalRadiatingSteps > 0) {
  html += `<li><strong>Girons rectangulaires :</strong> ${best.numRectTreads} à— ${formatValuePrecise(best.treadDepth, isMetric)}</li>`;
  // Construire le descriptif des marches rayonnantes selon le nombre de séries
  let radiatingDesc = '';
  if (best.numRadiatingSeries === 1) {
   // 1 série de 90°: "2×45°" ou "3à—30°"
   radiatingDesc = best.radiatingAngle === 45 ? '2×45°' : '3à—30°';
  } else {
   // 2 séries de 90°: "2×2×45°" ou "2×3à—30°"
   radiatingDesc = best.radiatingAngle === 45 ? '2×2×45°' : '2×3à—30°';
  }
  ...
  html += `<li><strong>Girons :</strong> ${best.numTreads} à— ${formatValuePrecise(best.treadDepth, isMetric)}</li>`;
 ...
 if (isMetric) {
  ...
  html += `${best.numRisers} à— ${best.riserHeight.toFixed(4)} mm = ${totalRiseCalc.toFixed(4)} mm `;
  ...
  html += `${best.numTreads} à— ${best.treadDepth.toFixed(4)} mm = ${totalTreadCalc.toFixed(4)} mm `;
  ...
  html += `${best.numRisers} à— ${riserIn.toFixed(6)}" = ${totalRiseIn.toFixed(6)}" âœ“`;
 }
 ...
 html += `<li>${best.stepRule.rule2.isValid ? 'âœ“' : '✗'} G à— H = ${best.stepRule.rule2.value.toFixed(2)} (${best.stepRule.rule2.range})</li>`;
 ...
 if (stairConfigValue === 'l_shaped' && (lShapedConfigValue === 'two_45deg' || lShapedConfigValue === 'three_30deg')) {
  ...
  html += '<li> â†’  1ère direction : ' + best.firstFlightRectTreads + ' girons à— ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
  ...
  html += '<li> â†’  2ème direction : ' + best.secondFlightRectTreads + ' girons à— ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
...
function displayVerificationResults(params) {
 ...
 html += `<li>${stepRule.rule2.isValid ? 'âœ“' : 'âœ“'} Règle 2 (Gà—H) : ${stepRule.rule2.value.toFixed(2)} poÂ² (71-74)</li>`;
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 function performCalculation() {
  ...
  // Validation: Largeur palier ≥ 2 à— largeur escalier
  ...
  * Règles:
  * - Largeur palier ≥ 2 à— largeur escalier (géométrie)
 ...
 function validateUShapedInputs() {
  ...
  // Valider: Largeur palier ≥ 2 à— largeur escalier
