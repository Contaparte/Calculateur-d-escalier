...
function generateStraightStairVisualization(stairData) {
    ...
    // Dimensions du SVG (responsive avec viewBox)
    const W = 900, H = 340;
    const midX = 420; // Separation entre les deux vues (plus d'espace)
    const gap = 60;   // Espace entre les vues
    ...
    // SVG responsive: width 100%, height auto, viewBox pour le ratio
    ...
    const P = { left: 50, right: 40, top: 50, bottom: 70 };
    ...
    svg += '<rect x="' + (P.left - 10) + '" y="8" width="' + (profW + 60) + '" height="24" fill="#e8f5e9" rx="3"/>';
    svg += '<text x="' + r((P.left + midX - gap) / 2) + '" y="25" style="font:bold 12px Arial;fill:#2e7d32;" text-anchor="middle">VUE DE PROFIL</text>';
    ...
    svg += '<line x1="' + (endX_prof - 5) + '" y1="' + endY_prof + '" x2="' + r(endX_prof + 40) + '" y2="' + endY_prof + '" stroke="#aaa" stroke-width="1" stroke-dasharray="4,2"/>';
    svg += '<text x="' + r(endX_prof + 45) + '" y="' + (endY_prof + 4) + '" style="font:10px Arial;fill:#666;">Arrivee</text>';
    
    // Cotation horizontale (en bas)
    const dimY_prof = startY + 25;
    ...
    svg += '<text x="' + r(startX + stairW_prof/2) + '" y="' + (dimY_prof + 16) + '" style="font:bold 11px Arial;fill:#333;" text-anchor="middle">' + runText + '</text>';
    svg += '<text x="' + r(startX + stairW_prof/2) + '" y="' + (dimY_prof + 30) + '" style="font:10px Arial;fill:#666;" text-anchor="middle">(' + numTreads + ' girons)</text>';
    
    // Cotation verticale (a droite)
    const dimX_prof = endX_prof + 30;
    ...
    svg += '<text x="' + (dimX_prof + 8) + '" y="' + (midY_prof - 6) + '" style="font:bold 11px Arial;fill:#333;">' + riseText + '</text>';
    svg += '<text x="' + (dimX_prof + 8) + '" y="' + (midY_prof + 10) + '" style="font:10px Arial;fill:#666;">(' + numRisers + ' CM)</text>';
    
    // ========== LIGNE DE SEPARATION ==========
    svg += '<line x1="' + midX + '" y1="40" x2="' + midX + '" y2="' + (H - 20) + '" stroke="#ccc" stroke-width="1.5" stroke-dasharray="8,4"/>';
    
    // ========== VUE EN PLAN (droite) ==========
    const planLeft = midX + gap;
    const planRight = W - 55;
    const planTop = 50;
    const planBottom = H - 45;
    ...
    const scalePlan = Math.min(planW / totalRun, planH / actualWidth) * 0.70;
    ...
    svg += '<rect x="' + (planLeft - 10) + '" y="8" width="' + (planW + 30) + '" height="24" fill="#e8f5e9" rx="3"/>';
    svg += '<text x="' + r((planLeft + planRight) / 2) + '" y="25" style="font:bold 12px Arial;fill:#2e7d32;" text-anchor="middle">VUE EN PLAN</text>';
    ...
    const dimY_plan = r(planStartY + stairH_plan + 22);
    ...
    svg += '<text x="' + r(planStartX + stairW_plan/2) + '" y="' + (dimY_plan + 16) + '" style="font:bold 11px Arial;fill:#333;" text-anchor="middle">' + runText + '</text>';
    ...
    svg += '<text x="' + (dimX_plan + 8) + '" y="' + (midY_plan + 4) + '" style="font:bold 11px Arial;fill:#333;">' + widthText + '</text>';
    
    // Legende en bas a gauche
    svg += '<text x="15" y="' + (H - 10) + '" style="font:10px Arial;fill:#555;">Legende: CM (contremarche) = ' + riserText + '  |  G (giron) = ' + treadText + '</text>';
