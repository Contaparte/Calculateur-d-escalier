...
function generateUShapedRadiatingVisualization(stairData) {
    ...
    // ===== LIGNES RAYONNANTES - PIVOT AU CENTRE =====
    // Le pivot est au point d'intersection central (bas des deux coins, au milieu)
    const pivotX = r(corner1X + stairWidth_plan); // Coin droit du carré gauche = coin gauche du carré droit
    const pivotY = r(corner1Y + stairWidth_plan); // Bas des carrés
    
    // COIN 1 (gauche): Lignes allant du centre vers le haut-gauche (angles de 90° à 180°)
    for (let i = 1; i <= stepsPerCorner; i++) {
        // Angle: de 90° (vers le haut) à 180° (vers la gauche)
        const angle = (Math.PI / 2) + (Math.PI / 2) * (i / stepsPerCorner);
        const x2 = r(pivotX + Math.cos(angle) * stairWidth_plan);
        const y2 = r(pivotY + Math.sin(angle) * stairWidth_plan);
        svg += '<line x1="' + pivotX + '" y1="' + pivotY + '" x2="' + x2 + '" y2="' + y2 + '" stroke="#e65100" stroke-width="1.5"/>';
    }
    
    // COIN 2 (droite): Lignes allant du centre vers le haut-droite (angles de 0° à 90°)
    for (let i = 0; i < stepsPerCorner; i++) {
        // Angle: de 0° (vers la droite) à 90° (vers le haut)
        const angle = (Math.PI / 2) * (i / stepsPerCorner);
        const x2 = r(pivotX + Math.cos(angle) * stairWidth_plan);
        const y2 = r(pivotY - Math.sin(angle) * stairWidth_plan); // -sin car Y inversé en SVG
        svg += '<line x1="' + pivotX + '" y1="' + pivotY + '" x2="' + x2 + '" y2="' + y2 + '" stroke="#e65100" stroke-width="1.5"/>';
    }
