...
function calculateUShapedStair(params) {
    ...
    if (hasRadiating) {
        ...
        // 45°: 2 marches par coin × 2 coins = 4 marches
        // 30°: 3 marches par coin × 2 coins = 6 marches
...
function generateUShapedMixedVisualization(stairData) {
    ...
    if (configType === 'rect_landing_radiating_rect') {
        ...
        svg += '<text x="' + r(landingX + stairWidth_plan/2) + '" y="' + r(landingY + landingDepth_plan * 0.35) + '" style="font:italic 10px Arial;fill:#f57f17;" text-anchor="middle">Palier</text>';
        ...
        // Flèche de direction avec virages à 90°
        const arrowX1 = r(flight1X + stairWidth_plan/2);
        const arrowX2 = r(flight2X + stairWidth_plan/2);
        const arrowY_landing = r(landingY + landingDepth_plan * 0.65);
        const arrowY_corner = r(cornerY + stairWidth_plan * 0.65);
        
        // Segment 1: Monte dans volée 1 (vertical)
        svg += '<line x1="' + arrowX1 + '" y1="' + r(flight1Y + flight1Run_plan) + '" x2="' + arrowX1 + '" y2="' + arrowY_landing + '" stroke="#1b5e20" stroke-width="1.5"/>';
        // Segment 2: Traverse le palier (horizontal, virage 90°)
        svg += '<line x1="' + arrowX1 + '" y1="' + arrowY_landing + '" x2="' + r(cornerX) + '" y2="' + arrowY_corner + '" stroke="#1b5e20" stroke-width="1.5"/>';
        // Segment 3: Traverse le coin rayonnant (horizontal)
        svg += '<line x1="' + r(cornerX) + '" y1="' + arrowY_corner + '" x2="' + arrowX2 + '" y2="' + arrowY_corner + '" stroke="#1b5e20" stroke-width="1.5"/>';
        // Segment 4: Descend dans volée 2 (vertical, virage 90°)
        svg += '<line x1="' + arrowX2 + '" y1="' + arrowY_corner + '" x2="' + arrowX2 + '" y2="' + r(flight2Y + flight3Run_plan) + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrUMMontee)"/>';
        ...
        // Segment 1: Monte dans volée 1 (vertical)
        svg += '<line x1="' + arrowX1 + '" y1="' + r(flight1Y + flight1Run_plan) + '" x2="' + arrowX1 + '" y2="' + arrowY_corner + '" stroke="#1b5e20" stroke-width="1.5"/>';
        // Segment 2: Traverse le coin rayonnant (horizontal, virage 90°)
        svg += '<line x1="' + arrowX1 + '" y1="' + arrowY_corner + '" x2="' + r(landingX) + '" y2="' + arrowY_corner + '" stroke="#1b5e20" stroke-width="1.5"/>';
        // Segment 3: Traverse le palier (horizontal)
        svg += '<line x1="' + r(landingX) + '" y1="' + arrowY_landing + '" x2="' + arrowX2 + '" y2="' + arrowY_landing + '" stroke="#1b5e20" stroke-width="1.5"/>';
        // Segment 4: Descend dans volée 2 (vertical, virage 90°)
        svg += '<line x1="' + arrowX2 + '" y1="' + arrowY_landing + '" x2="' + arrowX2 + '" y2="' + r(flight2Y + flight3Run_plan) + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrUMMontee)"/>';
        svg += '<text x="' + arrowX1 + '" y="' + r(flight1Y + flight1Run_plan + 14) + '" style="font:bold 9px Arial;fill:#1b5e20;" text-anchor="middle">En haut</text>';
    }
...
/**
 * Génère la visualisation SVG pour un escalier en U avec configuration:
 * Marche(s) rectangulaire(s) + marches rayonnantes + palier + marches rayonnantes + marche(s) rectangulaire(s)
 * 
 * Conforme au CNB 9.8.4.6: marches rayonnantes à 30° ou 45°, max 90° par série
 * Pour un virage de 180°, deux séries de 90° sont nécessaires avec un palier entre les deux
 */
function generateUShapedDoubleRadiatingVisualization(stairData) {
    ...
    // Nombre de marches rayonnantes: 2 séries de 90° chacune
    const stepsPerCorner = radiatingAngle === 30 ? 3 : 2;
    const totalRadiatingSteps = stepsPerCorner * 2; // 4 marches (45°) ou 6 marches (30°)
    ...
    let flight1Treads, flight2Treads;
    if (totalRectRun > 0 && numRectTreads > 0) {
        const ratio1 = actualFlight1Run / totalRectRun;
        flight1Treads = Math.max(1, Math.round(numRectTreads * ratio1));
        flight2Treads = Math.max(1, numRectTreads - flight1Treads);
    } else {
        flight1Treads = Math.ceil(numRectTreads / 2);
        flight2Treads = numRectTreads - flight1Treads;
    }
    ...
    // ========== VUE EN PLAN ==========
    const margin = { left: 70, right: 70, top: 55, bottom: 95 };
    const planW = W - margin.left - margin.right;
    const planH = H - margin.top - margin.bottom;
    ...
    const totalPlanWidth = actualLandingWidth;
    const maxFlightRun = Math.max(actualFlight1Run, actualFlight2Run);
    const totalPlanHeight = maxFlightRun + actualWidth + actualLandingDepth;
    ...
    // ===== ZONE RAYONNANTE 1 (coin gauche, 90°) =====
    const corner1X = planStartX;
    const corner1Y = planStartY;
    svg += '<rect x="' + corner1X + '" y="' + corner1Y + '" width="' + stairWidth_plan + '" height="' + stairWidth_plan + '" fill="#ffecb3" stroke="#f9a825" stroke-width="2"/>';
    
    // Pivot au coin intérieur droit-bas
    const pivot1X = r(corner1X + stairWidth_plan);
    const pivot1Y = r(corner1Y + stairWidth_plan);
    
    // Lignes rayonnantes coin 1: de 180° (bas-gauche) à 90° (haut)
    for (let i = 0; i <= stepsPerCorner; i++) {
        const angle = Math.PI - (i * (Math.PI / 2) / stepsPerCorner);
        let x2, y2;
    ...
    // Étiquette coin 1
    svg += '<text x="' + r(corner1X + stairWidth_plan * 0.35) + '" y="' + r(corner1Y + stairWidth_plan * 0.4) + '" style="font:bold 8px Arial;fill:#e65100;" text-anchor="middle">' + stepsPerCorner + String.fromCharCode(215) + radiatingAngle + String.fromCharCode(176) + '</text>';
    svg += '<text x="' + r(corner1X + stairWidth_plan * 0.35) + '" y="' + r(corner1Y + stairWidth_plan * 0.6) + '" style="font:8px Arial;fill:#e65100;" text-anchor="middle">' + currentStep + '-' + (currentStep + stepsPerCorner - 1) + '</text>';
    currentStep += stepsPerCorner;
    ...
    // Position du texte "Palier" ajustée pour éviter conflit avec la flèche
    svg += '<text x="' + r(landingX + landingDepth_plan/2) + '" y="' + r(landingY + stairWidth_plan * 0.3) + '" style="font:italic 10px Arial;fill:#f57f17;" text-anchor="middle">Palier</text>';
    
    // ===== ZONE RAYONNANTE 2 (coin droit, 90°) =====
    const corner2X = r(landingX + landingDepth_plan);
    const corner2Y = planStartY;
    svg += '<rect x="' + corner2X + '" y="' + corner2Y + '" width="' + stairWidth_plan + '" height="' + stairWidth_plan + '" fill="#ffecb3" stroke="#f9a825" stroke-width="2"/>';
    
    // Pivot au coin intérieur gauche-bas
    const pivot2X = corner2X;
    const pivot2Y = r(corner2Y + stairWidth_plan);
    
    // Lignes rayonnantes coin 2: de 90° (haut) à 0° (droite)
    for (let i = 0; i <= stepsPerCorner; i++) {
        const angle = Math.PI / 2 - (i * (Math.PI / 2) / stepsPerCorner);
        let x2, y2;
    ...
    // Étiquette coin 2
    svg += '<text x="' + r(corner2X + stairWidth_plan * 0.65) + '" y="' + r(corner2Y + stairWidth_plan * 0.4) + '" style="font:bold 8px Arial;fill:#e65100;" text-anchor="middle">' + stepsPerCorner + String.fromCharCode(215) + radiatingAngle + String.fromCharCode(176) + '</text>';
    svg += '<text x="' + r(corner2X + stairWidth_plan * 0.65) + '" y="' + r(corner2Y + stairWidth_plan * 0.6) + '" style="font:8px Arial;fill:#e65100;" text-anchor="middle">' + currentStep + '-' + (currentStep + stepsPerCorner - 1) + '</text>';
    currentStep += stepsPerCorner;
    ...
    if (flight2Treads > 0) {
        ...
        // Numérotation
        svg += '<text x="' + r(flight2X + stairWidth_plan * 0.75) + '" y="' + r(flight2Y + treadDepth_plan/2 + 3) + '" style="font:9px Arial;fill:#333;font-weight:bold;" text-anchor="middle">' + currentStep + '</text>';
        svg += '<text x="' + r(flight2X + stairWidth_plan * 0.75) + '" y="' + r(flight2Y + flight2Run_plan - treadDepth_plan/2 + 3) + '" style="font:9px Arial;fill:#333;font-weight:bold;" text-anchor="middle">' + numTreads + '</text>';
    }
    
    // ===== FLÈCHE DE DIRECTION (tournant à 90° à chaque coin) =====
    const arrowX1 = r(flight1X + stairWidth_plan * 0.5);  // Centre volée 1
    const arrowX2 = r(flight2X + stairWidth_plan * 0.5);  // Centre volée 2
    
    // Segment 1: Montée volée 1 (vertical vers le haut)
    const seg1StartY = r(flight1Y + flight1Run_plan);
    const seg1EndY = r(corner1Y + stairWidth_plan * 0.7);
    svg += '<line x1="' + arrowX1 + '" y1="' + seg1StartY + '" x2="' + arrowX1 + '" y2="' + seg1EndY + '" stroke="#1b5e20" stroke-width="1.5"/>';
    
    // Segment 2: Traversée coin 1 (virage 90° vers la droite - horizontal)
    const seg2EndX = r(landingX + landingDepth_plan * 0.15);
    const seg2Y = r(corner1Y + stairWidth_plan * 0.7);
    svg += '<line x1="' + arrowX1 + '" y1="' + seg2Y + '" x2="' + seg2EndX + '" y2="' + seg2Y + '" stroke="#1b5e20" stroke-width="1.5"/>';
    
    // Segment 3: Traversée palier (horizontal)
    const seg3StartX = seg2EndX;
    const seg3EndX = r(landingX + landingDepth_plan * 0.85);
    const seg3Y = r(landingY + stairWidth_plan * 0.7);
    svg += '<line x1="' + seg3StartX + '" y1="' + seg3Y + '" x2="' + seg3EndX + '" y2="' + seg3Y + '" stroke="#1b5e20" stroke-width="1.5"/>';
    
    // Segment 4: Traversée coin 2 (virage 90° vers le bas - horizontal puis vertical)
    const seg4StartX = seg3EndX;
    const seg4EndX = arrowX2;
    svg += '<line x1="' + seg4StartX + '" y1="' + seg3Y + '" x2="' + seg4EndX + '" y2="' + seg3Y + '" stroke="#1b5e20" stroke-width="1.5"/>';
    
    // Segment 5: Descente volée 2 (vertical vers le bas avec flèche)
    const seg5StartY = seg3Y;
    const seg5EndY = r(flight2Y + flight2Run_plan);
    svg += '<line x1="' + arrowX2 + '" y1="' + seg5StartY + '" x2="' + arrowX2 + '" y2="' + seg5EndY + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrUDRMontee)"/>';
    
    // Label "En haut"
    svg += '<text x="' + arrowX1 + '" y="' + r(seg1StartY + 14) + '" style="font:bold 9px Arial;fill:#1b5e20;" text-anchor="middle">En haut</text>';
    ...
    svg += '<text x="' + (legendX + cellW * 4.5) + '" y="' + (legendY + 12) + '" style="font:bold 7px Arial;fill:#333;" text-anchor="middle">Ray. coin 1</text>';
    svg += '<text x="' + (legendX + cellW * 5.5) + '" y="' + (legendY + 12) + '" style="font:bold 7px Arial;fill:#333;" text-anchor="middle">Ray. coin 2</text>';
    ...
    svg += '<text x="' + (legendX + cellW * 4.5) + '" y="' + (legendY + cellH + 14) + '" style="font:8px Arial;fill:#e65100;" text-anchor="middle">' + stepsPerCorner + String.fromCharCode(215) + radiatingAngle + String.fromCharCode(176) + '</text>';
    svg += '<text x="' + (legendX + cellW * 5.5) + '" y="' + (legendY + cellH + 14) + '" style="font:8px Arial;fill:#e65100;" text-anchor="middle">' + stepsPerCorner + String.fromCharCode(215) + radiatingAngle + String.fromCharCode(176) + '</text>';
...
function displayCalculatorResults(solutions, params) {
    ...
    if (best.totalRadiatingSteps > 0) {
        ...
        html += `<li><strong>Marches rayonnantes :</strong> ${best.totalRadiatingSteps} (${best.totalRadiatingSteps === 4 ? '2×45°' : '2×3×30°'})</li>`;
    ...
    if (isCompliant && stairConfigValue === 'u_shaped' && uShapedIsMixed) {
        ...
        if (uShapedConfigValue === 'rect_landing_radiating_rect' || uShapedConfigValue === 'rect_radiating_landing_rect') {
            ...
            // Pour rect_radiating_landing_radiating_rect
