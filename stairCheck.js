...
/**
 * Génère une visualisation SVG améliorée d'un escalier en L avec marches rayonnantes
 * Style similaire aux tracés CAD
 * @param {Object} stairData - Données de l'escalier
 * @returns {string} SVG HTML
 */
function generateLShapedVisualization(stairData) {
    const {
        firstFlightRun,
        secondFlightRun,
        firstFlightRectTreads,
    ...
    const w1 = isMetric ? firstFlightRun : firstFlightRun * 25.4;
    const w2 = isMetric ? secondFlightRun : secondFlightRun * 25.4;
    const width = isMetric ? stairWidth : stairWidth * 25.4;
    const tread = isMetric ? treadDepth : treadDepth * 25.4;
    
    // Configuration SVG - dimensions fixes
    const svgWidth = 800;
    const svgHeight = 600;
    const padding = 100;
    
    // Calculer l'échelle pour que tout rentre
    const maxDimX = w1 + tread + width + padding * 2;
    const maxDimY = w2 + tread + width + padding * 2;
    const scaleX = (svgWidth - padding * 2) / maxDimX;
    const scaleY = (svgHeight - padding * 2) / maxDimY;
    const scale = Math.min(scaleX, scaleY, 0.8); // Max 0.8 pour laisser de l'espace
    
    // Point de départ (bas gauche)
    const startX = padding;
    const startY = svgHeight - padding;
    
    let svg = `<svg width="${svgWidth}" height="${svgHeight}" xmlns="http://www.w3.org/2000/svg" style="border: 1px solid #ccc; background: #2d2d2d; border-radius: 4px;">`;
    
    // Définitions de styles (couleurs type CAD)
    svg += `<defs>
        <style>
            .tread-line { stroke: #ff4444; stroke-width: 2; }
            .riser-line { stroke: #ff4444; stroke-width: 1; }
            .radiating-line { stroke: #ffaa00; stroke-width: 3; }
            .radiating-arc { stroke: #ffaa00; stroke-width: 2; fill: none; stroke-dasharray: 3,3; }
            .dimension-line { stroke: #888; stroke-width: 1; }
            .dimension-arrow { stroke: #888; stroke-width: 1; fill: #888; }
            .dimension-text { font-family: 'Courier New', monospace; font-size: 13px; fill: #fff; font-weight: bold; }
            .dimension-value { font-family: 'Courier New', monospace; font-size: 15px; fill: #ffff00; font-weight: bold; }
            .outer-rect { stroke: #ff4444; stroke-width: 3; fill: none; }
            .grid-line { stroke: #444; stroke-width: 0.5; }
            .label-text { font-family: Arial; font-size: 11px; fill: #aaa; }
        </style>
    ...
    for (let i = 0; i < svgWidth; i += 50) {
        svg += `<line x1="${i}" y1="0" x2="${i}" y2="${svgHeight}" class="grid-line"/>`;
    }
    for (let i = 0; i < svgHeight; i += 50) {
        svg += `<line x1="0" y1="${i}" x2="${svgWidth}" y2="${i}" class="grid-line"/>`;
    }
    svg += `</g>`;
    
    // Groupe principal avec échelle
    svg += `<g transform="scale(${scale}) translate(${padding/scale}, ${padding/scale})">`;
    
    // 1. Rectangle de la 1ère direction (horizontal)
    const rect1X = 0;
    const rect1Y = 0;
    const rect1W = firstFlightRectTreads * tread;
    const rect1H = width;
    
    svg += `<rect x="${rect1X}" y="${rect1Y}" width="${rect1W}" height="${rect1H}" class="outer-rect"/>`;
    
    // Dessiner les girons (contremarches) de la 1ère direction
    for (let i = 1; i < firstFlightRectTreads; i++) {
        const x = rect1X + i * tread;
        svg += `<line x1="${x}" y1="${rect1Y}" x2="${x}" y2="${rect1Y + rect1H}" class="riser-line"/>`;
    }
    
    // 2. Marches rayonnantes (à l'angle)
    const radStartX = rect1W;
    const radStartY = 0;
    const radEndX = rect1W + tread;
    const radEndY = -tread;
    
    if (numRadiatingSteps === 2) {
        // 2 marches de 45°
        const midX = radStartX + tread * 0.707;
        const midY = radStartY - tread * 0.707;
        svg += `<line x1="${radStartX}" y1="${radStartY}" x2="${midX}" y2="${midY}" class="radiating-line"/>`;
        svg += `<line x1="${midX}" y1="${midY}" x2="${radEndX}" y2="${radEndY}" class="radiating-line"/>`;
        
        // Lignes de connexion
        svg += `<line x1="${radStartX}" y1="${radStartY + rect1H}" x2="${midX}" y2="${midY + rect1H}" class="radiating-line"/>`;
        svg += `<line x1="${midX}" y1="${midY + rect1H}" x2="${radEndX}" y2="${radEndY + rect1H}" class="radiating-line"/>`;
        
        // Arc pour montrer l'angle
        svg += `<path d="M ${radStartX + 30} ${radStartY} A 30 30 0 0 0 ${radStartX} ${radStartY - 30}" class="radiating-arc"/>`;
        
    } else if (numRadiatingSteps === 3) {
        // 3 marches de 30°
        const angle1 = 30 * Math.PI / 180;
        const angle2 = 60 * Math.PI / 180;
        
        const x1 = radStartX + tread * Math.cos(angle1);
        const y1 = radStartY - tread * Math.sin(angle1);
        const x2 = radStartX + tread * Math.cos(angle2);
        const y2 = radStartY - tread * Math.sin(angle2);
        
        svg += `<line x1="${radStartX}" y1="${radStartY}" x2="${x1}" y2="${y1}" class="radiating-line"/>`;
        svg += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" class="radiating-line"/>`;
        svg += `<line x1="${x2}" y1="${y2}" x2="${radEndX}" y2="${radEndY}" class="radiating-line"/>`;
        
        // Lignes de connexion (côté large)
        svg += `<line x1="${radStartX}" y1="${radStartY + rect1H}" x2="${x1}" y2="${y1 + rect1H}" class="radiating-line"/>`;
        svg += `<line x1="${x1}" y1="${y1 + rect1H}" x2="${x2}" y2="${y2 + rect1H}" class="radiating-line"/>`;
        svg += `<line x1="${x2}" y1="${y2 + rect1H}" x2="${radEndX}" y2="${radEndY + rect1H}" class="radiating-line"/>`;
        
        // Arcs pour montrer les angles (30°, 60°, 90°)
        svg += `<path d="M ${radStartX + 40} ${radStartY} A 40 40 0 0 0 ${radStartX + 40*Math.cos(angle1)} ${radStartY - 40*Math.sin(angle1)}" class="radiating-arc"/>`;
        svg += `<path d="M ${radStartX + 50*Math.cos(angle1)} ${radStartY - 50*Math.sin(angle1)} A 50 50 0 0 0 ${radStartX + 50*Math.cos(angle2)} ${radStartY - 50*Math.sin(angle2)}" class="radiating-arc"/>`;
        svg += `<path d="M ${radStartX + 60*Math.cos(angle2)} ${radStartY - 60*Math.sin(angle2)} A 60 60 0 0 0 ${radStartX} ${radStartY - 60}" class="radiating-arc"/>`;
        
        // Labels d'angles
        svg += `<text x="${radStartX + 50}" y="${radStartY - 10}" class="label-text" font-size="10">30°</text>`;
        svg += `<text x="${radStartX + 40}" y="${radStartY - 40}" class="label-text" font-size="10">60°</text>`;
        svg += `<text x="${radStartX + 20}" y="${radStartY - 70}" class="label-text" font-size="10">90°</text>`;
    }
    
    // 3. Rectangle de la 2ème direction (vertical)
    const rect2X = radEndX;
    const rect2Y = radEndY - secondFlightRectTreads * tread;
    const rect2W = width;
    const rect2H = secondFlightRectTreads * tread;
    
    svg += `<rect x="${rect2X}" y="${rect2Y}" width="${rect2W}" height="${rect2H}" class="outer-rect"/>`;
    
    // Dessiner les girons de la 2ème direction
    for (let i = 1; i < secondFlightRectTreads; i++) {
        const y = rect2Y + i * tread;
        svg += `<line x1="${rect2X}" y1="${y}" x2="${rect2X + rect2W}" y2="${y}" class="riser-line"/>`;
    }
    
    svg += `</g>`;
    
    // Dimensions (en dehors du groupe scalé pour rester lisibles)
    const dimScale = scale;
    const dimPadding = padding;
    
    // Dimension horizontale (1ère direction)
    const dimY1 = svgHeight - dimPadding/2;
    const dimX1Start = dimPadding + rect1X * dimScale;
    const dimX1End = dimPadding + rect1W * dimScale;
    
    svg += `<line x1="${dimX1Start}" y1="${dimY1}" x2="${dimX1End}" y2="${dimY1}" class="dimension-line" marker-end="url(#arrowhead)" marker-start="url(#arrowhead)"/>`;
    const dim1Text = formatValuePrecise(firstFlightRun, isMetric);
    svg += `<text x="${(dimX1Start + dimX1End)/2}" y="${dimY1 + 20}" class="dimension-value" text-anchor="middle">${dim1Text}</text>`;
    
    // Dimension verticale (2ème direction)
    const dimX2 = svgWidth - dimPadding/2;
    const dimY2Start = dimPadding + (radEndY - secondFlightRectTreads * tread) * dimScale;
    const dimY2End = dimPadding + radEndY * dimScale;
    
    svg += `<line x1="${dimX2}" y1="${dimY2Start}" x2="${dimX2}" y2="${dimY2End}" class="dimension-line" marker-end="url(#arrowhead)" marker-start="url(#arrowhead)"/>`;
    const dim2Text = formatValuePrecise(secondFlightRun, isMetric);
    svg += `<text x="${dimX2 + 15}" y="${(dimY2Start + dimY2End)/2 + 5}" class="dimension-value" text-anchor="start">${dim2Text}</text>`;
    
    // Titre et légende
    svg += `<g transform="translate(10, 20)">
        <text x="0" y="0" class="dimension-text" font-size="16">VUE EN PLAN - Escalier en L</text>
        <text x="0" y="20" class="label-text">Rouge: Girons rectangulaires (${firstFlightRectTreads} + ${secondFlightRectTreads} = ${firstFlightRectTreads + secondFlightRectTreads})</text>
        <text x="0" y="35" class="label-text">Orange: Marches rayonnantes (${numRadiatingSteps})</text>
        <text x="0" y="50" class="label-text">Giron uniforme: ${formatValuePrecise(treadDepth, isMetric)}</text>
    </g>`;
...
function displayCalculatorResults(solutions, params) {
    ...
    if (stairConfigValue === 'l_shaped' && (lShapedConfigValue === 'two_45deg' || lShapedConfigValue === 'three_30deg')) {
        ...
        // Visualisation graphique de l'escalier en L
        html += '<div class="result-section">';
        html += '<h4>Visualisation graphique</h4>';
        html += generateLShapedVisualization({
            ...
            stairWidth: params.stairWidth || (isMetric ? 914 : 36),
            isMetric: isMetric
        });
        html += '</div>';
    }
