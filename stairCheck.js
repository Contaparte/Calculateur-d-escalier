...
function calculateLShapedWithRadiating(params) {
    ...
    for (let numRisers = minRisers; numRisers <= maxRisers + 3; numRisers++) {
        ...
        // CORRECTION: Pour un escalier en L, répartir les girons entre les deux volées
        // Les marches rayonnantes occupent un espace diagonal à l'intersection
        // On estime qu'une marche rayonnante occupe environ 0.7 × giron dans chaque direction
        
        // Répartition proportionnelle des girons rectangulaires
        const ratio = firstFlightRun / totalRun;
        const firstFlightRectTreads = Math.round(numRectTreads * ratio);
        const secondFlightRectTreads = numRectTreads - firstFlightRectTreads;
        
        // Espace disponible pour les girons dans chaque volée
        // (en tenant compte de l'espace occupé par les marches rayonnantes)
        const radiatingSpace = numRadiatingSteps * radiatingFactor;
        const firstFlightAvailable = firstFlightRun - (radiatingSpace);
        const secondFlightAvailable = secondFlightRun - (radiatingSpace);
        
        // Calculer le giron nécessaire pour chaque volée
        const treadDepth1 = firstFlightRectTreads > 0 ? firstFlightAvailable / firstFlightRectTreads : 0;
        const treadDepth2 = secondFlightRectTreads > 0 ? secondFlightAvailable / secondFlightRectTreads : 0;
        
        // Utiliser la moyenne ou vérifier qu'ils sont similaires
        const treadDepth = (treadDepth1 + treadDepth2) / 2;
        const treadVariation = Math.abs(treadDepth1 - treadDepth2);
        
        // Si la variation est trop grande, cette solution n'est pas géométriquement valide
...
function displayCalculatorResults(solutions, params) {
    ...
    if (stairConfigValue === 'l_shaped' && (lShapedConfigValue === 'two_45deg' || lShapedConfigValue === 'three_30deg')) {
        html += '<div class="result-section">';
        html += '<h4>Notes - Escalier en L avec marches rayonnantes</h4>';
        html += '<ul>';
        html += '<li>Les marches rayonnantes se situent à l\'intersection des deux volées perpendiculaires</li>';
        html += '<li>Côté long 1ère volée : ' + formatValuePrecise(best.firstFlightRun, isMetric) + '</li>';
        html += '<li>Girons rectangulaires : ' + best.numRectTreads + ' total</li>';
        if (best.firstFlightRectTreads !== undefined) {
            html += '<li>  → 1ère volée : ' + best.firstFlightRectTreads + ' girons</li>';
            html += '<li>  → 2ème volée : ' + best.secondFlightRectTreads + ' girons</li>';
        }
        html += '<li>Côté long 2ème volée : ' + formatValuePrecise(best.secondFlightRun, isMetric) + '</li>';
