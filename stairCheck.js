...
function checkStepRule(riserMm, treadMm) {
 ...
 // Règle 2: G x H = 71 à  74 poÃ‚Â² (458-477 cmÃ‚Â²)
...
function calculateUShapedStair(params) {
 ...
 let minRequiredWidth = 2 * stairWidth; // Minimum: 2 x largeur escalier (volées côte à côte)
 let flight2RunError = null;
 
 if (uShapedConfig === 'rect_landing_rect' || uShapedConfig === 'two_landings') {
  // Palier seul: largeur min = 2 x stairWidth
  minRequiredWidth = 2 * stairWidth;
 } else if (uShapedConfig === 'rect_landing_radiating_rect' || uShapedConfig === 'rect_radiating_landing_rect') {
  // 1 coin rayonnant + palier: largeur min = 2 x stairWidth (coin = stairWidth, palier se partage l'espace)
  minRequiredWidth = 2 * stairWidth;
 } else if (uShapedConfig === 'rect_radiating_landing_radiating_rect') {
  // 2 coins rayonnants + palier: largeur min = 2 x stairWidth + landingLength (profondeur palier minimum)
 ...
 // Configuration avec DEUX séries de marches rayonnantes (2 x 90°)
 else if (uShapedConfig === 'rect_radiating_landing_radiating_rect') {
...
function generateStraightStairVisualization(stairData) {
 ...
 // Valeurs
 svg += '<text x="' + (legendX + cellW * 0.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + numRisers + ' x ' + riserText + '</text>';
 svg += '<text x="' + (legendX + cellW * 1.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + numTreads + ' x ' + treadText + '</text>';
...
function generateLShapedStairVisualization(stairData) {
 ...
 if (isLanding) {
  ...
  // Numéro du palier dans la séquence continue
  svg += '<text x="' + r(palierX + landingW_plan/2) + '" y="' + r(palierY + landingW_plan * 0.35 + 4) + '" style="font:bold 9px Arial;fill:#f57f17;" text-anchor="middle">' + (flight1Treads + 1) + '</text>';
  ...
  if (numRadSteps === 2) {
   ...
   svg += '<text x="' + r(cornerX + L/2 - 5) + '" y="' + r(cornerY + L/2 + 14) + '" style="font:bold 10px Arial;fill:#e65100;" text-anchor="middle">2x45°</text>';
  } else {
   ...
   svg += '<text x="' + r(cornerX + L/2 - 5) + '" y="' + r(cornerY + L/2 + 14) + '" style="font:bold 10px Arial;fill:#e65100;" text-anchor="middle">3x30°</text>';
  }
 ...
 if (isLanding) {
  ...
  svg += '<text x="' + (legendX + cellW * 4.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + numRadSteps + 'x' + (numRadSteps === 2 ? '45°' : '30°') + '</text>';
 }
...
function generateUShapedStairVisualization(stairData) {
 ...
 // IMPORTANT: Les dimensions des volées sont calculées à  partir du nombre de girons x profondeur uniforme
 ...
 // Numéro du palier dans la séquence continue
 svg += '<text x="' + r(landingX + landingW * 0.15) + '" y="' + r(landingY + landingH * 0.5 + 4) + '" style="font:bold 9px Arial;fill:#f57f17;" text-anchor="middle">' + (flight1Treads + 1) + '</text>';
...
function generateUShapedRadiatingVisualization(stairData) {
 ...
 if (flight1RectTreadsCalc > 0) {
  // Hauteur du rectangle = nombre de girons x giron uniforme (pour cohérence visuelle)
 ...
 if (flight2RectTreadsCalc > 0) {
  // Hauteur du rectangle = nombre de girons x giron uniforme
 ...
 const dimX1 = r(flight1X - 30);
 // Afficher la dimension réelle (giron uniforme x nombre de girons + largeur)
...
function generateUShapedMixedVisualization(stairData) {
 ...
 // Espace entre volées = largeur totale - (2 x largeur escalier)
...
function generateUShapedDoubleRadiatingVisualization(stairData) {
    ...
    // ===== PALIER CENTRAL =====
    // Le palier occupe l'espace entre les deux coins: flight2Run - (2 x stairWidth)
...
function displayCalculatorResults(solutions, params) {
 ...
 if (!solutions || solutions.length === 0) {
  ...
  if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
   ...
   if (treadTooSmall) {
    if (isMetric) {
    html += `<li>Longueur totale minimale requise >= ${minLengthWithLanding.toFixed(0)} mm (${minTreads} girons x ${limits.minTread} mm + 2x palier)</li>`;
    } else {
    html += `<li>Longueur totale minimale requise >= ${metricToImperial(minLengthWithLanding)} (${minTreads} girons x ${metricToImperial(limits.minTread)} + 2x palier)</li>`;
    }
   ...
   if (treadTooSmall) {
    if (isMetric) {
    html += `<li>Longueur totale minimale requise >= ${minLength.toFixed(0)} mm (${minTreads} girons x ${limits.minTread} mm)</li>`;
    } else {
    html += `<li>Longueur totale minimale requise >= ${metricToImperial(minLength)} (${minTreads} girons x ${metricToImperial(limits.minTread)})</li>`;
    }
   ...
   if (treadTooSmall) {
    if (isMetric) {
    html += `<li>Longueur minimale requise >= ${minLength.toFixed(0)} mm (${minTreads} girons x ${limits.minTread} mm)</li>`;
    } else {
    html += `<li>Longueur minimale requise >= ${metricToImperial(minLength)} (${minTreads} girons x ${metricToImperial(limits.minTread)})</li>`;
    }
 ...
 html += '<ul>';
 html += `<li><strong>Contremarches :</strong> ${best.numRisers} x ${formatValuePrecise(best.riserHeight, isMetric)}</li>`;
 
 // Pour les configurations avec marches rayonnantes, afficher distinctement
 if (best.totalRadiatingSteps > 0) {
  html += `<li><strong>Girons rectangulaires :</strong> ${best.numRectTreads} x ${formatValuePrecise(best.treadDepth, isMetric)}</li>`;
  // Construire le descriptif des marches rayonnantes selon le nombre de séries
  let radiatingDesc = '';
  if (best.numRadiatingSeries === 1) {
   // 1 série de 90°: "2x45°" ou "3x30°"
   radiatingDesc = best.radiatingAngle === 45 ? '2x45°' : '3x30°';
  } else {
   // 2 séries de 90°: "2x2x45°" ou "2x3x30°"
   radiatingDesc = best.radiatingAngle === 45 ? '2x2x45°' : '2x3x30°';
  }
  ...
  html += `<li><strong>Girons :</strong> ${best.numTreads} x ${formatValuePrecise(best.treadDepth, isMetric)}</li>`;
 ...
 if (isMetric) {
  ...
  html += `${best.numRisers} x ${best.riserHeight.toFixed(4)} mm = ${totalRiseCalc.toFixed(4)} mm `;
  ...
  html += `${best.numTreads} x ${best.treadDepth.toFixed(4)} mm = ${totalTreadCalc.toFixed(4)} mm `;
  ...
  html += `${best.numRisers} x ${riserIn.toFixed(6)}" = ${totalRiseIn.toFixed(6)}" âœ“`;
 }
 ...
 html += `<li>${best.stepRule.rule2.isValid ? 'âœ“' : 'â—‹'} G x H = ${best.stepRule.rule2.value.toFixed(2)} (${best.stepRule.rule2.range})</li>`;
 ...
 if (stairConfigValue === 'l_shaped' && (lShapedConfigValue === 'two_45deg' || lShapedConfigValue === 'three_30deg')) {
  ...
  html += '<li> â†³  1ère direction : ' + best.firstFlightRectTreads + ' girons x ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
  ...
  html += '<li> â†³  2ème direction : ' + best.secondFlightRectTreads + ' girons x ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
...
function displayVerificationResults(params) {
 ...
 html += `<li>${stepRule.rule2.isValid ? 'âœ“' : 'âœ—'} Règle 2 (GxH) : ${stepRule.rule2.value.toFixed(2)} poÃ‚Â² (71-74)</li>`;
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 function performCalculation() {
  ...
  if (stairConfigValue === 'l_shaped') {
   ...
   uLandingWidthValue = uLandingWidthEl ? (parseFloat(uLandingWidthEl.value) || 0) : 0;
   uLandingDepthValue = uLandingDepthEl ? (parseFloat(uLandingDepthEl.value) || 0) : 0;
  ...
  if (stairConfigValue === 'l_shaped') {
   ...
   uLandingWidthValue = uLandingWidthImpEl ? (imperialToMetric(validateImperialInput(uLandingWidthImpEl.value)) || 0) : 0;
   uLandingDepthValue = uLandingDepthImpEl ? (imperialToMetric(validateImperialInput(uLandingDepthImpEl.value)) || 0) : 0;
   uRadiatingAngleValue = uRadiatingAngleEl ? parseInt(uRadiatingAngleEl.value) : 45;
  ...
  // Validation: Largeur palier >= 2 x largeur escalier
  ...
  // Vérifier si la configuration a un palier
  const uConfigHasLanding = uShapedConfigValue && uShapedConfigValue.includes('landing');
  if (uConfigHasLanding) {
   if (uLandingDepthValue <= 0 || isNaN(uLandingDepthValue)) {
   // Palier requis mais non spécifié
   const errEl = document.getElementById('uLandingDepthError');
   if (errEl) {
    errEl.textContent = 'Veuillez entrer la profondeur du palier (min. = largeur escalier selon CNB 9.8.6.3).';
   }
   isValid = false;
   } else if (stairWidthValue > 0 && uLandingDepthValue < stairWidthValue) {
   // Palier trop petit
   const errEl = document.getElementById('uLandingDepthError');
   if (errEl) {
    if (isMetric) {
    ...
    errEl.textContent = `La profondeur du palier (${metricToImperial(uLandingDepthValue)}) ne peut être inférieure à la largeur de l'escalier (min. ${metricToImperial(stairWidthValue)}) selon CNB 9.8.6.3.`;
    }
   }
   isValid = false;
   }
  ...
  // Calculer l'espace entre les volées (si largeur palier > 2x largeur escalier)
  ...
  * Règles:
  * - Largeur palier >= 2 x largeur escalier (géométrie)
 ...
 function validateUShapedInputs() {
  ...
  // Valider: Largeur palier >= 2 x largeur escalier
