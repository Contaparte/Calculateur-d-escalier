...
/**
 * Calcule la dimension manquante (hauteur ou longueur) selon la prioritÃ©
 * @param {Object} params - Paramètres incluant la dimension connue
 * @returns {Object} - Dimension calculÃ©e et solutions
 */
...
function displayCalculatorResults(solutions, params) {
    ...
    // Avertissement dimension calculÃ©e
    if (dimensionCalculated) {
        const priorityLabel = priority === 'comfort' ? 'confort optimal' : "Ã©conomie d'espace";
        if (dimensionCalculated === 'rise') {
            html += '<div class="step-formula">';
            html += `<strong>📐 Hauteur suggÃ©rÃ©e (${priorityLabel}) :</strong> ${formatValuePrecise(totalRiseValue, isMetric)}`;
            html += '</div>';
        } else if (dimensionCalculated === 'run') {
            html += '<div class="step-formula">';
            html += `<strong>📐 Longueur suggÃ©rÃ©e (${priorityLabel}) :</strong> ${formatValuePrecise(totalRunValue, isMetric)}`;
            html += '</div>';
        }
    }
    ...
    // DÃ©terminer la longueur disponible pour les girons
    ...
    if (isMetric) {
        // VÃ©rification hauteur (contremarches)
        ...
        html += riseError < 0.01 ? 'âœ"' : `âš  Ã‰cart ${riseError.toFixed(4)} mm`;
        
        // VÃ©rification longueur (girons) - pour TOUS les escaliers
        if (!best.isSpiral && best.numTreads && best.treadDepth) {
            ...
            html += treadError < 0.01 ? 'âœ"' : `âš  Ã‰cart ${treadError.toFixed(4)} mm`;
        }
    } else {
        // VÃ©rification hauteur (contremarches) en impÃ©rial
        ...
        // VÃ©rification longueur (girons) en impÃ©rial - pour TOUS les escaliers
        if (!best.isSpiral && best.numTreads && best.treadDepth) {
            html += '<br>';
            ...
            html += treadError < 0.001 ? 'âœ"' : `âš  Ã‰cart ${treadError.toFixed(6)}"`;
        }
    }
