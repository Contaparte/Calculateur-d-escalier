...
function generateStraightStairVisualization(stairData) {
    ...
    // Dimensions du SVG (plus large pour 2 vues)
    const W = 750, H = 400;
    const midX = 380; // Separation entre les deux vues
    ...
    let svg = '<svg width="' + W + '" height="' + H + '" xmlns="http://www.w3.org/2000/svg" style="background:#fff;border:1px solid #ddd;border-radius:4px;">';
    ...
    const P = { left: 45, right: 30, top: 45, bottom: 75 };
    const profW = midX - P.left - P.right - 20;
    ...
    // Titre profil
    svg += '<text x="' + (midX/2) + '" y="22" style="font:bold 11px Arial;fill:#1b5e20;" text-anchor="middle">VUE DE PROFIL</text>';
    ...
    svg += '<path d="' + pathData + '" fill="none" stroke="#1b5e20" stroke-width="2"/>';
    
    // Lignes de niveau
    svg += '<line x1="' + (startX - 20) + '" y1="' + startY + '" x2="' + r(startX + stairW_prof + 5) + '" y2="' + startY + '" stroke="#999" stroke-width="1" stroke-dasharray="3,2"/>';
    svg += '<text x="' + (startX - 25) + '" y="' + (startY + 4) + '" style="font:9px Arial;fill:#666;" text-anchor="end">Sol</text>';
    
    svg += '<line x1="' + (endX_prof - 5) + '" y1="' + endY_prof + '" x2="' + r(endX_prof + 35) + '" y2="' + endY_prof + '" stroke="#999" stroke-width="1" stroke-dasharray="3,2"/>';
    svg += '<text x="' + r(endX_prof + 38) + '" y="' + (endY_prof + 4) + '" style="font:9px Arial;fill:#666;">Arrivee</text>';
    
    // Cotation horizontale (en bas)
    const dimY_prof = startY + 22;
    ...
    svg += '<line x1="' + startX + '" y1="' + (startY + 2) + '" x2="' + startX + '" y2="' + (dimY_prof + 3) + '" stroke="#555" stroke-width="1"/>';
    svg += '<line x1="' + r(startX + stairW_prof) + '" y1="' + (startY + 2) + '" x2="' + r(startX + stairW_prof) + '" y2="' + (dimY_prof + 3) + '" stroke="#555" stroke-width="1"/>';
    svg += '<text x="' + r(startX + stairW_prof/2) + '" y="' + (dimY_prof + 14) + '" style="font:bold 10px Arial;fill:#333;" text-anchor="middle">' + runText + '</text>';
    svg += '<text x="' + r(startX + stairW_prof/2) + '" y="' + (dimY_prof + 26) + '" style="font:9px Arial;fill:#666;" text-anchor="middle">(' + numTreads + ' G)</text>';
    
    // Cotation verticale (a droite)
    ...
    svg += '<line x1="' + (endX_prof + 2) + '" y1="' + startY + '" x2="' + (dimX_prof + 3) + '" y2="' + startY + '" stroke="#555" stroke-width="1"/>';
    svg += '<line x1="' + (endX_prof + 2) + '" y1="' + endY_prof + '" x2="' + (dimX_prof + 3) + '" y2="' + endY_prof + '" stroke="#555" stroke-width="1"/>';
    
    const midY_prof = r((startY + endY_prof) / 2);
    svg += '<text x="' + (dimX_prof + 5) + '" y="' + (midY_prof - 5) + '" style="font:bold 10px Arial;fill:#333;">' + riseText + '</text>';
    svg += '<text x="' + (dimX_prof + 5) + '" y="' + (midY_prof + 8) + '" style="font:9px Arial;fill:#666;">(' + numRisers + ' CM)</text>';
    
    // ========== VUE EN PLAN (droite) ==========
    const planLeft = midX + 20;
    const planRight = W - 60;
    const planTop = 45;
    const planBottom = H - 50;
    const planW = planRight - planLeft;
    const planH = planBottom - planTop;
    
    // Utiliser la largeur d'escalier ou une valeur par defaut (914mm = 3')
    ...
    const planStartX = r(planLeft + (planW - stairW_plan) / 2);
    const planStartY = r(planTop + (planH - stairH_plan) / 2);
    
    // Titre plan
    svg += '<text x="' + r((planLeft + planRight) / 2) + '" y="22" style="font:bold 11px Arial;fill:#1b5e20;" text-anchor="middle">VUE EN PLAN</text>';
    
    // Rectangle de l'escalier
    svg += '<rect x="' + planStartX + '" y="' + planStartY + '" width="' + stairW_plan + '" height="' + stairH_plan + '" fill="#c8e6c9" stroke="#2e7d32" stroke-width="1.5"/>';
    
    // Lignes des girons (marches)
    ...
    svg += '<line x1="' + r(planStartX + 15) + '" y1="' + arrowY + '" x2="' + r(planStartX + stairW_plan - 15) + '" y2="' + arrowY + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrE)"/>';
    svg += '<text x="' + r(planStartX + stairW_plan/2) + '" y="' + (arrowY - 8) + '" style="font:9px Arial;fill:#1b5e20;font-style:italic;" text-anchor="middle">Montee</text>';
    ...
    svg += '<line x1="' + planStartX + '" y1="' + r(planStartY + stairH_plan + 2) + '" x2="' + planStartX + '" y2="' + (dimY_plan + 3) + '" stroke="#555" stroke-width="1"/>';
    svg += '<line x1="' + r(planStartX + stairW_plan) + '" y1="' + r(planStartY + stairH_plan + 2) + '" x2="' + r(planStartX + stairW_plan) + '" y2="' + (dimY_plan + 3) + '" stroke="#555" stroke-width="1"/>';
    svg += '<text x="' + r(planStartX + stairW_plan/2) + '" y="' + (dimY_plan + 14) + '" style="font:bold 10px Arial;fill:#333;" text-anchor="middle">' + runText + '</text>';
    
    // Cotation largeur (a droite)
    const dimX_plan = r(planStartX + stairW_plan + 15);
    ...
    svg += '<line x1="' + r(planStartX + stairW_plan + 2) + '" y1="' + planStartY + '" x2="' + (dimX_plan + 3) + '" y2="' + planStartY + '" stroke="#555" stroke-width="1"/>';
    svg += '<line x1="' + r(planStartX + stairW_plan + 2) + '" y1="' + r(planStartY + stairH_plan) + '" x2="' + (dimX_plan + 3) + '" y2="' + r(planStartY + stairH_plan) + '" stroke="#555" stroke-width="1"/>';
    
    const midY_plan = r(planStartY + stairH_plan / 2);
    svg += '<text x="' + (dimX_plan + 5) + '" y="' + midY_plan + '" style="font:bold 10px Arial;fill:#333;">' + widthText + '</text>';
    
    // Ligne de separation
    svg += '<line x1="' + midX + '" y1="35" x2="' + midX + '" y2="' + (H - 25) + '" stroke="#ddd" stroke-width="1" stroke-dasharray="5,3"/>';
    
    // Legende en bas
    svg += '<text x="15" y="' + (H - 8) + '" style="font:9px Arial;fill:#555;">CM = ' + riserText + '  |  G = ' + treadText + '</text>';
