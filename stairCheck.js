...
function displayCalculatorResults(solutions, params) {
    ...
    if (!solutions || solutions.length === 0) {
        ...
        if (isMetric) {
            html += `<li>Pour ${(totalRiseValue / 1000).toFixed(2)} m de hauteur, longueur minimale ≈ ${minLength} mm</li>`;
        } else {
        ...
        // Afficher la suggestion Privé vs Commun seulement pour la Partie 9
    ...
    html += `<h3>${isCompliant ? '✓' : '⚠️'} Solution optimale (${codeRef})</h3>`;
    
    // Avertissement largeur
    if (!isWidthOk) {
        html += `<div class="warning"><p>⚠️ Largeur ${formatValue(stairWidthValue, isMetric, 0)} inférieure au minimum requis (${formatValue(limits.minWidth, isMetric, 0)})</p></div>`;
    }
    ...
    html += `<li><strong>Largeur :</strong> ${formatValue(stairWidthValue, isMetric, 0)} ${isWidthOk ? '✓' : '⚠️'}</li>`;
    html += '</ul></div>';
    ...
    if (isMetric) {
        ...
        html += riseError < 0.01 ? '✓ Exact' : `⚠️ Écart ${riseError.toFixed(4)} mm`;
        ...
        if (best.useLandingConfiguration) {
            ...
            html += treadError < 0.01 ? '✓ Exact' : `⚠️ Écart ${treadError.toFixed(4)} mm`;
        }
    } else {
    ...
    // Règle du pas
    html += '<div class="result-section">';
    html += `<h4>${best.stepRule.isValid ? '✓' : '⚠️'} Règle du pas (${best.stepRule.validCount}/3)</h4>`;
    html += '<ul>';
...
function displayVerificationResults(params) {
    ...
    if (isCompliant) {
        ...
        html += `<h3>⚠️ Non conforme au ${codeReference}</h3>`;
        html += '<div class="result-section">';
    ...
    // Règle du pas
    html += '<div class="result-section">';
    html += `<h4>${stepRule.isValid ? '✓' : '⚠️'} Règle du pas (${stepRule.validCount}/3)</h4>`;
    html += '<ul>';
