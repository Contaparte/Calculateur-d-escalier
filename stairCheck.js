...
function checkStepRule(riserMm, treadMm) {
 ...
 // Règle 2: G × H = 71 à  74 poÂ² (458-477 cmÂ²)
...
function generateStraightStairVisualization(stairData) {
 ...
 // Calculer l'échelle avec référence stable
 const refWidth = 3000;
 const refHeight = 1200;
 const refScale = Math.min(planW / refWidth, planH / refHeight) * 0.85;
 const actualScale = Math.min(planW / totalRun, planH / actualWidth) * 0.85;
 const scalePlan = Math.min(refScale, actualScale);
...
function generateLShapedStairVisualization(stairData) {
 ...
 // Échelle stable avec référence
 const refWidth = 2500;
 const refHeight = 2500;
 const refScale = Math.min(planW / refWidth, planH / refHeight) * 0.75;
 const actualScale = Math.min(planW / totalPlanW, planH / totalPlanH) * 0.75;
 const scalePlan = Math.min(refScale, actualScale);
...
function generateUShapedStairVisualization(stairData) {
 ...
 // Utiliser une échelle de référence stable basée sur des dimensions typiques
 // Ceci évite les changements d'échelle brusques lors de modifications des paramètres
 const refWidth = 2000; // Largeur de référence typique (mm)
 const refHeight = 3000; // Hauteur de référence typique (mm)
 const refScale = Math.min(planW / refWidth, planH / refHeight) * 0.70;
 
 // Utiliser l'échelle de référence, mais ajuster si les dimensions sont très différentes
 const totalPlanWidth = actualLandingWidth;
 const maxFlightRun = Math.max(actualFlight1Run, actualFlight3Run);
 const actualScale = Math.min(planW / totalPlanWidth, planH / maxFlightRun) * 0.70;
 
 // Choisir l'échelle qui permet d'afficher tout le dessin sans dépasser
 const scalePlan = Math.min(refScale, actualScale);
...
function generateUShapedRadiatingVisualization(stairData) {
 ...
 // Calculer l'échelle avec référence stable
 const refWidth = 2000;
 const refHeight = 3000;
 const refScale = Math.min(planW / refWidth, planH / refHeight) * 0.65;
 
 const totalPlanWidth = actualLandingWidth;
 const maxFlightRun = Math.max(actualFlight1Run, actualFlight2Run);
 const totalPlanHeight = maxFlightRun + actualWidth;
 const actualScale = Math.min(planW / totalPlanWidth, planH / totalPlanHeight) * 0.65;
...
function generateUShapedMixedVisualization(stairData) {
 ...
 const refWidth = 2000;
 const refHeight = 3000;
 const refScale = Math.min(planW / refWidth, planH / refHeight) * 0.60;
 
 const totalPlanWidth = actualFlight2Run;
 const maxFlightRun = Math.max(actualFlight1Run, actualFlight3Run);
 const actualScale = Math.min(planW / totalPlanWidth, planH / maxFlightRun) * 0.60;
 ...
 // Position de départ centrée (inclut l'espace entre volées)
 const totalW_plan = (2 * stairWidth_plan) + spaceBetween_plan;
 ...
 if (configType === 'rect_landing_radiating_rect') {
  ...
  const landingX = r(planStartX + stairWidth_plan + spaceBetween_plan);
  const landingY = planStartY;
  svg += '<rect x="' + landingX + '" y="' + landingY + '" width="' + stairWidth_plan + '" height="' + landingDepth_plan + '" fill="#fff9c4" stroke="#f9a825" stroke-width="2"/>';
  // Texte Palier positionné en haut pour éviter conflit avec la flèche
  svg += '<text x="' + r(landingX + stairWidth_plan/2) + '" y="' + r(landingY + landingDepth_plan * 0.35 + 4) + '" style="font:italic 10px Arial;fill:#f57f17;" text-anchor="middle">Palier</text>';
...
function generateUShapedDoubleRadiatingVisualization(stairData) {
    ...
    const actualFlight2Run = flight2Run || (2 * actualWidth);
    ...
    const refWidth = 2000;
    const refHeight = 3000;
    const refScale = Math.min(planW / refWidth, planH / refHeight) * 0.55;
    
    const totalPlanWidth = actualFlight2Run;
    const maxFlightRun = Math.max(actualFlight1Run, actualFlight3Run);
    const actualScale = Math.min(planW / totalPlanWidth, planH / maxFlightRun) * 0.55;
    ...
    const totalW_plan = (2 * stairWidth_plan) + landingDepth_plan + spaceBetween_plan;
    const totalH_plan = Math.max(flight1Run_visual, flight2Run_visual) + stairWidth_plan;
    const planStartX = r(margin.left + (planW - totalW_plan) / 2);
    const planStartY = r(margin.top + (planH - totalH_plan) / 2);
...
function displayVerificationResults(params) {
 ...
 html += `<li>${stepRule.rule2.isValid ? '✓' : '✗'} Règle 2 (G×H) : ${stepRule.rule2.value.toFixed(2)} poÂ² (71-74)</li>`;
