 ...
 */
/**
 * Génère une visualisation SVG d'un escalier droit (vue en plan uniquement)
 * Toutes les valeurs d'entrée sont en mm (unités internes du calculateur)
 * L'affichage est formaté selon le système de mesure choisi par l'utilisateur
 */
function generateStraightStairVisualization(stairData) {
    ...
    // Dimensions du SVG - optimisées pour vue en plan seule
    const W = 700, H = 380;
    ...
    // Marqueurs flèches pour cotations
    svg += '<defs>';
    ...
    // Marqueur flèche de montée - plus fine et élégante
    ...
    // ========== VUE EN PLAN (centrée) ==========
    const margin = { left: 60, right: 80, top: 70, bottom: 90 };
    const planW = W - margin.left - margin.right;
    const planH = H - margin.top - margin.bottom;
    
    // Calculer l'échelle pour maximiser l'utilisation de l'espace
    ...
    // Numérotation des marches (optionnel - première et dernière)
    ...
    // Flèche de montée - fine, de la 1ère à la dernière contremarche
    const arrowY = r(planStartY + stairH_plan / 2);
    const arrowStartX = r(planStartX + treadW_plan);  // Commence à la 1ère contremarche
    const arrowEndX = r(planStartX + stairW_plan);     // Finit à la dernière contremarche
    svg += '<line x1="' + arrowStartX + '" y1="' + arrowY + '" x2="' + arrowEndX + '" y2="' + arrowY + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrMontee)"/>';
    svg += '<text x="' + r((arrowStartX + arrowEndX) / 2) + '" y="' + (arrowY - 12) + '" style="font:italic 11px Arial;fill:#1b5e20;" text-anchor="middle">Montée</text>';
    
    // Indicateurs Départ et Arrivée
    svg += '<text x="' + r(planStartX + treadW_plan/2) + '" y="' + (planStartY - 8) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">Départ</text>';
    svg += '<text x="' + r(planStartX + stairW_plan - treadW_plan/2) + '" y="' + (planStartY - 8) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">Arrivée</text>';
    ...
    svg += '<text x="' + r(planStartX + stairW_plan/2) + '" y="' + (dimY_bottom + 28) + '" style="font:10px Arial;fill:#666;" text-anchor="middle">(' + numTreads + ' girons × ' + treadText + ')</text>';
    
    // Cotation largeur (à droite)
    ...
    // Cotation hauteur totale (à gauche)
    ...
    // ===== LÉGENDE COMPLÈTE EN BAS =====
    const legendY = H - 12;
    svg += '<text x="15" y="' + legendY + '" style="font:10px Arial;fill:#555;">';
    svg += 'Légende: CM (contremarche) = ' + riserText + '  |  G (giron) = ' + treadText + '  |  Largeur = ' + widthText + '  |  Hauteur totale = ' + riseText + ' (' + numRisers + ' CM)';
    svg += '</text>';
 ...
 * Gère également le cas des marches rayonnantes à l'extrémité
 */
/**
 * Génère la visualisation SVG pour un escalier en L (90 degrés)
...
function generateLShapedStairVisualization(stairData) {
    ...
    // Calculer les dimensions des volées
    ...
    // ========== VUE EN PLAN ==========
    const margin = { left: 50, right: 70, top: 70, bottom: 70 };
    const planW = W - margin.left - margin.right;
    const planH = H - margin.top - margin.bottom;
    ...
    const totalPlanW = flight1Run + actualWidth;
    const totalPlanH = flight2Run + actualWidth;
    ...
    const planStartX = r(margin.left + (planW - (stairW1_plan + stairWidth_plan)) / 2);
    const planStartY = r(margin.top + (planH - (stairW2_plan + stairWidth_plan)) / 2);
    
    if (isLanding) {
        // ===== ESCALIER EN L AVEC PALIER STANDARD =====
        
        // Volée 1 (horizontale) - du bas vers le coin
        ...
        svg += '<text x="' + r(palierX + landingW_plan/2) + '" y="' + r(palierY + landingW_plan/2 + 4) + '" style="font:italic 10px Arial;fill:#f57f17;" text-anchor="middle">Palier</text>';
        
        // Volée 2 (verticale) - du coin vers le haut
        ...
        // Flèches de montée - fines, de la 1ère à la dernière contremarche de chaque volée
        // Volée 1
        ...
        // Texte "Montée" au centre
        svg += '<text x="' + r(planStartX + stairW1_plan/2) + '" y="' + (arrow1Y - 10) + '" style="font:italic 10px Arial;fill:#1b5e20;" text-anchor="middle">Montée</text>';
        
        // Indicateurs Départ et Arrivée
        svg += '<text x="' + r(planStartX + treadW1_exact/2) + '" y="' + r(planStartY + stairW2_plan - 5) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">Départ</text>';
        svg += '<text x="' + r(palierX + stairWidth_plan/2) + '" y="' + (planStartY - 8) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">Arrivée</text>';
        
        // Cotations
        const dimY1 = r(planStartY + stairW2_plan + stairWidth_plan + 25);
        const run1Text = formatValuePrecise(flight1Run, isMetric);
        ...
        svg += '<text x="' + r(planStartX + stairW1_plan/2) + '" y="' + (dimY1 + 14) + '" style="font:10px Arial;fill:#333;" text-anchor="middle">' + run1Text + ' (' + flight1Treads + ' G)</text>';
        
        const dimX2 = r(palierX + stairWidth_plan + 25);
        const run2Text = formatValuePrecise(flight2Run, isMetric);
        ...
        svg += '<text x="' + (dimX2 + 5) + '" y="' + r(planStartY + stairW2_plan/2 + 4) + '" style="font:10px Arial;fill:#333;">' + run2Text + ' (' + flight2Treads + ' G)</text>';
        
        // Cotation palier
        const landingText = formatValuePrecise(actualLanding, isMetric);
        svg += '<line x1="' + palierX + '" y1="' + dimY1 + '" x2="' + r(palierX + landingW_plan) + '" y2="' + dimY1 + '" stroke="#f9a825" stroke-width="1" marker-start="url(#arrLS)" marker-end="url(#arrLE)"/>';
        svg += '<text x="' + r(palierX + landingW_plan/2) + '" y="' + (dimY1 + 14) + '" style="font:9px Arial;fill:#f57f17;" text-anchor="middle">Palier</text>';
        ...
        if (isRadiatingAtExtremity && radiatingAtEnd) {
            ...
            if (numRadSteps === 2) {
                ...
                svg += '<text x="' + r(cornerX + L/2) + '" y="' + r(cornerY + L/2 + 4) + '" style="font:bold 10px Arial;fill:#e65100;" text-anchor="middle">2×45°</text>';
            } else {
                ...
                svg += '<text x="' + r(cornerX + L/2) + '" y="' + r(cornerY + L/2 + 4) + '" style="font:bold 10px Arial;fill:#e65100;" text-anchor="middle">3×30°</text>';
            }
            
            // Flèche de montée continue
            const arrow1Y = r(planStartY + stairWidth_plan/2);
            const arrowStartX = r(planStartX + treadW1_exact);
            svg += '<line x1="' + arrowStartX + '" y1="' + arrow1Y + '" x2="' + r(cornerX + L/2) + '" y2="' + arrow1Y + '" stroke="#1b5e20" stroke-width="1.5"/>';
            svg += '<line x1="' + r(cornerX + L/2) + '" y1="' + arrow1Y + '" x2="' + r(cornerX + L/2) + '" y2="' + r(cornerY - 10) + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrLMontee)"/>';
            svg += '<text x="' + r(planStartX + stairW1_plan/2) + '" y="' + (arrow1Y - 10) + '" style="font:italic 10px Arial;fill:#1b5e20;" text-anchor="middle">Montée</text>';
            
            // Indicateurs
            svg += '<text x="' + r(planStartX + treadW1_exact/2) + '" y="' + (planStartY - 8) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">Départ</text>';
            svg += '<text x="' + r(cornerX + L/2) + '" y="' + (cornerY - 15) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">Arrivée</text>';
            
            // Cotations
            const dimY1 = r(planStartY + stairWidth_plan + 25);
            const run1Text = formatValuePrecise(firstFlightRun, isMetric);
            ...
            svg += '<text x="' + r(planStartX + stairW1_plan/2) + '" y="' + r(planStartY + stairWidth_plan + 12) + '" style="font:9px Arial;fill:#666;" text-anchor="middle">' + flight1Treads + ' girons rect.</text>';
            
        } else if (isRadiatingAtExtremity && radiatingAtStart) {
            // Marches rayonnantes au DÉBUT
            const treadW2_exact = stairW2_plan / flight2Treads;
            ...
            // Flèche de montée
            const arrowX = r(cornerX + stairWidth_plan/2);
            const arrowStartY = r(cornerY + L/2);
            svg += '<line x1="' + arrowX + '" y1="' + arrowStartY + '" x2="' + arrowX + '" y2="' + r(planStartY + 10) + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrLMontee)"/>';
            svg += '<text x="' + r(cornerX - 15) + '" y="' + r(planStartY + stairW2_plan/2) + '" style="font:italic 10px Arial;fill:#1b5e20;" text-anchor="end">Montée</text>';
            
            // Indicateurs
            svg += '<text x="' + r(cornerX + L/2) + '" y="' + r(cornerY + L + 15) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">Départ</text>';
            svg += '<text x="' + r(cornerX + stairWidth_plan/2) + '" y="' + (planStartY - 8) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">Arrivée</text>';
            
            // Cotations
            const dimX2 = r(cornerX + stairWidth_plan + 25);
            const run2Text = formatValuePrecise(secondFlightRun, isMetric);
            ...
            svg += '<text x="' + r(cornerX - 8) + '" y="' + r(planStartY + stairW2_plan/2 + 4) + '" style="font:9px Arial;fill:#666;" text-anchor="end">' + flight2Treads + ' girons rect.</text>';
            ...
            // Flèche de montée (une seule volée continue) - virage à 90°
            const arrow1Y = r(planStartY + stairW2_plan + stairWidth_plan/2);
            ...
            svg += '<line x1="' + arrowCornerX + '" y1="' + r(cornerY) + '" x2="' + arrowCornerX + '" y2="' + r(planStartY + 10) + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrLMontee)"/>';
    ...
    // ===== LÉGENDE EN BAS =====
    let legendText = 'Légende: CM = ' + riserText + '  |  G = ' + treadText + '  |  Largeur = ' + widthText + '  |  Hauteur = ' + riseText + ' (' + numRisers + ' CM)';
    if (isLanding) {
        legendText += '  |  Palier = ' + formatValuePrecise(actualLanding, isMetric);
    } else {
        legendText += '  |  ' + numRadSteps + ' marches ray. (' + (numRadSteps === 2 ? '45°' : '30°') + ')';
    }
    svg += '<text x="15" y="' + (H - 12) + '" style="font:9px Arial;fill:#555;">' + legendText + '</text>';
...
function displayCalculatorResults(solutions, params) {
    ...
    html += `<h3>${isCompliant ? 'âœ“' : 'âœ—'} Solution optimale (${codeRef})</h3>`;
    ...
    // Règle du pas
    html += '<div class="result-section">';
    html += `<h4>${best.stepRule.isValid ? 'âœ“' : 'âœ—'} Règle du pas (${best.stepRule.validCount} sur 3)</h4>`;
    html += '<ul>';
    html += `<li>${best.stepRule.rule1.isValid ? 'âœ“' : 'âœ—'} G + H = ${best.stepRule.rule1.value.toFixed(2)}" (${best.stepRule.rule1.range})</li>`;
    html += `<li>${best.stepRule.rule2.isValid ? 'âœ“' : 'âœ—'} G × H = ${best.stepRule.rule2.value.toFixed(2)} (${best.stepRule.rule2.range})</li>`;
    ...
    if (stairConfigValue === 'l_shaped' && (lShapedConfigValue === 'two_45deg' || lShapedConfigValue === 'three_30deg')) {
        ...
        if (best.firstFlightRectTreads !== undefined) {
            ...
            html += '<li>  ↳ 1ère direction : ' + best.firstFlightRectTreads + ' girons × ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
            ...
            html += '<li>  ↳ 2ème direction : ' + best.secondFlightRectTreads + ' girons × ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
