...
function validateImperialInput(inputValue) {
    ...
    inputValue = inputValue.replace(/[""Ã¢Ã¢â€šÂ¬Ã‚Â³]/g, '"');
 ...
 * doivent Ãªtre testÃ©s APRÃƒÆ’Ã‹â€ S les formats avec apostrophe obligatoire
 */
/**
 * Convertit une valeur impÃ©riale en millimÃ¨tres
...
function imperialToMetric(imperialValue) {
    ...
    input = input.replace(/[''']/g, "'").replace(/[""Ã¢Ã¢â€šÂ¬Ã‚Â³]/g, '"');
...
// =====================================================================
// RÃƒÆ’Ã‹â€ GLE DU PAS ET VALIDATION
// =====================================================================
...
function calculateLShapedWithRadiating(params) {
    ...
    for (let numRisers = minRisers; numRisers <= maxRisers + 3; numRisers++) {
        ...
        // Tous les girons rectangulaires doivent avoir la MÃƒÅ ME profondeur (uniformitÃ© dans une volÃ©e)
 ...
 */
/**
 * GÃ©nÃ¨re une visualisation SVG d'un escalier droit (vue de profil)
 * Toutes les valeurs d'entrÃ©e sont en mm (unitÃ©s internes du calculateur)
 * L'affichage est formatÃ© selon le systÃ¨me de mesure choisi par l'utilisateur
 */
...
function generateLShapedStairVisualization(stairData) {
    ...
    if (isLanding) {
        ...
        const centerX = cornerX;
        const centerY = r(cornerY + stairWidth_plan);
        const radius = stairWidth_plan;
        
        if (numRadSteps === 2) {
            // 2 marches a 45 degres - une ligne a 45 degres
            const endX45 = r(centerX + radius * Math.cos(Math.PI/4));
            const endY45 = r(centerY - radius * Math.sin(Math.PI/4));
            svg += '<line x1="' + centerX + '" y1="' + centerY + '" x2="' + endX45 + '" y2="' + endY45 + '" stroke="#e65100" stroke-width="2"/>';
            svg += '<text x="' + r(cornerX + stairWidth_plan/2) + '" y="' + r(cornerY + stairWidth_plan/2 + 4) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">2x45</text>';
        } else {
            // 3 marches a 30 degres - deux lignes a 30 et 60 degres
            const endX30 = r(centerX + radius * Math.cos(Math.PI/6));
            const endY30 = r(centerY - radius * Math.sin(Math.PI/6));
            const endX60 = r(centerX + radius * Math.cos(Math.PI/3));
            const endY60 = r(centerY - radius * Math.sin(Math.PI/3));
            svg += '<line x1="' + centerX + '" y1="' + centerY + '" x2="' + endX30 + '" y2="' + endY30 + '" stroke="#e65100" stroke-width="2"/>';
            svg += '<line x1="' + centerX + '" y1="' + centerY + '" x2="' + endX60 + '" y2="' + endY60 + '" stroke="#e65100" stroke-width="2"/>';
            svg += '<text x="' + r(cornerX + stairWidth_plan/2) + '" y="' + r(cornerY + stairWidth_plan/2 + 4) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">3x30</text>';
        }
        ...
        // Fleche de montee (une seule volee continue)
        // Partie 1 - horizontale
        const arrow1Y = r(planStartY + stairW2_plan + stairWidth_plan/2);
        svg += '<line x1="' + r(planStartX + 15) + '" y1="' + arrow1Y + '" x2="' + r(planStartX + stairW1_plan - 5) + '" y2="' + arrow1Y + '" stroke="#1b5e20" stroke-width="2"/>';
        
        // Partie courbe au coin (arc)
        const arcCX = cornerX;
        const arcCY = r(cornerY + stairWidth_plan);
        const arcR = stairWidth_plan/2;
        svg += '<path d="M' + r(cornerX - 5) + ',' + r(cornerY + stairWidth_plan/2) + ' A' + arcR + ',' + arcR + ' 0 0 0 ' + r(cornerX + stairWidth_plan/2) + ',' + r(cornerY + 5) + '" fill="none" stroke="#1b5e20" stroke-width="2"/>';
        
        // Partie 2 - verticale
        const arrow2X = r(cornerX + stairWidth_plan/2);
        svg += '<line x1="' + arrow2X + '" y1="' + r(cornerY - 5) + '" x2="' + arrow2X + '" y2="' + r(planStartY + 15) + '" stroke="#1b5e20" stroke-width="2" marker-end="url(#arrLEG)"/>';
...
function displayCalculatorResults(solutions, params) {
    ...
    if (!solutions || solutions.length === 0) {
        ...
        if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
            ...
            if (isMetric) {
                html += `<li>Longueur totale minimale requise Ã¢Ã¢â‚¬Â°Ã‹â€  ${minLengthWithLanding.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm + 2Ã— palier)</li>`;
            } else {
                html += `<li>Longueur totale minimale requise Ã¢Ã¢â‚¬Â°Ã‹â€  ${metricToImperial(minLengthWithLanding)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)} + 2Ã— palier)</li>`;
            }
            ...
            if (isMetric) {
                html += `<li>Longueur totale minimale requise Ã¢â€°Ë† ${minLength.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm)</li>`;
            } else {
                html += `<li>Longueur totale minimale requise Ã¢â€°Ë† ${metricToImperial(minLength)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)})</li>`;
            }
            ...
            if (isMetric) {
                html += `<li>Longueur minimale requise Ã¢Ã¢â‚¬Â°Ã‹â€  ${minLength.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm)</li>`;
            } else {
                html += `<li>Longueur minimale requise Ã¢Ã¢â‚¬Â°Ã‹â€  ${metricToImperial(minLength)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)})</li>`;
            }
    ...
    if (!isWidthOk) {
        html += `<div class="warning"><p>âœ“ Largeur ${formatValue(stairWidthValue, isMetric, 0)} infÃ©rieure au minimum requis (${formatValue(limits.minWidth, isMetric, 0)})</p></div>`;
    }
    ...
    if (isMetric) {
        ...
        html += riseError < 0.01 ? 'âœ“ Exact' : `âœ“ Ã‰cart ${riseError.toFixed(4)} mm`;
        ...
        if (best.useLandingConfiguration) {
            ...
            html += treadError < 0.01 ? 'âœ“ Exact' : `âœ“ Ã‰cart ${treadError.toFixed(4)} mm`;
        }
    } else {
    ...
    // RÃ¨gle du pas
    html += '<div class="result-section">';
    html += `<h4>${best.stepRule.isValid ? 'âœ“' : 'âœ“ '} RÃ¨gle du pas (${best.stepRule.validCount} sur 3)</h4>`;
    html += '<ul>';
    ...
    if (stairConfigValue === 'l_shaped' && (lShapedConfigValue === 'two_45deg' || lShapedConfigValue === 'three_30deg')) {
        ...
        if (best.firstFlightRectTreads !== undefined) {
            ...
            html += '<li>  Ã¢â€ â€™ 1Ã¨re direction : ' + best.firstFlightRectTreads + ' girons Ã— ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
            ...
            html += '<li>  Ã¢â€ â€™ 2Ã¨me direction : ' + best.secondFlightRectTreads + ' girons Ã— ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
