...
function imperialToMetric(imperialValue) {
    ...
    input = input.replace(/[''â€²]/g, "'").replace(/[""â€³]/g, '"');
...
function displayCalculatorResults(solutions, params) {
    ...
    if (!solutions || solutions.length === 0) {
        // Utiliser les vraies limites CNB pour le calcul de la longueur minimale
        const limits = getCNBLimits(buildingTypeValue, stairTypeValue);
        const minRisersNeeded = Math.ceil(totalRiseValue / limits.maxRiser);
        const minTreadsNeeded = minRisersNeeded - 1;
        const minLength = Math.ceil((minTreadsNeeded * limits.minTread) / 100) * 100;
        
        let html = '<h3>âš ï¸ Aucune solution conforme trouvÃ©e</h3>';
        html += '<div class="warning">';
        html += '<p><strong>Suggestions :</strong></p>';
        html += '<ul>';
        
        if (isMetric) {
            html += `<li>Pour ${(totalRiseValue / 1000).toFixed(2)} m de hauteur, longueur minimale â‰ˆ ${minLength} mm</li>`;
        } else {
            html += `<li>Pour ${metricToImperial(totalRiseValue)} de hauteur, longueur minimale â‰ˆ ${metricToImperial(minLength)}</li>`;
        }
        
        html += '<li>Essayez une configuration avec palier (escalier en L ou U)</li>';
        
        // Afficher la suggestion Prive vs Commun seulement pour la Partie 9
        if (buildingTypeValue === 'part9') {
            html += '<li>VÃ©rifiez le type d\'escalier (PrivÃ© vs Commun)</li>';
        } else {
            html += `<li>Note : Partie 3 requiert giron min. ${limits.minTread} mm et contremarche max. ${limits.maxRiser} mm</li>`;
        }
    ...
    html += `<h3>${isCompliant ? 'âœ“' : 'âš ï¸'} Solution optimale (${codeRef})</h3>`;
    
    // Avertissement largeur
    if (!isWidthOk) {
        html += `<div class="warning"><p>âš ï¸ Largeur ${formatValue(stairWidthValue, isMetric, 0)} infÃ©rieure au minimum requis (${formatValue(limits.minWidth, isMetric, 0)})</p></div>`;
    }
    
    // Dimensions principales
    html += '<div class="result-section">';
    html += '<h4>ðŸ“ Dimensions calculÃ©es</h4>';
    html += '<ul>';
    ...
    html += `<li><strong>Largeur :</strong> ${formatValue(stairWidthValue, isMetric, 0)} ${isWidthOk ? 'âœ“' : 'âš ï¸'}</li>`;
    html += '</ul></div>';
    ...
    if (isMetric) {
        ...
        html += riseError < 0.01 ? 'âœ“ Exact' : `âš ï¸ Ã‰cart ${riseError.toFixed(4)} mm`;
        ...
        if (best.useLandingConfiguration) {
            ...
            html += treadError < 0.01 ? 'âœ“ Exact' : `âš ï¸ Ã‰cart ${treadError.toFixed(4)} mm`;
        }
    } else {
    ...
    // RÃ¨gle du pas
    html += '<div class="result-section">';
    html += `<h4>${best.stepRule.isValid ? 'âœ“' : 'âš ï¸'} RÃ¨gle du pas (${best.stepRule.validCount}/3)</h4>`;
    html += '<ul>';
    ...
    html += '<div class="warning">';
    html += '<p><strong>ðŸ“ Instructions pour le traÃ§age CAD :</strong></p>';
    html += '<ul>';
    ...
    if (best.isSpiral) {
        html += '<div class="result-section">';
        html += '<h4>ðŸ“ Notes - Escalier hÃ©licoÃ¯dal</h4>';
        html += '<ul>';
...
function displayVerificationResults(params) {
    ...
    if (isCompliant) {
        ...
        html += `<h3>âš ï¸ Non conforme au ${codeReference}</h3>`;
        html += '<div class="result-section">';
    ...
    // RÃ¨gle du pas
    html += '<div class="result-section">';
    html += `<h4>${stepRule.isValid ? 'âœ“' : 'âš ï¸'} RÃ¨gle du pas (${stepRule.validCount}/3)</h4>`;
    html += '<ul>';
