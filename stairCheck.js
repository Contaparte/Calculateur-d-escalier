...
function calculateLShapedWithLanding(params) {
    ...
    for (let numRisers = minRisers; numRisers <= maxRisers + 3; numRisers++) {
        ...
        // Contremarches par volÃ©e: volÃ©e 1 a une CM de plus (monte sur le palier)
        const risersInFlight1 = Math.max(1, treadsInFlight1) + 1;
        const risersInFlight2 = numRisers - risersInFlight1;
        
        // Dimensions rÃ©elles de chaque cÃ´tÃ© long (girons + palier)
...
function displayCalculatorResults(solutions, params) {
    ...
    if (best.useLandingConfiguration) {
        ...
        html += `<li><strong>VolÃ©e 1 :</strong> ${best.risersInFlight1} contremarches, ${best.treadsInFlight1} girons</li>`;
        html += `<li><strong>VolÃ©e 2 :</strong> ${best.risersInFlight2} contremarches, ${best.treadsInFlight2} girons</li>`;
    }
    
    html += `<li><strong>Largeur :</strong> ${formatValue(stairWidthValue, isMetric, 0)} ${isWidthOk ? 'âœ“' : 'âš  Â'}</li>`;
    html += '</ul></div>';
    ...
    if (isMetric) {
        ...
        if (best.useLandingConfiguration) {
            html += '<br><br><strong>Dimensions rÃ©elles par VolÃ©e :</strong><br>';
            ...
            html += `VolÃ©e 1 : ${best.treadsInFlight1} Ã— ${treadIn.toFixed(6)}" + ${landingIn.toFixed(6)}" = ${flight1RealIn.toFixed(6)}" `;
            const diff1 = flight1RealIn - flight1EnteredIn;
            if (Math.abs(diff1) < 0.001) {
                html += 'âœ“';
            } else {
                html += diff1 > 0 ? `(+${diff1.toFixed(3)}" vs entrÃ©e)` : `(${diff1.toFixed(3)}" vs entrÃ©e)`;
            }
            
            html += '<br>';
            html += `VolÃ©e 2 : ${best.treadsInFlight2} Ã— ${treadIn.toFixed(6)}" + ${landingIn.toFixed(6)}" = ${flight2RealIn.toFixed(6)}" `;
            const diff2 = flight2RealIn - flight2EnteredIn;
            if (Math.abs(diff2) < 0.001) {
                html += 'âœ“';
            } else {
                html += diff2 > 0 ? `(+${diff2.toFixed(3)}" vs entrÃ©e)` : `(${diff2.toFixed(3)}" vs entrÃ©e)`;
            }
        }
    }
    ...
    if (best.useLandingConfiguration) {
        html += `<li>VolÃ©e 1 : ${best.treadsInFlight1} girons + palier (voir dimensions rÃ©elles ci-dessus)</li>`;
        html += `<li>VolÃ©e 2 : palier + ${best.treadsInFlight2} girons (voir dimensions rÃ©elles ci-dessus)</li>`;
    } else {
    ...
    if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
        ...
        html += `<li>VolÃ©e 1 : du plancher infÃ©rieur vers le palier (${best.risersInFlight1} CM, ${best.treadsInFlight1} girons)</li>`;
        html += `<li>VolÃ©e 2 : du palier vers le plancher supÃ©rieur (${best.risersInFlight2} CM, ${best.treadsInFlight2} girons)</li>`;
