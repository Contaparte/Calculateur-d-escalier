...
function generateStraightStairVisualization(stairData) {
    ...
    // Calculer totalRun a partir des girons (plus fiable)
    const totalRun = treadDepth * numTreads;
    
    // Validation des donnees
    if (!numRisers || !numTreads || !riserHeight || !treadDepth || !totalRise || isNaN(totalRun)) {
        return '<div style="padding:20px;color:#c00;background:#fee;border:1px solid #c00;border-radius:4px;">Erreur: donnees insuffisantes pour la visualisation</div>';
    }
    ...
    // Debut du SVG
    let svg = '<svg width="' + W + '" height="' + H + '" xmlns="http://www.w3.org/2000/svg" style="background:#fff;border:1px solid #ccc;border-radius:4px;">';
    ...
    // Titre
    svg += '<text x="15" y="25" style="font:bold 12px Arial;fill:#1b5e20;">Vue de profil - Volee droite</text>';
    
    // Construction du path de l'escalier
    ...
    // Path de remplissage (ferme)
    let fillPath = pathData + ' L' + r(endX + 20) + ',' + endY + ' L' + r(endX + 20) + ',' + startY + ' Z';
    
    // Dessiner le remplissage puis le contour
    svg += '<path d="' + fillPath + '" fill="#c8e6c9" stroke="#2e7d32" stroke-width="1"/>';
    svg += '<path d="' + pathData + '" fill="none" stroke="#1b5e20" stroke-width="2"/>';
    
    // Lignes de niveau (sol et arrivee)
    svg += '<line x1="' + (startX - 25) + '" y1="' + startY + '" x2="' + r(startX + stairW + 5) + '" y2="' + startY + '" stroke="#999" stroke-width="1" stroke-dasharray="4,2"/>';
    svg += '<text x="' + (startX - 30) + '" y="' + (startY + 4) + '" style="font:10px Arial;fill:#666;" text-anchor="end">Sol</text>';
    
    svg += '<line x1="' + (endX - 5) + '" y1="' + endY + '" x2="' + r(endX + 45) + '" y2="' + endY + '" stroke="#999" stroke-width="1" stroke-dasharray="4,2"/>';
    svg += '<text x="' + r(endX + 50) + '" y="' + (endY + 4) + '" style="font:10px Arial;fill:#666;">Arrivee</text>';
    
    // Cotation horizontale (longueur)
    const dimY = startY + 25;
    svg += '<line x1="' + startX + '" y1="' + dimY + '" x2="' + r(startX + stairW) + '" y2="' + dimY + '" stroke="#555" stroke-width="1" marker-start="url(#arrS)" marker-end="url(#arrE)"/>';
    svg += '<line x1="' + startX + '" y1="' + (startY + 2) + '" x2="' + startX + '" y2="' + (dimY + 3) + '" stroke="#555" stroke-width="1"/>';
    svg += '<line x1="' + r(startX + stairW) + '" y1="' + (startY + 2) + '" x2="' + r(startX + stairW) + '" y2="' + (dimY + 3) + '" stroke="#555" stroke-width="1"/>';
    svg += '<text x="' + r(startX + stairW/2) + '" y="' + (dimY + 14) + '" style="font:bold 11px Arial;fill:#333;" text-anchor="middle">' + runText + '</text>';
    svg += '<text x="' + r(startX + stairW/2) + '" y="' + (dimY + 26) + '" style="font:10px Arial;fill:#666;" text-anchor="middle">(' + numTreads + ' girons x ' + treadText + ')</text>';
    
    // Cotation verticale (hauteur)
    const dimX = endX + 32;
    svg += '<line x1="' + dimX + '" y1="' + startY + '" x2="' + dimX + '" y2="' + endY + '" stroke="#555" stroke-width="1" marker-start="url(#arrS)" marker-end="url(#arrE)"/>';
    svg += '<line x1="' + (endX + 2) + '" y1="' + startY + '" x2="' + (dimX + 3) + '" y2="' + startY + '" stroke="#555" stroke-width="1"/>';
    svg += '<line x1="' + (endX + 2) + '" y1="' + endY + '" x2="' + (dimX + 3) + '" y2="' + endY + '" stroke="#555" stroke-width="1"/>';
    
    const midY = r((startY + endY) / 2);
    svg += '<text x="' + (dimX + 8) + '" y="' + (midY - 5) + '" style="font:bold 11px Arial;fill:#333;">' + riseText + '</text>';
    svg += '<text x="' + (dimX + 8) + '" y="' + (midY + 9) + '" style="font:10px Arial;fill:#666;">(' + numRisers + ' CM)</text>';
    
    // Legende en bas
    svg += '<text x="15" y="' + (H - 10) + '" style="font:10px Arial;fill:#666;">CM = ' + riserText + ' | G = ' + treadText + '</text>';
