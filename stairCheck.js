...
function generateUShapedRadiatingVisualization(stairData) {
    ...
    // Profondeur de giron calculée pour chaque partie (peut différer si longueurs différentes)
    const treadDepth1_plan = flight1RectTreads > 0 ? r(flight1Run_plan / flight1RectTreads) : 0;
    const treadDepth2_plan = flight2RectTreads > 0 ? r(flight2Run_plan / flight2RectTreads) : 0;
    ...
    if (flight1RectTreads > 0 && flight1Run_plan > 0) {
        ...
        for (let i = 1; i <= flight1RectTreads; i++) {
            const lineY = r(flight1Y + flight1Run_plan - (i * treadDepth1_plan));
            svg += '<line x1="' + flight1X + '" y1="' + lineY + '" x2="' + r(flight1X + stairWidth_plan) + '" y2="' + lineY + '" stroke="#1b5e20" stroke-width="1.5"/>';
        }
        
        // Numérotation partie 1 - DÉCALÉE vers l'extérieur (25% de la largeur, côté gauche)
        // Seulement première et dernière marche
        const tread1CenterY = r(flight1Y + flight1Run_plan - treadDepth1_plan/2);
        svg += '<text x="' + r(flight1X + stairWidth_plan * 0.25) + '" y="' + r(tread1CenterY + 3) + '" style="font:9px Arial;fill:#333;font-weight:bold;" text-anchor="middle">1</text>';
        const treadLastV1CenterY = r(flight1Y + treadDepth1_plan/2);
    ...
    // ===== LIGNES RAYONNANTES - PIVOT UNIQUE AU CENTRE =====
    // Le pivot est au point d'intersection central (bas des deux coins, entre eux)
    // C'est le point où toutes les contremarches rayonnantes se rejoignent
    const pivotX = r(corner1X + stairWidth_plan + spaceBetween_plan / 2);
    const pivotY = r(corner1Y + stairWidth_plan);
    
    // Tracer TOUTES les lignes de contremarche rayonnantes depuis le pivot central
    // Les lignes forment un éventail de 180° (de gauche à droite en passant par le haut)
    // Pour N marches rayonnantes, on a N+1 lignes de contremarche
    for (let i = 0; i <= totalRadiatingSteps; i++) {
        // Angle: de π (180°, vers la gauche) à 0 (vers la droite)
        // En passant par π/2 (90°, vers le haut)
        const angle = Math.PI * (1 - i / totalRadiatingSteps);
        
        // Calculer le point d'arrivée de la ligne
        // La ligne doit atteindre le bord supérieur de la zone rayonnante
        let x2, y2;
        
        if (Math.abs(Math.sin(angle)) < 0.001) {
            // Ligne quasi-horizontale (0° ou 180°) - va jusqu'au bord vertical
            x2 = pivotX + Math.cos(angle) * stairWidth_plan;
            y2 = pivotY;
        } else {
            // Ligne normale - calculer l'intersection avec le bord supérieur
            // y2 = corner1Y (bord supérieur)
            // t = (corner1Y - pivotY) / (-sin(angle)) où t est le paramètre de la ligne
            const t = (corner1Y - pivotY) / (-Math.sin(angle));
            x2 = pivotX + Math.cos(angle) * t;
            y2 = corner1Y;
        }
    ...
    if (flight2RectTreads > 0 && flight2Run_plan > 0) {
        ...
        for (let i = 1; i <= flight2RectTreads; i++) {
            const lineY = r(flight2Y + ((i - 1) * treadDepth2_plan));
            svg += '<line x1="' + flight2X + '" y1="' + lineY + '" x2="' + r(flight2X + stairWidth_plan) + '" y2="' + lineY + '" stroke="#1b5e20" stroke-width="1.5"/>';
        }
        ...
        const treadFirstV2CenterY = r(flight2Y + treadDepth2_plan/2);
        ...
        const treadLastV2CenterY = r(flight2Y + flight2Run_plan - treadDepth2_plan/2);
        svg += '<text x="' + r(flight2X + stairWidth_plan * 0.75) + '" y="' + r(treadLastV2CenterY + 3) + '" style="font:9px Arial;fill:#333;font-weight:bold;" text-anchor="middle">' + numTreads + '</text>';
