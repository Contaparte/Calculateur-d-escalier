...
function generateLShapedStairVisualization(stairData) {
    ...
    if (isLanding) {
        ...
        // Règle: les lignes partent du coin du CÔTÉ COURT (intérieur) vers le CÔTÉ LONG (extérieur)
        // Le côté court est au coin inférieur-gauche du carré (où les côtés intérieurs des volées se rejoignent)
        const pivotX = cornerX;
        const pivotY = r(cornerY + stairWidth_plan);
        
        if (numRadSteps === 2) {
            // 2 marches à 45 degrés - une ligne à 45 degrés
            // La ligne va du pivot (bas-gauche/côté court) vers le haut-droit (côté long)
            const endX45 = r(pivotX + stairWidth_plan * Math.cos(Math.PI/4));
            const endY45 = r(pivotY - stairWidth_plan * Math.sin(Math.PI/4));
            ...
            // 3 marches à 30 degrés - deux lignes à 30 et 60 degrés depuis l'horizontale
            // Ligne à 30° (plus proche de l'horizontale/volée 1)
            const endX30 = r(pivotX + stairWidth_plan * Math.cos(Math.PI/6));
            const endY30 = r(pivotY - stairWidth_plan * Math.sin(Math.PI/6));
            // Ligne à 60° (plus proche de la verticale/volée 2)
            const endX60 = r(pivotX + stairWidth_plan * Math.cos(Math.PI/3));
            const endY60 = r(pivotY - stairWidth_plan * Math.sin(Math.PI/3));
