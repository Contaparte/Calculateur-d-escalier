...
function generateDancingStepsVisualization(stairData) {
 ...
 // Calcul des dimensions totales basées sur les vraies longueurs et l'angle personnalisé
 let totalWidth, totalHeight;
 if (isUConfig) {
  // Pour U avec angle personnalisé: calculer les projections des deux virages
  // Virage 1: de vertical vers horizontal (angle dancingAngle)
  // Virage 2: de horizontal vers vertical (angle dancingAngle)
  const turnProjection = actualWidth; // Zone de virage occupe la largeur de l'escalier
  totalWidth = rect2Length + turnProjection * 2;
  totalHeight = Math.max(rect1Length, rect3Length) + turnProjection * 2;
 } else {
  // Pour L: tenir compte de l'angle personnalisé pour la projection de la volée 2
  const flight2ProjectionX = rect2Length * Math.sin(angleRad);
  const flight2ProjectionY = rect2Length * Math.cos(angleRad);
  totalWidth = rect1Length + actualWidth + flight2ProjectionX;
  totalHeight = actualWidth + flight2ProjectionY;
 }
 
 // S'assurer qu'on a des dimensions positives
 totalWidth = Math.max(totalWidth, actualWidth * 2);
 totalHeight = Math.max(totalHeight, actualWidth * 2);
 ...
 const rect3_plan = isUConfig ? rect3Length * scale : 0;
 ...
 if (isUConfig) {
  ...
  // Structure: marches rect 1 (vertical) → virage 1 (dancingAngle) → marches rect 2 (horizontal) → virage 2 (dancingAngle) → marches rect 3 (vertical)
  // Parcours: En haut (gauche) → descend → tourne → va à droite → tourne → monte
  
  // Volée 1 (verticale, descend depuis "En haut")
  ...
  // Zone de virage 1 - CORRIGÉ: utiliser dancingAngle au lieu de 90° fixe
  const turn1CenterX = planStartX + stairWidth_plan;
  const turn1CenterY = planStartY + rect1_plan;
  const effectiveInnerR = Math.min(stairWidth_plan * 0.15, 10);
  const effectiveOuterR = stairWidth_plan;
  
  // CORRECTION: Utiliser dancingAngle pour le secteur (sens horaire: de 180° vers 180°+angle)
  const angle1Start = 180; // Vers la gauche (arrivée de volée 1)
  const angle1End = 180 + dancingAngle; // Tourner de dancingAngle en sens horaire
  const largeArc1 = dancingAngle > 180 ? 1 : 0;
  
  // Path du secteur d'anneau - Zone 1
  ...
  pathD1 += ' A ' + r(effectiveInnerR) + ' ' + r(effectiveInnerR) + ' 0 0 0 ' + r(turn1CenterX + effectiveInnerR * Math.cos(angle1Start * Math.PI / 180)) + ' ' + r(turn1CenterY + effectiveInnerR * Math.sin(angle1Start * Math.PI / 180));
  pathD1 += ' Z';
  svg += '<path d="' + pathD1 + '" fill="#ffe0b2" stroke="#ff9800" stroke-width="2"/>';
  
  // Lignes radiales pour zone 1 - utiliser dancingAngle au lieu de 90°
  ...
  svg += '<text x="' + r(turn1CenterX - stairWidth_plan * 0.35) + '" y="' + r(turn1CenterY + stairWidth_plan * 0.65) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">' + dancingStepsPerZone + '×' + Math.round(angleStepDisplay) + '°</text>';
  svg += '<text x="' + r(turn1CenterX - stairWidth_plan * 0.35) + '" y="' + r(turn1CenterY + stairWidth_plan * 0.82) + '" style="font:8px Arial;fill:#e65100;" text-anchor="middle">' + startStep1 + '-' + endStep1 + '</text>';
  
  // Volée 2 (horizontale, vers la droite) - CORRIGÉ: commence après le virage 1
  const v2X = planStartX + stairWidth_plan; // Commence après la zone de virage 1
  const v2Y = planStartY + rect1_plan + stairWidth_plan; // En bas après virage 1
  const v2Width = rect2_plan; // Utiliser la vraie longueur des marches rect 2
  ...
  // Zone de virage 2 - CORRIGÉ: utiliser dancingAngle au lieu de 90° fixe
  const turn2CenterX = planStartX + stairWidth_plan + rect2_plan;
  const turn2CenterY = planStartY + rect1_plan + stairWidth_plan;
  
  // Dessiner secteur 2 (virage selon dancingAngle vers le haut)
  const angle2Start = 180 + dancingAngle; // Fin du virage 1 = début du parcours horizontal
  const angle2End = 180 + dancingAngle + dancingAngle; // Tourner de dancingAngle supplémentaire
  ...
  // Lignes radiales pour zone 2 - utiliser dancingAngle
  for (let i = 0; i <= dancingStepsPerZone; i++) {
  ...
  svg += '<text x="' + r(turn2CenterX + stairWidth_plan * 0.65) + '" y="' + r(turn2CenterY + stairWidth_plan * 0.35) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">' + dancingStepsPerZone + '×' + Math.round(angleStepDisplay) + '°</text>';
  svg += '<text x="' + r(turn2CenterX + stairWidth_plan * 0.65) + '" y="' + r(turn2CenterY + stairWidth_plan * 0.52) + '" style="font:8px Arial;fill:#e65100;" text-anchor="middle">' + startStep2b + '-' + endStep2b + '</text>';
  
  // Volée 3 (verticale, monte vers le haut) - CORRIGÉ: après le virage 2
  const v3X = planStartX + stairWidth_plan + rect2_plan + stairWidth_plan; // Après virage 2
  const v3Y = planStartY + rect1_plan + stairWidth_plan - rect3_plan; // Commence à la bonne hauteur
  ...
  // Flèche de montée
  const arrowStartY = r(v1Y + v1Height/2);
  ...
  svg += '<line x1="' + r(v1X + stairWidth_plan/2) + '" y1="' + arrowStartY + '" x2="' + r(v1X + stairWidth_plan/2) + '" y2="' + r(v2Y + stairWidth_plan/2) + '" stroke="#1b5e20" stroke-width="1.5"/>';
  ...
  // Cotations - largeur totale
  const totalWidthDisplay = rect2_plan + stairWidth_plan * 3;
  ...
  svg += '<text x="' + r(planStartX + totalWidthDisplay/2) + '" y="' + r(dimY + 14) + '" style="font:10px Arial;fill:#333;" text-anchor="middle">' + formatValueForPlan(secondFlightRun + actualWidth, isMetric) + '</text>';
  
 } else {
  // ========== CONFIGURATION L (1 VIRAGE) ==========
  // CORRIGÉ: Géométrie correcte avec centre au coin intérieur et volée 2 alignée
  // Structure: marches rect 1 (horizontal) → zone de virage (sens horaire) → marches rect 2 (vertical)
  
  // Volée 1 (horizontale, de gauche à droite, en HAUT)
  const v1X = planStartX;
  const v1Y = planStartY;
  ...
  // CORRECTION GÉOMÉTRIE: Le centre du virage est au coin INTÉRIEUR de l'escalier
  // Pour un virage sens horaire: le centre est en haut à droite de la volée 1 (coin supérieur-droit)
  const turnCenterX = v1X + rect1_plan; // Fin de la volée 1
  const turnCenterY = v1Y; // Bord supérieur (coin intérieur)
  
  // Rayons: le rayon extérieur = largeur de l'escalier (va vers le bas/extérieur)
  const effectiveInnerR = Math.min(stairWidth_plan * 0.15, 10);
  const effectiveOuterR = stairWidth_plan;
  
  // Angles: virage sens horaire de 90° (vers le bas depuis le coin) vers 90°+angle
  // Dans SVG: 90° = vers le bas, 0° = vers la droite
  const angleStart = 90; // Vers le bas (perpendiculaire à la volée 1)
  const angleEnd = 90 + dancingAngle; // Tourner de dancingAngle en sens horaire
  const angleStep = dancingAngle / dancingStepsPerZone;
  const largeArcFlag = dancingAngle > 180 ? 1 : 0;
  
  // Points du secteur d'anneau
  const startOuterX = turnCenterX + effectiveOuterR * Math.cos(angleStart * Math.PI / 180);
  const startOuterY = turnCenterY + effectiveOuterR * Math.sin(angleStart * Math.PI / 180);
  const endOuterX = turnCenterX + effectiveOuterR * Math.cos(angleEnd * Math.PI / 180);
  const endOuterY = turnCenterY + effectiveOuterR * Math.sin(angleEnd * Math.PI / 180);
  const startInnerX = turnCenterX + effectiveInnerR * Math.cos(angleStart * Math.PI / 180);
  const startInnerY = turnCenterY + effectiveInnerR * Math.sin(angleStart * Math.PI / 180);
  const endInnerX = turnCenterX + effectiveInnerR * Math.cos(angleEnd * Math.PI / 180);
  const endInnerY = turnCenterY + effectiveInnerR * Math.sin(angleEnd * Math.PI / 180);
  ...
  // Lignes radiales pour les contremarches (sens horaire)
  for (let i = 0; i <= dancingStepsPerZone; i++) {
  ...
  const annotationAngle = (angleStart + dancingAngle/2) * Math.PI / 180;
  const annotationR = effectiveOuterR * 0.6;
  const annotationX = turnCenterX + annotationR * Math.cos(annotationAngle);
  const annotationY = turnCenterY + annotationR * Math.sin(annotationAngle);
  ...
  // CORRECTION VOLÉE 2: Doit être ALIGNÉE avec la largeur de l'escalier
  // La volée 2 part du coin extérieur et descend dans la direction de l'angle final
  // Direction de la volée 2: perpendiculaire au rayon final (tangente au cercle)
  const flight2DirAngle = angleEnd + 90; // Direction tangente au cercle
  const flight2DirRad = flight2DirAngle * Math.PI / 180;
  
  // CORRECTION: Point de départ = intersection du bord extérieur du secteur avec la ligne d'alignement
  // La volée 2 doit être alignée avec le bord droit de l'escalier (x = turnCenterX + stairWidth_plan pour angle < 90°)
  // Pour un angle de 90°, la volée 2 est parfaitement verticale à x = turnCenterX
  // Pour un angle < 90°, la volée 2 est inclinée
  
  // Point de départ de la volée 2 sur le bord extérieur du secteur
  const v2StartX = endOuterX;
  const v2StartY = endOuterY;
  
  // Point de fin de la volée 2
  const v2EndX = v2StartX + rect2_plan * Math.cos(flight2DirRad);
  const v2EndY = v2StartY + rect2_plan * Math.sin(flight2DirRad);
  
  // Direction perpendiculaire pour la largeur de l'escalier (vers l'intérieur)
  const perpAngle = (flight2DirAngle - 90) * Math.PI / 180;
  
  if (flight2RectTreads > 0 && rect2_plan > 0) {
   // Les 4 coins de la volée 2
   // CORRECTION: Le côté intérieur doit toucher le rayon intérieur du secteur
   const v2P1 = { x: v2StartX, y: v2StartY }; // Coin extérieur début
   const v2P2 = { x: v2EndX, y: v2EndY }; // Coin extérieur fin
   const v2P3 = { x: v2EndX + stairWidth_plan * Math.cos(perpAngle), y: v2EndY + stairWidth_plan * Math.sin(perpAngle) }; // Coin intérieur fin
   const v2P4 = { x: v2StartX + stairWidth_plan * Math.cos(perpAngle), y: v2StartY + stairWidth_plan * Math.sin(perpAngle) }; // Coin intérieur début
   ...
   // Lignes des contremarches
   const treadLen = rect2_plan / flight2RectTreads;
   for (let i = 1; i < flight2RectTreads; i++) {
    const lineStartX = v2StartX + i * treadLen * Math.cos(flight2DirRad);
    const lineStartY = v2StartY + i * treadLen * Math.sin(flight2DirRad);
  ...
  svg += '<line x1="' + arrowStartX + '" y1="' + arrowY + '" x2="' + r(turnCenterX) + '" y2="' + arrowY + '" stroke="#1b5e20" stroke-width="1.5"/>';
  
  // Segment vers la volée 2
  const arrowEndX = v2EndX + stairWidth_plan/2 * Math.cos(perpAngle);
  const arrowEndY = v2EndY + stairWidth_plan/2 * Math.sin(perpAngle);
  svg += '<line x1="' + r(turnCenterX) + '" y1="' + arrowY + '" x2="' + r(arrowEndX) + '" y2="' + r(arrowEndY) + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrDMontee)"/>';
  ...
  svg += '<text x="' + r(v1X + rect1_plan/2) + '" y="' + r(dimY1 - 5) + '" style="font:10px Arial;fill:#333;" text-anchor="middle">' + formatValueForPlan(firstFlightRun, isMetric) + '</text>';
  
  // Cotation verticale (longueur volée 2) - à droite
  const dimX2 = r(Math.max(turnCenterX + stairWidth_plan, v2EndX) + 20);
  const dimY2Start = r(v1Y + stairWidth_plan);
  const dimY2End = r(Math.max(v2EndY, v2EndY + stairWidth_plan * Math.sin(perpAngle)));
  svg += '<line x1="' + dimX2 + '" y1="' + dimY2Start + '" x2="' + dimX2 + '" y2="' + dimY2End + '" stroke="#555" stroke-width="1" marker-start="url(#arrDS)" marker-end="url(#arrDE)"/>';
  svg += '<text x="' + r(dimX2 + 5) + '" y="' + r((parseFloat(dimY2Start) + parseFloat(dimY2End))/2 + 4) + '" style="font:10px Arial;fill:#333;">' + formatValueForPlan(secondFlightRun, isMetric) + '</text>';
 }
