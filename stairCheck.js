 ...
 * =====================================================================
 * 
 * Conforme au Code de construction du Québec, Chapitre I ââ‚¬" Bâtiment
 * et Code national du bâtiment ââ‚¬" Canada 2020 (modifié)
 * 
 * Caractéristiques:
...
function generateUShapedStairVisualization(stairData) {
    ...
    // Profondeur de giron uniforme en pixels (basée sur la profondeur réelle)
    ...
    // Lignes des girons volée 1 (horizontales) - tracées avec la profondeur uniforme
    if (flight1Treads > 0) {
        for (let i = 1; i <= flight1Treads; i++) {
            // Position Y: du bas vers le haut, chaque giron a la même profondeur
    ...
    // Lignes des girons volée 2 (horizontales) - tracées avec la profondeur uniforme, alignées avec volée 1
    // Pour aligner les lignes avec la volée 1 quand les girons sont identiques:
    // V1 trace depuis le bas (i*tread depuis le bas = run - i*tread depuis le haut)
    // V2 trace depuis le haut (aligner avec V1 en commençant à (i-1)*tread depuis le haut)
    if (flight2Treads > 0) {
        for (let i = 1; i <= flight2Treads; i++) {
            // Position Y: alignée avec les lignes de la volée 1
            // i=1 donne Y = flight2Y (au niveau du palier, aligné avec la dernière ligne de V1)
    ...
    const rightDimStart = planStartY;
    const rightDimEnd = r(flight2Y + flight2Run_plan);
    const rightDimCenter = r((rightDimStart + rightDimEnd) / 2);
    
    // Cotation TOTALE côté gauche (volée 1 + palier)
    ...
    // Cotation TOTALE côté droit (volée 2 + palier)
    ...
    svg += '<line x1="' + dimX2 + '" y1="' + rightDimStart + '" x2="' + dimX2 + '" y2="' + rightDimEnd + '" stroke="#555" stroke-width="1" marker-start="url(#arrUS)" marker-end="url(#arrUE)"/>';
    svg += '<text x="' + (dimX2 + 5) + '" y="' + rightDimCenter + '" style="font:10px Arial;fill:#333;" text-anchor="middle" dominant-baseline="middle" transform="rotate(-90 ' + (dimX2 + 5) + ' ' + rightDimCenter + ')">' + totalRightText + '</text>';
    
    // Cotation largeur totale (en bas)
    const dimY1 = r(flight1Y + Math.max(flight1Run_plan, flight2Run_plan) + 20);
    const totalWidthText = formatValuePrecise(actualLandingWidth, isMetric);
    ...
    // Annotations girons (dans les volées)
    ...
    const legendY1 = H - 24;
    const legendY2 = H - 10;
    const landingDepthText = formatValuePrecise(actualLandingDepth, isMetric);
    
    const legend1 = 'Légende: CM = ' + riserText + '  |  G = ' + treadText + '  |  Largeur = ' + widthText;
    svg += '<text x="15" y="' + legendY1 + '" style="font:9px Arial;fill:#555;">' + legend1 + '</text>';
    
    const legend2 = 'Hauteur = ' + riseText + ' (' + numRisers + ' CM)  |  Palier = ' + landingDepthText + ' × ' + totalWidthText;
    svg += '<text x="15" y="' + legendY2 + '" style="font:9px Arial;fill:#555;">' + legend2 + '</text>';
...
function displayCalculatorResults(solutions, params) {
    ...
    html += `<li>${best.stepRule.rule2.isValid ? 'âœ“' : '○'} G × H = ${best.stepRule.rule2.value.toFixed(2)} (${best.stepRule.rule2.range})</li>`;
...
function displayVerificationResults(params) {
    ...
    html += `<li>${stepRule.rule2.isValid ? 'âœ“' : '✗'} Règle 2 (G×H) : ${stepRule.rule2.value.toFixed(2)} po² (71-74)</li>`;
