...
const CNB_LIMITS = {
    ...
    part3: {
        common: {
            ...
            minWidthUnder3: 900,    // CNB 3.4.3.2 Tableau - â"°Â¤3 étages
    ...
    // =====================================================================
    // ESCALIERS HÉLICOÃDAUX (CNB 2020, Article 9.8.4.7)
    // =====================================================================
    spiral: {
...
function generateUShapedStairVisualization(stairData) {
    ...
    if (flight1Treads > 0) {
        for (let i = 1; i <= flight1Treads; i++) {
            // Position Y: du bas vers le haut, chaque giron a la MÊME profondeur que dans volée 2
    ...
    if (flight2Treads > 0) {
        ...
        // Annotation nombre de girons rectangulaires volée 2 - MÊME DIRECTION que volée 1 (rotation -90)
 ...
 * Configuration: 3 volées droites + 2 zones de marches rayonnantes (coins à 90°)
...
function generateUShapedRadiatingVisualization(stairData) {
    ...
    if (numRadiatingStepsPerCorner === 2) {
        ...
        svg += '<text x="' + r(corner1X + stairWidth_plan * 0.65) + '" y="' + r(corner1Y + stairWidth_plan * 0.35) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">2×45°</text>';
    } else {
        // 3 marches de 30°
        ...
        svg += '<text x="' + r(corner1X + stairWidth_plan * 0.65) + '" y="' + r(corner1Y + stairWidth_plan * 0.35) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">3×30°</text>';
    }
    ...
    if (numRadiatingStepsPerCorner === 2) {
        ...
        svg += '<text x="' + r(corner2X + stairWidth_plan * 0.35) + '" y="' + r(corner2Y + stairWidth_plan * 0.35) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">2×45°</text>';
    } else {
        // 3 marches de 30°
        const angle30 = Math.PI / 6;
        const angle60 = Math.PI / 3;
        ...
        svg += '<text x="' + r(corner2X + stairWidth_plan * 0.35) + '" y="' + r(corner2Y + stairWidth_plan * 0.35) + '" style="font:bold 9px Arial;fill:#e65100;" text-anchor="middle">3×30°</text>';
    }
    ...
    svg += '<text x="' + (legendX + cellW * 2.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + widthText + '</text>';
    svg += '<text x="' + (legendX + cellW * 3.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + riseText + '</text>';
    svg += '<text x="' + (legendX + cellW * 4.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + totalRadiatingSteps + '×' + radiatingAngle + '°</text>';
...
function displayCalculatorResults(solutions, params) {
    ...
    if (!isWidthOk) {
        html += `<div class="warning"><p>⚠Â  Largeur ${formatValue(stairWidthValue, isMetric, 0)} inférieure au minimum requis (${formatValue(limits.minWidth, isMetric, 0)})</p></div>`;
    }
    if (!isRiserOk) {
        html += `<div class="warning"><p>⚠Â  Hauteur de contremarche ${formatValuePrecise(best.riserHeight, isMetric)} hors limites CNB (${formatValue(limits.minRiser, isMetric, 0)} à ${formatValue(limits.maxRiser, isMetric, 0)})</p></div>`;
    }
    if (!isTreadOk) {
        if (hasMaxTreadLimit) {
            html += `<div class="warning"><p>⚠Â  Profondeur de giron ${formatValuePrecise(best.treadDepth, isMetric)} hors limites CNB (${formatValue(limits.minTread, isMetric, 0)} à ${formatValue(limits.maxTread, isMetric, 0)})</p></div>`;
        } else {
            html += `<div class="warning"><p>⚠Â  Profondeur de giron ${formatValuePrecise(best.treadDepth, isMetric)} inférieure au minimum CNB (${formatValue(limits.minTread, isMetric, 0)})</p></div>`;
        }
    }
    ...
    if (best.useLandingConfiguration) {
        ...
        if (best.flight1Height && best.flight2Height) {
            ...
            if (!flight1Ok || !flight2Ok) {
                html += '</ul></div>';
                html += '<div class="warning"><p><strong>âÅ¡Â  Non-conformité CNB 9.8.3.3 :</strong> ';
                if (!flight1Ok && !flight2Ok) {
    ...
    if (isMetric) {
        ...
        html += riseError < 0.01 ? '✓ Exact' : `⚠Â   Écart ${riseError.toFixed(4)} mm`;
        ...
        if (best.useLandingConfiguration) {
            ...
            html += treadError < 0.01 ? '✓ Exact' : `⚠Â   Écart ${treadError.toFixed(4)} mm`;
        }
    } else {
    ...
    // Visualisation graphique pour escalier en U avec marches rayonnantes
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performCalculation() {
        ...
        if (stairConfigValue === 'l_shaped') {
            ...
            // Validation: Largeur palier â≥ 2 × largeur escalier
     ...
     * Règles:
     * - Largeur palier â≥ 2 × largeur escalier (géométrie)
    ...
    function validateUShapedInputs() {
        ...
        // Valider: Largeur palier â≥ 2 × largeur escalier
