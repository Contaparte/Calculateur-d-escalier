...
function generateUShapedRadiatingVisualization(stairData) {
    ...
    const stepsPerSeries = radiatingAngle === 30 ? 3 : 2;
    const totalRadiatingSteps = stepsPerSeries * 2; // 4 marches (45°) ou 6 marches (30°)
    ...
    const totalPlanWidth = actualLandingWidth;
    const maxFlightRun = Math.max(actualFlight1Run, actualFlight2Run);
    const totalPlanHeight = maxFlightRun + actualWidth; // volées + zone rayonnante
    ...
    // ===== ZONE DE MARCHES RAYONNANTES (en haut, faisant le virage de 180°) =====
    const radiatingZoneX = planStartX;
    const radiatingZoneY = planStartY;
    const radiatingZoneWidth = totalW_plan;
    const radiatingZoneHeight = stairWidth_plan;
    
    // Fond de la zone rayonnante
    svg += '<rect x="' + radiatingZoneX + '" y="' + radiatingZoneY + '" width="' + radiatingZoneWidth + '" height="' + radiatingZoneHeight + '" fill="#fff9c4" stroke="#f9a825" stroke-width="2"/>';
    
    // Centre de rotation (milieu de la zone rayonnante)
    const pivotX = r(radiatingZoneX + radiatingZoneWidth / 2);
    const pivotY = r(radiatingZoneY + radiatingZoneHeight);
    
    // Dessiner les lignes des marches rayonnantes
    // Les marches rayonnantes font le virage de 180° autour du point central
    const innerRadius = 0;
    const outerRadius = radiatingZoneHeight;
    
    // Angle de départ: 0° (vers la droite depuis le pivot)
    // On va de 180° (gauche) à 0° (droite) en sens horaire
    for (let i = 1; i < totalRadiatingSteps; i++) {
        const angle = Math.PI - (i * (Math.PI / totalRadiatingSteps));
        const x1 = r(pivotX + Math.cos(angle) * innerRadius);
        const y1 = r(pivotY + Math.sin(angle) * innerRadius);
        const x2 = r(pivotX + Math.cos(angle) * outerRadius);
        const y2 = r(pivotY + Math.sin(angle) * outerRadius);
        svg += '<line x1="' + x1 + '" y1="' + y1 + '" x2="' + x2 + '" y2="' + y2 + '" stroke="#f57f17" stroke-width="1.5"/>';
    }
    
    // Annotation des marches rayonnantes
    svg += '<text x="' + pivotX + '" y="' + r(radiatingZoneY + radiatingZoneHeight/2) + '" style="font:bold 10px Arial;fill:#e65100;" text-anchor="middle">' + totalRadiatingSteps + String.fromCharCode(215) + radiatingAngle + String.fromCharCode(176) + '</text>';
    ...
    if (flight1RectTreads > 0) {
        ...
        // Numéros des marches volée 1
        for (let i = 1; i <= flight1RectTreads; i++) {
            const stepY = r(flight1Y + flight1Run_plan - ((i - 0.5) * treadDepth_plan));
            svg += '<text x="' + r(flight1X + stairWidth_plan/2) + '" y="' + stepY + '" style="font:bold 9px Arial;fill:#1b5e20;" text-anchor="middle" dominant-baseline="middle">' + i + '</text>';
        }
        
        // Annotation volée 1
        svg += '<text x="' + r(flight1X + stairWidth_plan + 5) + '" y="' + r(flight1Y + flight1Run_plan/2) + '" style="font:9px Arial;fill:#555;" transform="rotate(90 ' + r(flight1X + stairWidth_plan + 5) + ' ' + r(flight1Y + flight1Run_plan/2) + ')">' + flight1RectTreads + ' girons rect.</text>';
    }
    ...
    if (flight2RectTreads > 0) {
        ...
        // Numéros des marches volée 2
        const startNum = flight1RectTreads + totalRadiatingSteps + 1;
        for (let i = 1; i <= flight2RectTreads; i++) {
            const stepY = r(flight2Y + ((i - 0.5) * treadDepth_plan));
            svg += '<text x="' + r(flight2X + stairWidth_plan/2) + '" y="' + stepY + '" style="font:bold 9px Arial;fill:#1b5e20;" text-anchor="middle" dominant-baseline="middle">' + (startNum + i - 1) + '</text>';
        }
        
        // Annotation volée 2
        svg += '<text x="' + r(flight2X - 5) + '" y="' + r(flight2Y + flight2Run_plan/2) + '" style="font:9px Arial;fill:#555;" transform="rotate(-90 ' + r(flight2X - 5) + ' ' + r(flight2Y + flight2Run_plan/2) + ')">' + flight2RectTreads + ' girons rect.</text>';
    }
    
    // Annotation numéros marches rayonnantes
    const radStartNum = flight1RectTreads + 1;
    const radEndNum = flight1RectTreads + totalRadiatingSteps;
    svg += '<text x="' + pivotX + '" y="' + r(radiatingZoneY + radiatingZoneHeight/2 + 12) + '" style="font:9px Arial;fill:#e65100;" text-anchor="middle">' + radStartNum + '-' + radEndNum + '</text>';
    ...
    // Segment 2: Traverse horizontalement (zone rayonnante)
    svg += '<line x1="' + arrowX1 + '" y1="' + arrowY1End + '" x2="' + arrowX1 + '" y2="' + r(radiatingZoneY + radiatingZoneHeight/2) + '" stroke="#1b5e20" stroke-width="1.5"/>';
    svg += '<line x1="' + arrowX1 + '" y1="' + r(radiatingZoneY + radiatingZoneHeight/2) + '" x2="' + arrowX2 + '" y2="' + r(radiatingZoneY + radiatingZoneHeight/2) + '" stroke="#1b5e20" stroke-width="1.5"/>';
    svg += '<line x1="' + arrowX2 + '" y1="' + r(radiatingZoneY + radiatingZoneHeight/2) + '" x2="' + arrowX2 + '" y2="' + arrowY2Start + '" stroke="#1b5e20" stroke-width="1.5"/>';
    ...
    svg += '<text x="' + (legendX + cellW * 2.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + widthText + '</text>';
    svg += '<text x="' + (legendX + cellW * 3.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + riseText + '</text>';
    svg += '<text x="' + (legendX + cellW * 4.5) + '" y="' + (legendY + cellH + 14) + '" style="font:9px Arial;fill:#555;" text-anchor="middle">' + totalRadiatingSteps + String.fromCharCode(215) + radiatingAngle + String.fromCharCode(176) + '</text>';
