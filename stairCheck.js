 ...
 * =====================================================================
 * 
 * Conforme au Code de construction du QuÃ©bec, Chapitre I "â€œ BÃ¢timent
 * et Code national du bÃ¢timent "â€œ Canada 2020 (modifiÃ©)
 * 
 * CaractÃ©ristiques:
...
const CNB_LIMITS = {
    ...
    part3: {
        common: {
            ...
            minWidthUnder3: 900,    // CNB 3.4.3.2 Tableau - â‰¤â‰¤3 Ã©tages
    ...
    // =====================================================================
    // ESCALIERS HÃ‰LICOÃ ÂDAUX (CNB 2020, Article 9.8.4.7)
    // =====================================================================
    spiral: {
        minWidth: 660,              // CNB 9.8.4.7.(1)b) - min 660 mm entre mains courantes
        minTreadAt300: 190,         // CNB 9.8.4.7.(1)d)i) - min 190 mm Ã Â  300 mm de l'axe
...
function metricToImperial(mmValue) {
    ...
    // GÃ©rer le cas oÃ Â¹ wholeInches >= 12 (par exemple, 3' 12" devient 4'-0")
    if (wholeInches >= 12) {
...
function calculateLShapedWithRadiating(params) {
    ...
    // DÃ©tecter si les marches rayonnantes sont Ã Â  l'extrÃ©mitÃ©
    // (quand l'une des dimensions est Ã©gale Ã Â  la largeur de l'escalier)
    ...
    if (radiatingAtEnd || radiatingAtStart) {
        // CAS SPÃ‰CIAL: Marches rayonnantes Ã Â  l'extrÃ©mitÃ©
        ...
        // CAS STANDARD: Marches rayonnantes Ã Â  l'intersection
...
function calculateUShapedStair(params) {
    ...
    switch (uShapedConfig) {
        ...
        case 'rect_landing_radiating_rect':
            // Configuration palier + marches rayonnantes (3 volÃ©es)
            numLandings = 1;
            availableForTreads = flight1Run + flight2Run + (flight3Run || 0) + (landingLength * 0.25);
            break;
        case 'rect_radiating_landing_rect':
            // Configuration marches rayonnantes + palier (3 volÃ©es)
            numLandings = 1;
            availableForTreads = flight1Run + flight2Run + (flight3Run || 0) + (landingLength * 0.25);
            break;
        case 'rect_radiating_landing_radiating_rect':
            // Configuration marches rayonnantes + palier + marches rayonnantes (3 volÃ©es)
            numLandings = 1;
    ...
    // DÃ©terminer le nombre de marches rayonnantes selon la configuration
    let totalRadiatingSteps = 0;
    // Note: rect_radiating_rect supprimé car non conforme au CNB 9.8.4.6 (max 90° par série de marches rayonnantes)
    const hasRadiating = uShapedConfig === 'rect_landing_radiating_rect' || uShapedConfig === 'rect_radiating_landing_rect' ||
                         uShapedConfig === 'rect_radiating_landing_radiating_rect';
    if (hasRadiating) {
        // Pour 180Â° de rotation totale avec marches rayonnantes:
        // 45Â°: 2 marches par coin Ã— 2 coins = 4 marches
        // 30Â°: 3 marches par coin Ã— 2 coins = 6 marches
    ...
    for (let numRisers = minRisers; numRisers <= maxRisers + 3; numRisers++) {
        ...
        // Donc le giron est calculÃ© sur les girons RECTANGULAIRES seulement
...
function calculateSpiralStair(params) {
    ...
    // Pour l'hÃ©licoÃ¯dal, le giron est mesurÃ© Ã Â  300mm de l'axe de la main courante
    const measurementRadius = innerRadius + 300;
    
    // Longueur de l'arc Ã Â  300mm
...
function generateLShapedStairVisualization(stairData) {
    ...
    if (isLanding) {
        ...
        // Annotation nombre de girons rectangulaires volÃ©e 2 (Ã Â  gauche de la volÃ©e)
...
function generateUShapedStairVisualization(stairData) {
    ...
    // IMPORTANT: Les dimensions des volÃ©es sont calculÃ©es Ã Â  partir du nombre de girons Ã— profondeur uniforme
    ...
    // ===== VOLÃ‰E 2 (droite, descend de haut vers bas = sens opposÃ© Ã Â  volÃ©e 1) =====
...
/**
 * GÃ©nÃ¨re une visualisation SVG d'un escalier en U avec marches rayonnantes (vue en plan)
 * Configuration: 2 volÃ©es droites + 2 coins de marches rayonnantes (90Â° chacun = 180Â° total)
 * Conforme au CNB 9.8.4.6: marches rayonnantes Ã Â  30Â° ou 45Â°, max 90Â° par sÃ©rie
 */
function generateUShapedRadiatingVisualization(stairData) {
    ...
    // Pour 180Â°, on a besoin de 2 sÃ©ries de 90Â° chacune
    const stepsPerCorner = radiatingAngle === 30 ? 3 : 2;
    const totalRadiatingSteps = stepsPerCorner * 2; // 4 marches (45Â°) ou 6 marches (30Â°)
    
    // Calcul des girons rectangulaires basÃ© sur les longueurs rÃ©elles des parties
    // Si les longueurs sont diffÃ©rentes, rÃ©partir proportionnellement
    const totalRectRun = actualFlight1Run + actualFlight2Run;
    const numRectTreads = Math.max(0, numTreads - totalRadiatingSteps);
    
    // RÃ©partition des girons selon les longueurs des parties
    ...
    // Calculer l'Ã©chelle - utiliser les longueurs rÃ©elles
    const totalPlanWidth = actualLandingWidth;
    const maxFlightRun = Math.max(actualFlight1Run, actualFlight2Run);
    ...
    // Un espace correspondrait Ã  un palier, ce qui est une autre configuration
    const spaceBetween = 0;
    const spaceBetween_plan = 0;
    
    // Longueurs des parties basÃ©es sur les valeurs RÃ‰ELLES entrÃ©es par l'utilisateur
    const flight1Run_plan = r(actualFlight1Run * scalePlan);
    const flight2Run_plan = r(actualFlight2Run * scalePlan);
    
    // CNB 9.8.4.4.(3) et 9.8.4.5.(1): Dans une mÃªme volÃ©e, le giron doit Ãªtre UNIFORME
    // Pour cette configuration sans palier = UNE SEULE VOLÃ‰E
    // Donc TOUS les girons rectangulaires ont la MÃŠME profondeur (treadDepth calculÃ© globalement)
    const uniformTreadDepth = treadDepth; // Giron uniforme calculÃ© par le calculateur
    const uniformTreadDepth_plan = r(uniformTreadDepth * scalePlan);
    
    // RÃ©partition des girons rectangulaires entre les deux parties
    // basÃ©e sur les longueurs entrÃ©es par l'utilisateur
    // Note: totalRectRun et numRectTreads dÃ©jÃ  calculÃ©s plus haut
    ...
    // pour que les lignes de contremarche soient correctement espacÃ©es
    ...
    // Position de dÃ©part centrÃ©e
    // Utiliser les dimensions visuelles (basÃ©es sur giron uniforme) pour le centrage
    ...
    if (flight1RectTreadsCalc > 0) {
        // Hauteur du rectangle = nombre de girons Ã— giron uniforme (pour cohÃ©rence visuelle)
    ...
    // Les deux coins Ã©tant adjacents, le pivot est Ã  leur intersection
    const pivotX = corner2X; // = corner1X + stairWidth_plan
    const pivotY = r(corner1Y + stairWidth_plan);
    
    // COIN 1: Lignes de 180Â° (gauche) Ã  90Â° (haut)
    ...
    // COIN 2: Lignes de 90Â° (haut) Ã  0Â° (droite) - la ligne Ã  90Â° est dÃ©jÃ  tracÃ©e
    ...
    if (flight2RectTreadsCalc > 0) {
        // Hauteur du rectangle = nombre de girons Ã— giron uniforme
    ...
    // "En haut" centrÃ© sous la partie 1 (entrÃ©e)
    svg += '<text x="' + arrowX1 + '" y="' + r(arrowY1Start + 14) + '" style="font:bold 9px Arial;fill:#1b5e20;" text-anchor="middle">En haut</text>';
    
    // ===== COTATIONS =====
    // Cotation cÃ´tÃ© gauche (hauteur totale)
    const leftDimStart = planStartY;
    const leftDimEnd = r(flight1Y + flight1Run_visual);
    const leftDimCenter = r((leftDimStart + leftDimEnd) / 2);
    
    const dimX1 = r(flight1X - 30);
    // Afficher la dimension rÃ©elle (giron uniforme Ã— nombre de girons + largeur)
    ...
    // Cotation largeur totale (en bas) - 2 Ã— largeur car coins adjacents
    ...
    // Valeurs - utiliser String.fromCharCode pour les caractÃ¨res spÃ©ciaux
...
function generateUShapedMixedVisualization(stairData) {
    ...
    const actualFlight3Run = flight3Run || 1000;
    const actualLandingDepth = landingDepth || actualWidth;
    const actualLandingWidth = landingWidth || (2 * actualWidth);
    ...
    // ========== VUE EN PLAN ==========
    const margin = { left: 70, right: 70, top: 55, bottom: 90 };
    const planW = W - margin.left - margin.right;
    const planH = H - margin.top - margin.bottom;
    ...
    const totalPlanHeight = maxFlightRun + actualLandingDepth + actualWidth; // volées + palier + zone rayonnante
    
    const scalePlan = Math.min(planW / totalPlanWidth, planH / totalPlanHeight) * 0.60;
    ...
    if (configType === 'rect_landing_radiating_rect') {
        ...
        const landingX = planStartX;
        const landingY = planStartY;
        svg += '<rect x="' + landingX + '" y="' + landingY + '" width="' + stairWidth_plan + '" height="' + landingDepth_plan + '" fill="#fff9c4" stroke="#f9a825" stroke-width="2"/>';
        svg += '<text x="' + r(landingX + stairWidth_plan/2) + '" y="' + r(landingY + landingDepth_plan/2 + 4) + '" style="font:italic 10px Arial;fill:#f57f17;" text-anchor="middle">Palier</text>';
        
        // Zone de marches rayonnantes (en haut à droite, coin 90°)
        const cornerX = r(planStartX + stairWidth_plan);
        const cornerY = planStartY;
        ...
        const arrowX1 = r(flight1X + stairWidth_plan/2);
        const arrowX2 = r(flight2X + stairWidth_plan/2);
        svg += '<line x1="' + arrowX1 + '" y1="' + r(flight1Y + flight1Run_plan) + '" x2="' + arrowX1 + '" y2="' + r(landingY + landingDepth_plan/2) + '" stroke="#1b5e20" stroke-width="1.5"/>';
        svg += '<line x1="' + arrowX1 + '" y1="' + r(landingY + landingDepth_plan/2) + '" x2="' + r(cornerX + stairWidth_plan * 0.3) + '" y2="' + r(cornerY + stairWidth_plan * 0.7) + '" stroke="#1b5e20" stroke-width="1.5"/>';
        svg += '<line x1="' + r(cornerX + stairWidth_plan * 0.3) + '" y1="' + r(cornerY + stairWidth_plan * 0.7) + '" x2="' + arrowX2 + '" y2="' + r(flight2Y) + '" stroke="#1b5e20" stroke-width="1.5"/>';
        ...
        const arrowX1 = r(flight1X + stairWidth_plan/2);
        const arrowX2 = r(flight2X + stairWidth_plan/2);
        svg += '<line x1="' + arrowX1 + '" y1="' + r(flight1Y + flight1Run_plan) + '" x2="' + arrowX1 + '" y2="' + r(cornerY + stairWidth_plan * 0.7) + '" stroke="#1b5e20" stroke-width="1.5"/>';
        svg += '<line x1="' + arrowX1 + '" y1="' + r(cornerY + stairWidth_plan * 0.7) + '" x2="' + r(landingX + stairWidth_plan * 0.5) + '" y2="' + r(landingY + landingDepth_plan * 0.5) + '" stroke="#1b5e20" stroke-width="1.5"/>';
        svg += '<line x1="' + r(landingX + stairWidth_plan * 0.5) + '" y1="' + r(landingY + landingDepth_plan * 0.5) + '" x2="' + arrowX2 + '" y2="' + r(flight2Y) + '" stroke="#1b5e20" stroke-width="1.5"/>';
        svg += '<line x1="' + arrowX2 + '" y1="' + r(flight2Y) + '" x2="' + arrowX2 + '" y2="' + r(flight2Y + flight3Run_plan) + '" stroke="#1b5e20" stroke-width="1.5" marker-end="url(#arrUMMontee)"/>';
        svg += '<text x="' + arrowX1 + '" y="' + r(flight1Y + flight1Run_plan + 14) + '" style="font:bold 9px Arial;fill:#1b5e20;" text-anchor="middle">En haut</text>';
    }
...
function displayCalculatorResults(solutions, params) {
    ...
    if (!solutions || solutions.length === 0) {
        ...
        if (flightHeightExceeded && !hasIntermediateLanding) {
            ...
            if (stairConfigValue === 'l_shaped' && lShapedConfigValue === 'standard_landing') {
                ...
                if (treadTooSmall) {
                    if (isMetric) {
                        html += `<li>Longueur totale minimale requise â‰¥ ${minLengthWithLanding.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm + 2Ã— palier)</li>`;
                    } else {
                        html += `<li>Longueur totale minimale requise â‰¥ ${metricToImperial(minLengthWithLanding)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)} + 2Ã— palier)</li>`;
                    }
                ...
                if (treadTooSmall) {
                    if (isMetric) {
                        html += `<li>Longueur totale minimale requise â‰¥ ${minLength.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm)</li>`;
                    } else {
                        html += `<li>Longueur totale minimale requise â‰¥ ${metricToImperial(minLength)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)})</li>`;
                    }
                ...
                html += '<li>Les marches rayonnantes se situent Ã Â  l\'intersection des deux volÃ©es</li>';
            } else {
                if (treadTooSmall) {
                    if (isMetric) {
                        html += `<li>Longueur minimale requise â‰¥ ${minLength.toFixed(0)} mm (${minTreads} girons Ã— ${limits.minTread} mm)</li>`;
                    } else {
                        html += `<li>Longueur minimale requise â‰¥ ${metricToImperial(minLength)} (${minTreads} girons Ã— ${metricToImperial(limits.minTread)})</li>`;
                    }
    ...
    if (!isWidthOk) {
        html += `<div class="warning"><p>âš   Largeur ${formatValue(stairWidthValue, isMetric, 0)} infÃ©rieure au minimum requis (${formatValue(limits.minWidth, isMetric, 0)})</p></div>`;
    }
    if (!isRiserOk) {
        html += `<div class="warning"><p>âš   Hauteur de contremarche ${formatValuePrecise(best.riserHeight, isMetric)} hors limites CNB (${formatValue(limits.minRiser, isMetric, 0)} Ã Â  ${formatValue(limits.maxRiser, isMetric, 0)})</p></div>`;
    }
    if (!isTreadOk) {
        if (hasMaxTreadLimit) {
            html += `<div class="warning"><p>âš   Profondeur de giron ${formatValuePrecise(best.treadDepth, isMetric)} hors limites CNB (${formatValue(limits.minTread, isMetric, 0)} Ã Â  ${formatValue(limits.maxTread, isMetric, 0)})</p></div>`;
        } else {
            html += `<div class="warning"><p>âš   Profondeur de giron ${formatValuePrecise(best.treadDepth, isMetric)} infÃ©rieure au minimum CNB (${formatValue(limits.minTread, isMetric, 0)})</p></div>`;
        }
    }
    ...
    if (best.totalRadiatingSteps > 0) {
        html += `<li><strong>Girons rectangulaires :</strong> ${best.numRectTreads} Ã— ${formatValuePrecise(best.treadDepth, isMetric)}</li>`;
        html += `<li><strong>Marches rayonnantes :</strong> ${best.totalRadiatingSteps} (${best.totalRadiatingSteps === 4 ? '2Ã—45Â°' : '2Ã—3Ã—30Â°'})</li>`;
    ...
    if (best.useLandingConfiguration) {
        ...
        if (best.flight1Height && best.flight2Height) {
            ...
            if (!flight1Ok || !flight2Ok) {
                html += '</ul></div>';
                html += '<div class="warning"><p><strong>âš   Non-conformitÃ© CNB 9.8.3.3 :</strong> ';
                if (!flight1Ok && !flight2Ok) {
    ...
    if (isMetric) {
        ...
        html += riseError < 0.01 ? 'âœ“ Exact' : `âš    Ã‰cart ${riseError.toFixed(4)} mm`;
        ...
        if (best.useLandingConfiguration) {
            ...
            html += treadError < 0.01 ? 'âœ“ Exact' : `âš    Ã‰cart ${treadError.toFixed(4)} mm`;
        }
    } else {
    ...
    if (stairConfigValue === 'l_shaped' && (lShapedConfigValue === 'two_45deg' || lShapedConfigValue === 'three_30deg')) {
        ...
        html += '<ul>';
        html += '<li>Les marches rayonnantes se situent Ã Â  l\'intersection des deux volÃ©es perpendiculaires</li>';
        ...
        if (best.firstFlightRectTreads !== undefined) {
            ...
            html += '<li>  â†³Ã‚Â  1Ã¨re direction : ' + best.firstFlightRectTreads + ' girons Ã— ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
            ...
            html += '<li>  â†³Ã‚Â  2Ã¨me direction : ' + best.secondFlightRectTreads + ' girons Ã— ' + formatValuePrecise(best.treadDepth, isMetric) + '</li>';
        ...
        html += '<li>Giron mesurÃ© Ã Â  500 mm de la rive Ã©troite (CNB)</li>';
        html += '</ul></div>';
    }
    ...
    if (isCompliant && stairConfigValue === 'l_shaped' && (best.useLandingConfiguration || best.numRadiatingSteps > 0)) {
        ...
        html += generateLShapedStairVisualization({
            ...
            // Pour marches rayonnantes Ã Â  l'extrÃ©mitÃ©
    ...
    if (isCompliant && stairConfigValue === 'u_shaped' && uShapedIsMixed) {
        ...
        if (uShapedConfigValue === 'rect_landing_radiating_rect' || uShapedConfigValue === 'rect_radiating_landing_rect') {
            ...
            const uFlight3 = best.flight3Run || params.uThirdFlightRunValue || 1000;
            const uLandingWidth = params.uLandingWidthValue || (2 * stairWidthValue);
            const uLandingDepth = params.uLandingDepthValue || stairWidthValue;
            const radiatingAngle = params.uRadiatingAngleValue || best.radiatingAngle || 45;
            ...
            // Pour rect_radiating_landing_radiating_rect (configuration plus complexe)
            let configDescription = 'marches rectangulaires + marches rayonnantes + palier + marches rayonnantes + marches rectangulaires';
    ...
    if (best.isSpiral) {
        ...
        html += '<ul>';
        html += '<li>Giron mesurÃ© Ã Â  300 mm de l\'axe de la main courante</li>';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ===== Mise Ã Â  jour des placeholders et visibilitÃ© selon le systÃ¨me de mesure =====
    ...
    function updateCalcLShapedSubOptions() {
        ...
        // (les marches rayonnantes se situent Ã Â  l'intersection des deux volÃ©es)
        calcStandardRunContainer.style.display = 'none';
    ...
    function updateCalcUShapedSubOptions() {
        ...
        // Configurations avec 3 volÃ©es (combinaison palier + marches rayonnantes)
        ...
        // Afficher/masquer le sÃ©lecteur d'angle des marches rayonnantes
        if (calcURadiatingAngleRow) {
            calcURadiatingAngleRow.style.display = hasRadiating ? 'block' : 'none';
        }
        
        // Afficher/masquer la 3Ã¨me volÃ©e
        if (uThirdFlightRow) {
            uThirdFlightRow.style.display = hasThreeFlights ? 'block' : 'none';
        }
        
        // Afficher/masquer les champs du palier (toujours visible pour les configurations conformes au CNB)
        if (uLandingRow) {
            uLandingRow.style.display = hasLanding ? 'block' : 'none';
        }
        
        // Mettre Ã Â  jour les labels des volÃ©es selon la configuration
    ...
    function updateUFlightLabels(config) {
        ...
        switch (config) {
            case 'rect_landing_rect':
                // 2 volÃ©es sÃ©parÃ©es par un palier
                label1.innerHTML = 'Longueur 1Ã¨re volÃ©e : <span class="info-icon" title="Distance horizontale de la premiÃ¨re volÃ©e (marches rectangulaires)">i</span>';
                label2.innerHTML = 'Longueur 2Ã¨me volÃ©e : <span class="info-icon" title="Distance horizontale de la deuxiÃ¨me volÃ©e (marches rectangulaires)">i</span>';
                break;
            case 'rect_landing_radiating_rect':
                // 3 volÃ©es: palier puis marches rayonnantes
                label1.innerHTML = 'Longueur 1Ã¨re partie : <span class="info-icon" title="Distance horizontale avant le palier">i</span>';
                label2.innerHTML = 'Longueur 2Ã¨me partie : <span class="info-icon" title="Distance horizontale entre le palier et les marches rayonnantes">i</span>';
                if (label3) label3.innerHTML = 'Longueur 3Ã¨me partie : <span class="info-icon" title="Distance horizontale aprÃ¨s les marches rayonnantes">i</span>';
                break;
            case 'rect_radiating_landing_rect':
                // 3 volÃ©es: marches rayonnantes puis palier
                label1.innerHTML = 'Longueur 1Ã¨re partie : <span class="info-icon" title="Distance horizontale avant les marches rayonnantes">i</span>';
                label2.innerHTML = 'Longueur 2Ã¨me partie : <span class="info-icon" title="Distance horizontale entre les marches rayonnantes et le palier">i</span>';
                if (label3) label3.innerHTML = 'Longueur 3Ã¨me partie : <span class="info-icon" title="Distance horizontale aprÃ¨s le palier">i</span>';
                break;
            case 'rect_radiating_landing_radiating_rect':
                // 3 volÃ©es: marches rayonnantes + palier + marches rayonnantes
                label1.innerHTML = 'Longueur 1Ã¨re partie : <span class="info-icon" title="Distance horizontale avant les premiÃ¨res marches rayonnantes">i</span>';
                label2.innerHTML = 'Longueur 2Ã¨me partie : <span class="info-icon" title="Distance horizontale de la partie centrale (avec palier)">i</span>';
                if (label3) label3.innerHTML = 'Longueur 3Ã¨me partie : <span class="info-icon" title="Distance horizontale aprÃ¨s les derniÃ¨res marches rayonnantes">i</span>';
                break;
        }
    }
    ...
    inputPairs.forEach(pair => {
        if (pair.metric && pair.imperial) {
            ...
            // Nettoyer/normaliser seulement Ã Â  la perte de focus
    ...
    function performCalculation() {
        ...
        if (stairConfigValue === 'l_shaped') {
            ...
            // Validation: Largeur palier â‰¥ 2 Ã— largeur escalier
            ...
            // Validation: Profondeur palier â‰¥ largeur escalier (CNB 9.8.6.3)
            if (uLandingDepthValue > 0 && stairWidthValue > 0 && uLandingDepthValue < stairWidthValue) {
                ...
                if (errEl) {
                    if (isMetric) {
                        errEl.textContent = `La profondeur du palier (${uLandingDepthValue.toFixed(0)} mm) ne peut Ãªtre infÃ©rieure Ã Â  la largeur de l'escalier (${stairWidthValue.toFixed(0)} mm) selon CNB 9.8.6.3.`;
                    } else {
                        errEl.textContent = `La profondeur du palier ne peut Ãªtre infÃ©rieure Ã Â  la largeur de l'escalier (min. ${metricToImperial(stairWidthValue)}) selon CNB 9.8.6.3.`;
                    }
                }
                isValid = false;
            }
        ...
        // Marches rayonnantes dans escaliers en U: permises uniquement dans logements privÃ©s (Partie 9)
        ...
        if (uShapedHasRadiating) {
            if (buildingTypeValue === 'part3') {
                contentDiv.innerHTML = '<h3>âœ— Configuration non permise</h3>' +
                    ...
                    '<p><strong>Solutions :</strong></p><ul>' +
                    '<li>Utilisez un palier standard au lieu des marches rayonnantes</li>' +
                    '<li>SÃ©lectionnez la configuration "Marche(s) rectangulaire(s) + palier + marche(s) rectangulaire(s)"</li></ul></div>';
                resultDiv.className = 'result non-compliant';
                resultDiv.style.display = 'block';
                return;
            }
    ...
    function performVerification() {
        ...
        // VÃ©rifications spÃ©cifiques Ã Â  la configuration
        if (stairConfigValue === 'dancing_steps' && narrowSideValue > 0) {
            if (narrowSideValue < limits.minNarrowSide) {
                issues.push(`Giron cÃ´tÃ© Ã©troit ${narrowSideValue.toFixed(0)} mm < minimum ${limits.minNarrowSide} mm Ã Â  300 mm de l'axe`);
                isCompliant = false;
            }
        ...
        if (stairConfigValue === 'spiral') {
            ...
            if (spiralTreadAt300Value > 0 && spiralTreadAt300Value < CNB_LIMITS.spiral.minTreadAt300) {
                issues.push(`Giron Ã Â  300 mm ${spiralTreadAt300Value.toFixed(0)} mm < minimum ${CNB_LIMITS.spiral.minTreadAt300} mm (CNB 9.8.4.7.(1)d)`);
                isCompliant = false;
            }
     ...
     * RÃ¨gles:
     * - Largeur palier â‰¥ 2 Ã— largeur escalier (gÃ©omÃ©trie)
    ...
    function validateUShapedInputs() {
        ...
        // Valider: Largeur palier â‰¥ 2 Ã— largeur escalier
        ...
        if (landingDepthError) {
            if (stairWidth > 0 && landingDepth > 0 && landingDepth < stairWidth) {
                if (isMetric) {
                    landingDepthError.textContent = `La profondeur du palier (${landingDepth.toFixed(0)} mm) ne peut Ãªtre infÃ©rieure Ã Â  la largeur de l'escalier (${stairWidth.toFixed(0)} mm) selon CNB 9.8.6.3.`;
                } else {
                    landingDepthError.textContent = `La profondeur du palier ne peut Ãªtre infÃ©rieure Ã Â  la largeur de l'escalier (min. ${metricToImperial(stairWidth)}) selon CNB 9.8.6.3.`;
                }
            } else {
        ...
        // Aussi valider la largeur de l'escalier si les dimensions du palier sont dÃ©jÃ Â  saisies
        const stairWidthError = document.getElementById('stairDesiredWidthError');
        if (stairWidthError && stairWidth > 0) {
            // VÃ©rifier si largeur escalier est compatible avec largeur palier dÃ©jÃ Â  saisie
    ...
    function validateLShapedInputs() {
        ...
        // Pour l'escalier en L, la profondeur du palier est Ã©gale Ã Â  la largeur de l'escalier
     ...
     * - Giron Ã Â  300 mm â‰¥ 190 mm (CNB 9.8.4.7)
     */
