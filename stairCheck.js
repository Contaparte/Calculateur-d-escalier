...
function calculateDancingSteps(params) {
 ...
 for (let numRisers = minRisers; numRisers <= maxRisers; numRisers++) {
  ...
  // Vérification CNB 9.8.4.5: uniformité des girons dans les volées à marches mixtes
  // Paragraphe (2): Si les marches dansantes sont au bas de la volée, leur giron
  // PEUT ÊTRE SUPÉRIEUR à celui des marches rectangulaires
  // Donc: treadAt300 >= rectTreadDepth est toujours acceptable
  // Si treadAt300 < rectTreadDepth, la différence doit respecter les tolérances (10mm max)
  const treadVariation = treadAt300 - rectTreadDepth;
  
  // Si giron dansantes < giron rectangulaire de plus de 10mm, rejeter
  // (CNB 9.8.4.4 tolérances: max 10mm de variation dans une volée)
  if (treadVariation < -10) {
   rejectionStats.treadVariationTooHigh.push({
    numRisers, numRectTreads, rectTreadDepth, treadVariation: Math.abs(treadVariation), riserHeight
   });
   continue;
  }
  
  // Note: Si treadVariation >= 0 (giron dansantes >= rectangulaire), c'est conforme CNB 9.8.4.5.(2)
