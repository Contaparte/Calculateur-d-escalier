...
function calculateDancingSteps(params) {
 ...
 for (let numRisers = minRisers; numRisers <= maxRisers; numRisers++) {
  ...
  // Vérification CNB 9.8.4.5: uniformité des girons (variation max 25mm)
  const treadVariation = Math.abs(treadAt300 - rectTreadDepth);
  if (treadVariation > 25) {
   rejectionStats.treadVariationTooHigh.push({
    numRisers, numRectTreads, rectTreadDepth, treadVariation, riserHeight
   });
   continue;
  }
...
function generateDancingNoSolutionError(stats, params) {
 ...
 // 4. Variation de giron trop élevée (CNB 9.8.4.5)
 if (stats.treadVariationTooHigh.length > 0) {
  ...
  html += `<p style="margin:0 0 8px 0;font-weight:600;color:#e65100;">Variation de giron trop élevée (CNB 9.8.4.5) : ${stats.treadVariationTooHigh.length} configuration(s)</p>`;
  ...
  html += `<p style="margin:0;font-size:12px;color:#666;">Variation minimale atteinte : <strong>${minVar.toFixed(0)} mm</strong> (max. permis : 25 mm)</p>`;
  
  // Calculer le rayon idéal
  const targetTread = (minRectTread + maxRectTread) / 2;
  const idealRadius = Math.round((targetTread / ((dancingAngle / dancingNumSteps) * Math.PI / 180)) - 300);
  
  html += `<p style="margin:8px 0 0 0;font-size:12px;"><strong>Solution :</strong> Ajuster le rayon intérieur à environ <strong>${idealRadius} mm</strong> pour un giron dansantes de ~${targetTread.toFixed(0)} mm.</p>`;
  html += '</div>';
 }
