...
function generateUShapedStairVisualization(stairData) {
 ...
 // Largeur totale = largeur du palier (2ème partie)
 // Hauteur totale = max des segments verticaux (qui incluent déjà le palier)
 const totalPlanWidth = actualLandingWidth;
 const maxFlightRun = Math.max(actualFlight1Run, actualFlight3Run);
 const totalPlanHeight = maxFlightRun;
...
function generateUShapedMixedVisualization(stairData) {
 ...
 // Calculer l'échelle
 // Largeur = segment horizontal (2ème partie)
 // Hauteur = max des segments verticaux (qui incluent la zone virage)
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 function updateCalcUShapedSubOptions() {
  ...
  // Afficher/masquer les champs du palier
  // Pour les configs avec rayonnantes: cacher "Largeur du palier" (redondant avec 2ème partie)
  // Garder seulement "Profondeur du palier"
  if (uLandingWidthRow) {
  // Largeur du palier visible seulement pour rect_landing_rect (sans rayonnantes)
  const showLandingWidth = hasLanding && !hasRadiating;
  uLandingWidthRow.style.display = showLandingWidth ? 'block' : 'none';
  }
  if (uLandingDepthRow) {
  // Profondeur du palier toujours visible si palier présent
